...
from rl_quant.features.massive_economic_event_source_v4 import (
    ...
    MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SPEC_SHA256,
    MassiveEconomicEventAuthorityV4,
    MassiveSourcedCashReturnV4,
    MassiveSourcedCorporateActionV4,
    MassiveSourcedTerminalEventV4,
    build_massive_economic_event_authority_v4,
    sourced_economic_event_order_v4,
)
...
MASSIVE_ECONOMIC_HISTORY_V4_SCHEMA = "rl-quant.massive-economic-history-at-origin-v4"
MASSIVE_ECONOMIC_HISTORY_V4_DATASET = "massive-economic-history-at-origin-v4"
MASSIVE_ECONOMIC_HISTORY_V4_SOURCE_SHA256 = file_sha256(Path(__file__))
MASSIVE_ECONOMIC_HISTORY_V4_SPEC_SHA256 = semantic_sha256(
    {
        ...
        "economic_event_source_spec": MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SPEC_SHA256,
        ...
        "tie_policy": "duplicate-global-economic-order-fails-closed",
        "base_global_economic_sequence": MASSIVE_ECONOMIC_BASE_GLOBAL_SEQUENCE_V4,
...
MASSIVE_ECONOMIC_HISTORY_V4_SOURCE_SCHEMA_SHA256 = semantic_sha256(
    {
        "schema": MASSIVE_ECONOMIC_HISTORY_V4_SCHEMA,
...
class MassiveEconomicHistoryV4Error(ValueError):
...
def _digest(name: str, value: object) -> str:
    if (
        ...
        raise MassiveEconomicHistoryV4Error(f"{name} must be a lowercase SHA-256")
    return value


def _nonnegative_int(name: str, value: object) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise MassiveEconomicHistoryV4Error(f"{name} must be nonnegative")
    return value


@dataclass(frozen=True, slots=True)
class MassiveEconomicHistoryOriginV4:
    ...
    def validate(self) -> None:
        try:
            ...
            raise MassiveEconomicHistoryV4Error(
                "economic origin date is invalid"
            ) from exc
        if source_date >= decision_date or self.source_staleness_sessions != 2:
            raise MassiveEconomicHistoryV4Error("economic origin chronology drifted")
        ...
        if self.receipt_sha256 != semantic_sha256(self.unsigned()):
            raise MassiveEconomicHistoryV4Error("economic origin receipt differs")


def build_massive_economic_history_origin_v4(
    *,
    session_authority: MassiveSessionAuthority,
    source_session_date: str,
    decision_origin_receipt_sha256: str,
) -> MassiveEconomicHistoryOriginV4:
    ...
    provisional = MassiveEconomicHistoryOriginV4(
...
class MassiveVintageHoldingV4:
    ...
    def validate(self) -> None:
        if (
            ...
            raise MassiveEconomicHistoryV4Error("vintage holding is invalid")
        ...
        if sequence > MASSIVE_ECONOMIC_BASE_GLOBAL_SEQUENCE_V4:
            raise MassiveEconomicHistoryV4Error(
                "holding acquisition sequence exceeds the base sentinel"
            )
        is_base_id = self.acquired_event_id.startswith("BASE:")
        if (sequence == MASSIVE_ECONOMIC_BASE_GLOBAL_SEQUENCE_V4) != is_base_id:
            raise MassiveEconomicHistoryV4Error(
...
@dataclass(frozen=True, slots=True)
class MassiveVintageEconomicPositionV4:
    holdings: tuple[MassiveVintageHoldingV4, ...]
    cash: float
    ...
    def validate(self) -> None:
        ...
        if keys != sorted(set(keys)):
            raise MassiveEconomicHistoryV4Error("vintage holdings are not canonical")
        if not math.isfinite(self.cash) or self.cash < 0.0:
            raise MassiveEconomicHistoryV4Error("vintage position cash is invalid")
        if (
...
@dataclass(frozen=True, slots=True)
class MassiveEconomicHistoryRowV4:
    ...
    terminal: bool
    position: MassiveVintageEconomicPositionV4 | None
    ...
    def validate(self) -> None:
        ...
        if any(
            ...
            raise MassiveEconomicHistoryV4Error("economic history flags are invalid")
        if not math.isfinite(self.economic_value) or self.economic_value < 0.0:
            raise MassiveEconomicHistoryV4Error("economic history value is invalid")
        ...
        if self.receipt_sha256 != semantic_sha256(self.unsigned()):
            raise MassiveEconomicHistoryV4Error("economic history row receipt differs")


@dataclass(frozen=True, slots=True)
class MassiveEconomicHistoryAtOriginV4:
    origin: MassiveEconomicHistoryOriginV4
    ...
    rows: tuple[MassiveEconomicHistoryRowV4, ...]
    row_count: int
    valid_row_count: int
    ...
    receipt_sha256: str
    schema: str = MASSIVE_ECONOMIC_HISTORY_V4_SCHEMA
    ...
    def validate(self) -> None:
        if self.schema != MASSIVE_ECONOMIC_HISTORY_V4_SCHEMA:
            raise MassiveEconomicHistoryV4Error("economic history schema drifted")
        ...
        if (
            self.feature_spec_receipt_sha256 != MASSIVE_ECONOMIC_HISTORY_V4_SPEC_SHA256
            or self.feature_source_sha256 != MASSIVE_ECONOMIC_HISTORY_V4_SOURCE_SHA256
        ...
        if (
            self.loaded_source.receipt.dataset_id != MASSIVE_ECONOMIC_HISTORY_V4_DATASET
            or self.loaded_source.receipt.schema_sha256
            != MASSIVE_ECONOMIC_HISTORY_V4_SOURCE_SCHEMA_SHA256
        ):
            raise MassiveEconomicHistoryV4Error(
...
def _event_identity(row: EconomicSourceEventV4) -> str:
    if isinstance(row, MassiveSourcedCashReturnV4):
        return row.event_id
    return row.event.event_id


def _event_order(row: EconomicSourceEventV4) -> tuple[int, int]:
    return sourced_economic_event_order_v4(row)


def _available_at(row: EconomicSourceEventV4) -> int:
    if isinstance(row, MassiveSourcedCashReturnV4):
...
def _normalized_position(
    *,
    holdings: Sequence[MassiveVintageHoldingV4],
    cash: float,
    applied: Sequence[str],
    excluded: Sequence[str],
) -> MassiveVintageEconomicPositionV4:
    ...
    position = MassiveVintageEconomicPositionV4(
        holdings=tuple(
            MassiveVintageHoldingV4(
                security_id=key[0],
...
def _exclude_event(
    position: MassiveVintageEconomicPositionV4, event_id: str
) -> MassiveVintageEconomicPositionV4:
...
def _apply_cash_return(
    position: MassiveVintageEconomicPositionV4,
    row: MassiveSourcedCashReturnV4,
) -> MassiveVintageEconomicPositionV4:
    return _normalized_position(
        holdings=position.holdings,
...
def _apply_security_event(
    position: MassiveVintageEconomicPositionV4,
    row: MassiveSourcedCorporateActionV4 | MassiveSourcedTerminalEventV4,
) -> MassiveVintageEconomicPositionV4:
    event = row.event
    ...
    holdings: list[MassiveVintageHoldingV4] = [
        holding
        for holding in position.holdings
        if (
            holding.security_id,
    ...
    if isinstance(row, MassiveSourcedTerminalEventV4):
        if event.kind is TerminalEventKind.MERGER_STOCK:
            ...
            holdings.append(
                MassiveVintageHoldingV4(
                    ...
                    acquired_global_economic_sequence=row.global_economic_sequence,
...
def _apply_corporate_event(
    *,
    position: MassiveVintageEconomicPositionV4,
    row: MassiveSourcedCorporateActionV4,
    event: CorporateActionRecord,
    event_id: str,
    eligible: tuple[MassiveVintageHoldingV4, ...],
    holdings: list[MassiveVintageHoldingV4],
    cash: float,
    ...
    if event.kind in {CorporateActionKind.SPLIT, CorporateActionKind.REVERSE_SPLIT}:
        ...
        raise MassiveEconomicHistoryV4Error("economic event kind is unsupported")
    return _normalized_position(
        holdings=holdings,
        cash=cash,
        applied=(*position.applied_event_ids, event_id),
...
def _mark_position(
    position: MassiveVintageEconomicPositionV4, marks: Mapping[str, float]
) -> float:
    value = position.cash
    for holding in position.holdings:
        if holding.security_id not in marks:
            raise MassiveEconomicHistoryV4Error("held successor has no economic mark")
        mark = float(marks[holding.security_id])
        if not math.isfinite(mark) or mark < 0.0:
            raise MassiveEconomicHistoryV4Error("economic mark is invalid")
        value += holding.shares * mark
    if not math.isfinite(value) or value < 0.0:
        raise MassiveEconomicHistoryV4Error("marked economic value is invalid")
    return value
...
def _listing_coordinates(
    *,
    ...
    if not sessions:
        raise MassiveEconomicHistoryV4Error("XNYS session authority is empty")
    session_dates = tuple(
...
def _validate_origin(
    *,
    ...
    expected = build_massive_economic_history_origin_v4(
...
def build_massive_economic_history_rows_v4(
    *,
    panel_rows: Sequence[MassiveSessionPanelRowV1],
    economic_authority: MassiveEconomicEventAuthorityV4,
    identity_authority: PITSecurityUniverseAuthority,
    session_authority: MassiveSessionAuthority,
    origin: MassiveEconomicHistoryOriginV4,
) -> tuple[tuple[MassiveEconomicHistoryRowV4, ...], tuple[str, ...], tuple[str, ...]]:
    ...
    if (
        ...
        raise MassiveEconomicHistoryV4Error("economic and identity authorities differ")
    rows = tuple(
        row
    ...
    if not keys or keys != tuple(sorted(set(keys))):
        raise MassiveEconomicHistoryV4Error("economic panel rows are not canonical")
    ...
    all_events: tuple[EconomicSourceEventV4, ...] = (
    ...
    output: list[MassiveEconomicHistoryRowV4] = []
    session_ordinal = {
    ...
    for origin_security_id in security_ids:
        position: MassiveVintageEconomicPositionV4 | None = None
        for session_index, group in by_index.items():
                ...
                position = _normalized_position(
                    holdings=(
                        MassiveVintageHoldingV4(
                            ...
                            acquired_global_economic_sequence=MASSIVE_ECONOMIC_BASE_GLOBAL_SEQUENCE_V4,
            ...
            if position is not None:
                for event in admitted:
                    ...
                    if (
                        ...
                        continue
                    if isinstance(event, MassiveSourcedCashReturnV4):
            ...
            if position is not None:
                try:
                    ...
                    valid = True
                except MassiveEconomicHistoryV4Error:
                    value = 0.0
            (
                listing_at_ms,
            ...
            provisional = MassiveEconomicHistoryRowV4(
...
def _payload(artifact: MassiveEconomicHistoryAtOriginV4) -> dict[str, object]:
    return {
...
def materialize_massive_economic_history_v4(
    *,
    ...
    published_at_ms: int,
) -> MassiveEconomicHistoryAtOriginV4:
    ...
    expected_authority = build_massive_economic_event_authority_v4(
    ...
    relative = (
        "massive-profitability-p0/economic-history-v4/"
        f"{origin.source_session_date}-{origin.decision_session_date}.json"
    )
    placeholder = MassiveEconomicHistoryAtOriginV4(
        origin=origin,
        ...
        feature_spec_receipt_sha256=MASSIVE_ECONOMIC_HISTORY_V4_SPEC_SHA256,
        feature_source_sha256=MASSIVE_ECONOMIC_HISTORY_V4_SOURCE_SHA256,
    ...
    publish_massive_source_object(
        ...
        dataset_id=MASSIVE_ECONOMIC_HISTORY_V4_DATASET,
        source_object_key=relative,
        requested_at_ms=published_at_ms,
        downloaded_at_ms=published_at_ms,
        schema_sha256=MASSIVE_ECONOMIC_HISTORY_V4_SOURCE_SCHEMA_SHA256,
    ...
    validate_massive_economic_history_v4(root=output_root, artifact=result)
    return result


def validate_massive_economic_history_v4(
    *, root: str | Path, artifact: MassiveEconomicHistoryAtOriginV4
) -> None:
    artifact.validate()
    ...
    if raw != canonical_json_file_bytes(payload) or raw != canonical_json_file_bytes(
        ...
        raise MassiveEconomicHistoryV4Error("economic history bytes differ")


__all__ = [
    "MASSIVE_ECONOMIC_HISTORY_V4_DATASET",
    "MASSIVE_ECONOMIC_HISTORY_V4_SOURCE_SCHEMA_SHA256",
    "MASSIVE_ECONOMIC_HISTORY_V4_SPEC_SHA256",
    "MassiveEconomicHistoryAtOriginV4",
    "MassiveEconomicHistoryOriginV4",
    "MassiveEconomicHistoryRowV4",
    "MassiveEconomicHistoryV4Error",
    "MassiveVintageEconomicPositionV4",
    "MassiveVintageHoldingV4",
