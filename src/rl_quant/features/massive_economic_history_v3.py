...
from rl_quant.features.massive_economic_event_source_v3 import (
...
MASSIVE_ECONOMIC_HISTORY_V3_SCHEMA = "rl-quant.massive-economic-history-at-origin-v3"
MASSIVE_ECONOMIC_HISTORY_V3_DATASET = "massive-economic-history-at-origin-v3"
MASSIVE_ECONOMIC_HISTORY_V3_SOURCE_SHA256 = file_sha256(Path(__file__))
MASSIVE_ECONOMIC_HISTORY_V3_SPEC_SHA256 = semantic_sha256(
    {
        ...
        "economic_event_source_spec": MASSIVE_ECONOMIC_EVENT_SOURCE_V3_SPEC_SHA256,
        ...
        "holding_vintage": "event-applies-only-to-lots-acquired-before-complete-event-order",
        "successor_vintage": "effective-time-sequence-and-event-id",
        "tie_break": "globally-authoritative-effective-time-sequence-event-id",
...
MASSIVE_ECONOMIC_HISTORY_V3_SOURCE_SCHEMA_SHA256 = semantic_sha256(
    {
        "schema": MASSIVE_ECONOMIC_HISTORY_V3_SCHEMA,
...
class MassiveEconomicHistoryV3Error(ValueError):
...
def _digest(name: str, value: object) -> str:
    if (
        ...
        raise MassiveEconomicHistoryV3Error(f"{name} must be a lowercase SHA-256")
    return value


def _nonnegative_int(name: str, value: object) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise MassiveEconomicHistoryV3Error(f"{name} must be nonnegative")
    return value


@dataclass(frozen=True, slots=True)
class MassiveEconomicHistoryOriginV3:
    ...
    def validate(self) -> None:
        try:
            ...
            raise MassiveEconomicHistoryV3Error(
                "economic origin date is invalid"
            ) from exc
        if source_date >= decision_date or self.source_staleness_sessions != 2:
            raise MassiveEconomicHistoryV3Error("economic origin chronology drifted")
        ...
        if self.receipt_sha256 != semantic_sha256(self.unsigned()):
            raise MassiveEconomicHistoryV3Error("economic origin receipt differs")


def build_massive_economic_history_origin_v3(
    *,
    session_authority: MassiveSessionAuthority,
    source_session_date: str,
    decision_origin_receipt_sha256: str,
) -> MassiveEconomicHistoryOriginV3:
    ...
    provisional = MassiveEconomicHistoryOriginV3(
...
class MassiveVintageHoldingV3:
    ...
    def validate(self) -> None:
        if (
            ...
            raise MassiveEconomicHistoryV3Error("vintage holding is invalid")
        _nonnegative_int("holding acquisition time", self.acquired_effective_at_ms)
        sequence = _nonnegative_int(
            "holding acquisition sequence", self.acquired_event_sequence
        )
        ...
        if (sequence == MASSIVE_ECONOMIC_BASE_SEQUENCE_V3) != is_base_id:
            raise MassiveEconomicHistoryV3Error(
                "base acquisition identity and sequence differ"
            )

    @property
    def acquisition_order(self) -> tuple[int, int, str]:
        return (
            self.acquired_effective_at_ms,
            self.acquired_event_sequence,
            self.acquired_event_id,
        )


@dataclass(frozen=True, slots=True)
class MassiveVintageEconomicPositionV3:
    holdings: tuple[MassiveVintageHoldingV3, ...]
    cash: float
    ...
    def validate(self) -> None:
        ...
        if keys != sorted(set(keys)):
            raise MassiveEconomicHistoryV3Error("vintage holdings are not canonical")
        if not math.isfinite(self.cash) or self.cash < 0.0:
            raise MassiveEconomicHistoryV3Error("vintage position cash is invalid")
        if (
...
@dataclass(frozen=True, slots=True)
class MassiveEconomicHistoryRowV3:
    ...
    terminal: bool
    position: MassiveVintageEconomicPositionV3 | None
    ...
    def validate(self) -> None:
        ...
        if any(
            ...
            raise MassiveEconomicHistoryV3Error("economic history flags are invalid")
        if not math.isfinite(self.economic_value) or self.economic_value < 0.0:
            raise MassiveEconomicHistoryV3Error("economic history value is invalid")
        ...
        if self.receipt_sha256 != semantic_sha256(self.unsigned()):
            raise MassiveEconomicHistoryV3Error("economic history row receipt differs")


@dataclass(frozen=True, slots=True)
class MassiveEconomicHistoryAtOriginV3:
    origin: MassiveEconomicHistoryOriginV3
    ...
    rows: tuple[MassiveEconomicHistoryRowV3, ...]
    row_count: int
    valid_row_count: int
    ...
    receipt_sha256: str
    schema: str = MASSIVE_ECONOMIC_HISTORY_V3_SCHEMA
    ...
    def validate(self) -> None:
        if self.schema != MASSIVE_ECONOMIC_HISTORY_V3_SCHEMA:
            raise MassiveEconomicHistoryV3Error("economic history schema drifted")
        ...
        if (
            self.feature_spec_receipt_sha256 != MASSIVE_ECONOMIC_HISTORY_V3_SPEC_SHA256
            or self.feature_source_sha256 != MASSIVE_ECONOMIC_HISTORY_V3_SOURCE_SHA256
        ...
        if (
            self.loaded_source.receipt.dataset_id != MASSIVE_ECONOMIC_HISTORY_V3_DATASET
            or self.loaded_source.receipt.schema_sha256
            != MASSIVE_ECONOMIC_HISTORY_V3_SOURCE_SCHEMA_SHA256
        ):
            raise MassiveEconomicHistoryV3Error(
...
def _event_identity(row: EconomicSourceEventV3) -> str:
    if isinstance(row, MassiveSourcedCashReturnV3):
        return row.event_id
    return row.event.event_id


def _event_order(row: EconomicSourceEventV3) -> tuple[int, int, str]:
    return sourced_economic_event_order_v3(row)


def _available_at(row: EconomicSourceEventV3) -> int:
    if isinstance(row, MassiveSourcedCashReturnV3):
...
def _normalized_position(
    *,
    holdings: Sequence[MassiveVintageHoldingV3],
    cash: float,
    applied: Sequence[str],
    excluded: Sequence[str],
) -> MassiveVintageEconomicPositionV3:
    ...
    position = MassiveVintageEconomicPositionV3(
        holdings=tuple(
            MassiveVintageHoldingV3(
                security_id=key[0],
...
def _exclude_event(
    position: MassiveVintageEconomicPositionV3, event_id: str
) -> MassiveVintageEconomicPositionV3:
...
def _apply_cash_return(
    position: MassiveVintageEconomicPositionV3,
    row: MassiveSourcedCashReturnV3,
) -> MassiveVintageEconomicPositionV3:
    return _normalized_position(
        holdings=position.holdings,
...
def _apply_security_event(
    position: MassiveVintageEconomicPositionV3,
    row: MassiveSourcedCorporateActionV3 | MassiveSourcedTerminalEventV3,
) -> MassiveVintageEconomicPositionV3:
    event = row.event
    ...
    holdings: list[MassiveVintageHoldingV3] = [
        holding
        for holding in position.holdings
        if (
            holding.security_id,
    ...
    if isinstance(row, MassiveSourcedTerminalEventV3):
...
def _apply_corporate_event(
    *,
    position: MassiveVintageEconomicPositionV3,
    row: MassiveSourcedCorporateActionV3,
    event: CorporateActionRecord,
    event_id: str,
    eligible: tuple[MassiveVintageHoldingV3, ...],
    holdings: list[MassiveVintageHoldingV3],
    cash: float,
    ...
    if event.kind in {CorporateActionKind.SPLIT, CorporateActionKind.REVERSE_SPLIT}:
        ...
        raise MassiveEconomicHistoryV3Error("economic event kind is unsupported")
    return _normalized_position(
        holdings=holdings,
        cash=cash,
        applied=(*position.applied_event_ids, event_id),
...
def _mark_position(
    position: MassiveVintageEconomicPositionV3, marks: Mapping[str, float]
) -> float:
    value = position.cash
    for holding in position.holdings:
        if holding.security_id not in marks:
            raise MassiveEconomicHistoryV3Error("held successor has no economic mark")
        mark = float(marks[holding.security_id])
        if not math.isfinite(mark) or mark < 0.0:
            raise MassiveEconomicHistoryV3Error("economic mark is invalid")
        value += holding.shares * mark
    if not math.isfinite(value) or value < 0.0:
        raise MassiveEconomicHistoryV3Error("marked economic value is invalid")
    return value
...
def _listing_coordinates(
    *,
    ...
    if not sessions:
        raise MassiveEconomicHistoryV3Error("XNYS session authority is empty")
    session_dates = tuple(
...
def _validate_origin(
    *,
    ...
    expected = build_massive_economic_history_origin_v3(
...
def build_massive_economic_history_rows_v3(
    *,
    panel_rows: Sequence[MassiveSessionPanelRowV1],
    economic_authority: MassiveEconomicEventAuthorityV3,
    identity_authority: PITSecurityUniverseAuthority,
    session_authority: MassiveSessionAuthority,
    origin: MassiveEconomicHistoryOriginV3,
) -> tuple[tuple[MassiveEconomicHistoryRowV3, ...], tuple[str, ...], tuple[str, ...]]:
    ...
    if (
        ...
        raise MassiveEconomicHistoryV3Error("economic and identity authorities differ")
    rows = tuple(
        row
    ...
    if not keys or keys != tuple(sorted(set(keys))):
        raise MassiveEconomicHistoryV3Error("economic panel rows are not canonical")
    ...
    all_events: tuple[EconomicSourceEventV3, ...] = (
    ...
    output: list[MassiveEconomicHistoryRowV3] = []
    session_ordinal = {
    ...
    for origin_security_id in security_ids:
        position: MassiveVintageEconomicPositionV3 | None = None
        for session_index, group in by_index.items():
                ...
                position = _normalized_position(
                    holdings=(
                        MassiveVintageHoldingV3(
                            ...
                            acquired_event_sequence=MASSIVE_ECONOMIC_BASE_SEQUENCE_V3,
            ...
            if position is not None:
                for event in admitted:
                    ...
                    if (
                        ...
                        continue
                    if isinstance(event, MassiveSourcedCashReturnV3):
            ...
            if position is not None:
                try:
                    ...
                    valid = True
                except MassiveEconomicHistoryV3Error:
                    value = 0.0
            (
                listing_at_ms,
            ...
            provisional = MassiveEconomicHistoryRowV3(
...
def _payload(artifact: MassiveEconomicHistoryAtOriginV3) -> dict[str, object]:
    return {
...
def materialize_massive_economic_history_v3(
    *,
    ...
    published_at_ms: int,
) -> MassiveEconomicHistoryAtOriginV3:
    ...
    expected_authority = build_massive_economic_event_authority_v3(
    ...
    rows, admitted, unavailable = build_massive_economic_history_rows_v3(
    ...
    relative = (
        "massive-profitability-p0/economic-history-v3/"
        f"{origin.source_session_date}-{origin.decision_session_date}.json"
    )
    placeholder = MassiveEconomicHistoryAtOriginV3(
        origin=origin,
        ...
        feature_spec_receipt_sha256=MASSIVE_ECONOMIC_HISTORY_V3_SPEC_SHA256,
        feature_source_sha256=MASSIVE_ECONOMIC_HISTORY_V3_SOURCE_SHA256,
    ...
    publish_massive_source_object(
        ...
        dataset_id=MASSIVE_ECONOMIC_HISTORY_V3_DATASET,
        source_object_key=relative,
        requested_at_ms=published_at_ms,
        downloaded_at_ms=published_at_ms,
        schema_sha256=MASSIVE_ECONOMIC_HISTORY_V3_SOURCE_SCHEMA_SHA256,
    ...
    validate_massive_economic_history_v3(root=output_root, artifact=result)
    return result


def validate_massive_economic_history_v3(
    *, root: str | Path, artifact: MassiveEconomicHistoryAtOriginV3
) -> None:
    artifact.validate()
    ...
    if raw != canonical_json_file_bytes(payload) or raw != canonical_json_file_bytes(
        ...
        raise MassiveEconomicHistoryV3Error("economic history bytes differ")


__all__ = [
    "MASSIVE_ECONOMIC_HISTORY_V3_DATASET",
    "MASSIVE_ECONOMIC_HISTORY_V3_SOURCE_SCHEMA_SHA256",
    "MASSIVE_ECONOMIC_HISTORY_V3_SPEC_SHA256",
    "MassiveEconomicHistoryAtOriginV3",
    "MassiveEconomicHistoryOriginV3",
    "MassiveEconomicHistoryRowV3",
    "MassiveEconomicHistoryV3Error",
    "MassiveVintageEconomicPositionV3",
    "MassiveVintageHoldingV3",
    "build_massive_economic_history_origin_v3",
    "build_massive_economic_history_rows_v3",
    "materialize_massive_economic_history_v3",
    "validate_massive_economic_history_v3",
]
