"""Committed, byte-derived economic-event inputs for profitability P0 V4."""
...
MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SCHEMA = "rl-quant.massive-economic-event-source-v4"
MASSIVE_ECONOMIC_EVENT_SOURCE_V4_KINDS = (
    "corporate-actions",
    "terminal-outcomes",
    "cash-returns",
)
MASSIVE_ECONOMIC_EVENT_SOURCE_V4_DATASETS = {
    "corporate-actions": "massive-economic-corporate-actions-v4",
    "terminal-outcomes": "massive-economic-terminal-outcomes-v4",
    "cash-returns": "massive-economic-cash-returns-v4",
}
MASSIVE_ECONOMIC_BASE_GLOBAL_SEQUENCE_V4 = 2**63 - 1
MASSIVE_ECONOMIC_EFFECTIVE_TIMESTAMP_CONTRACTS_V4 = {
    "corporate-actions": {
...
MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SOURCE_SHA256 = file_sha256(Path(__file__))
MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SPEC_SHA256 = semantic_sha256(
    {
        "protocol_receipt": MASSIVE_FINALIZED_PROFITABILITY_P0_RECEIPT_SHA256,
        "source_kinds": MASSIVE_ECONOMIC_EVENT_SOURCE_V4_KINDS,
        ...
        "effective_timestamp_contracts": MASSIVE_ECONOMIC_EFFECTIVE_TIMESTAMP_CONTRACTS_V4,
        "event_order": "globally-unique-effective-time-and-global-economic-sequence",
        "event_id_role": "audit-identity-only-never-economic-order",
        "global_economic_sequence": (
            "provider-derived-or-frozen-canonical-rule-nonnegative-and-"
            "strictly-below-base-sentinel"
        ),
        "base_global_economic_sequence": MASSIVE_ECONOMIC_BASE_GLOBAL_SEQUENCE_V4,
        "identity": "permanent-security-authority-receipt",
    }
)
MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SOURCE_SCHEMA_SHA256 = semantic_sha256(
    {
...
class MassiveEconomicEventSourceV4Error(ValueError):
...
def _digest(name: str, value: object) -> str:
    if (
        ...
        raise MassiveEconomicEventSourceV4Error(f"{name} must be a lowercase SHA-256")
    return value


def _nonnegative_int(name: str, value: object) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise MassiveEconomicEventSourceV4Error(f"{name} must be nonnegative")
    return value
...
def _global_economic_sequence(name: str, value: object) -> int:
    ...
    if sequence >= MASSIVE_ECONOMIC_BASE_GLOBAL_SEQUENCE_V4:
        raise MassiveEconomicEventSourceV4Error(
...
def _text(name: str, value: object) -> str:
    if not isinstance(value, str) or not value or value != value.strip():
        raise MassiveEconomicEventSourceV4Error(f"{name} must be canonical text")
    return value
...
def _number(name: str, value: object) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise MassiveEconomicEventSourceV4Error(f"{name} must be numeric")
    normalized = float(value)
    if not math.isfinite(normalized):
        raise MassiveEconomicEventSourceV4Error(f"{name} must be finite")
    return normalized


def expected_effective_timestamp_contract_v4(
    *, source_kind: str, event_kind: str
) -> str:
    try:
        return MASSIVE_ECONOMIC_EFFECTIVE_TIMESTAMP_CONTRACTS_V4[source_kind][
            event_kind
        ]
...
def economic_event_source_row_receipt_v4(
    *, source_kind: str, record: Mapping[str, object]
) -> str:
    """Derive a row receipt; *record* must omit its own receipt field."""

    if source_kind not in MASSIVE_ECONOMIC_EVENT_SOURCE_V4_KINDS:
        raise MassiveEconomicEventSourceV4Error("economic source kind is unsupported")
...
class MassiveSourcedCorporateActionV4:
    event: CorporateActionRecord
    global_economic_sequence: int
    ...
    def validate(self) -> None:
        ...
        _global_economic_sequence(
            "corporate-action sequence", self.global_economic_sequence
        )
        if (
...
@dataclass(frozen=True, slots=True)
class MassiveSourcedTerminalEventV4:
    ...
    def validate(self) -> None:
        ...
        _global_economic_sequence(
            "terminal-event sequence", self.global_economic_sequence
        )
        if (
            self.effective_timestamp_contract
            != expected_effective_timestamp_contract_v4(
...
@dataclass(frozen=True, slots=True)
class MassiveSourcedCashReturnV4:
    event_id: str
    ...
    def validate(self) -> None:
        ...
        _global_economic_sequence("cash-return sequence", self.global_economic_sequence)
        if (
            self.effective_timestamp_contract
            != expected_effective_timestamp_contract_v4(
...
def sourced_economic_event_order_v4(
    row: MassiveSourcedEconomicEventV4,
) -> tuple[int, int]:
    ...
    if isinstance(row, MassiveSourcedCashReturnV4):
        return (
            ...
            row.global_economic_sequence,
        )
    return (row.event.effective_at_ms, row.global_economic_sequence)


def sourced_economic_event_identity_v4(
    row: MassiveSourcedEconomicEventV4,
) -> str:
...
@dataclass(frozen=True, slots=True)
class MassiveEconomicEventSourceV4:
    source_kind: str
    identity_authority_receipt_sha256: str
    corporate_actions: tuple[MassiveSourcedCorporateActionV4, ...]
    terminal_events: tuple[MassiveSourcedTerminalEventV4, ...]
    cash_returns: tuple[MassiveSourcedCashReturnV4, ...]
    row_inventory_sha256: str
    parser_source_sha256: str
    loaded_source: LoadedMassiveSourceObject
    receipt_sha256: str
    schema: str = MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SCHEMA
    ...
    def validate(self) -> None:
        if (
            self.schema != MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SCHEMA
            or self.source_kind not in MASSIVE_ECONOMIC_EVENT_SOURCE_V4_KINDS
        ):
            raise MassiveEconomicEventSourceV4Error("economic source identity drifted")
        for value in (
        ...
        if self.parser_source_sha256 != MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SOURCE_SHA256:
            raise MassiveEconomicEventSourceV4Error("economic source parser drifted")
        if (
        ...
        for row in records:
            ...
            if isinstance(
                row,
                (MassiveSourcedCorporateActionV4, MassiveSourcedTerminalEventV4),
            ):
                keys.append((row.event.effective_at_ms, row.global_economic_sequence))
            else:
                keys.append(
                    (
                        row.cash_return.effective_at_ms,
        ...
        if (
            ...
            != MASSIVE_ECONOMIC_EVENT_SOURCE_V4_DATASETS[self.source_kind]
...
@dataclass(frozen=True, slots=True)
class MassiveEconomicEventAuthorityV4:
    identity_authority_receipt_sha256: str
    security_ids: tuple[str, ...]
    sources: tuple[MassiveEconomicEventSourceV4, ...]
    corporate_actions: tuple[MassiveSourcedCorporateActionV4, ...]
    terminal_events: tuple[MassiveSourcedTerminalEventV4, ...]
    cash_returns: tuple[MassiveSourcedCashReturnV4, ...]
    ...
    def validate(self) -> None:
        ...
        for row in security_events:
            ...
            key = sourced_economic_event_order_v4(row)
        ...
        for cash_row in self.cash_returns:
            ...
            key = sourced_economic_event_order_v4(cash_row)
            if key in order_keys:
                raise MassiveEconomicEventSourceV4Error(
        ...
        global_rows: tuple[MassiveSourcedEconomicEventV4, ...] = (
            *self.corporate_actions,
            *self.terminal_events,
            *self.cash_returns,
        )
        global_inventory = tuple(
            (
                *sourced_economic_event_order_v4(row),
                sourced_economic_event_identity_v4(row),
                row.receipt_sha256,
            )
            for row in sorted(global_rows, key=sourced_economic_event_order_v4)
        )
...
def _exact_keys(value: Mapping[str, object], expected: set[str], *, name: str) -> None:
    if set(value) != expected:
        raise MassiveEconomicEventSourceV4Error(f"{name} field inventory differs")


def _parse_corporate(record: Mapping[str, object]) -> MassiveSourcedCorporateActionV4:
    expected = {
        "event_id",
        "security_id",
        "kind",
    ...
    if source_row_receipt != economic_event_source_row_receipt_v4(
    ...
    try:
        ...
        if timestamp_contract != expected_effective_timestamp_contract_v4(
        ...
        sequence = _global_economic_sequence(
            "corporate sequence", record["global_economic_sequence"]
        )
        upstream = _digest(
            "corporate upstream source", record["upstream_source_receipt_sha256"]
        )
    except MassiveEconomicEventSourceV4Error:
        raise
    except (TypeError, ValueError) as exc:
        raise MassiveEconomicEventSourceV4Error(
            "corporate-action row is malformed"
        ) from exc
    provisional = MassiveSourcedCorporateActionV4(
        event=event,
        global_economic_sequence=sequence,
...
def _parse_terminal(record: Mapping[str, object]) -> MassiveSourcedTerminalEventV4:
    expected = {
        "event_id",
        "security_id",
        "kind",
    ...
    try:
        ...
        sequence = _global_economic_sequence(
            "terminal sequence", record["global_economic_sequence"]
        )
        upstream = _digest(
    ...
    provisional = MassiveSourcedTerminalEventV4(
        event=event,
        global_economic_sequence=sequence,
        effective_timestamp_contract=timestamp_contract,
        upstream_source_receipt_sha256=upstream,
...
def _parse_cash(record: Mapping[str, object]) -> MassiveSourcedCashReturnV4:
    expected = {
        "event_id",
        "effective_at_ms",
        "available_at_ms",
    ...
    try:
        ...
        sequence = _global_economic_sequence(
            "cash-return sequence", record["global_economic_sequence"]
        )
        timestamp_contract = _text(
        ...
        raise MassiveEconomicEventSourceV4Error("cash-return row is malformed") from exc
    provisional = MassiveSourcedCashReturnV4(
        event_id=event_id,
        cash_return=cash,
        global_economic_sequence=sequence,
        effective_timestamp_contract=timestamp_contract,
...
def parse_massive_economic_event_source_v4(
    *, root: str | Path, loaded_source: LoadedMassiveSourceObject
) -> MassiveEconomicEventSourceV4:
    ...
    if payload["schema"] != MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SCHEMA:
        raise MassiveEconomicEventSourceV4Error("economic event source schema drifted")
    source_kind = payload["source_kind"]
    if (
        not isinstance(source_kind, str)
        or source_kind not in MASSIVE_ECONOMIC_EVENT_SOURCE_V4_KINDS
    ):
        raise MassiveEconomicEventSourceV4Error("economic event source kind is invalid")
    ...
    if any(not isinstance(record, dict) for record in raw_records):
        raise MassiveEconomicEventSourceV4Error("economic source record is malformed")
    corporate = (
    ...
    provisional = MassiveEconomicEventSourceV4(
        ...
        parser_source_sha256=MASSIVE_ECONOMIC_EVENT_SOURCE_V4_SOURCE_SHA256,
...
def build_massive_economic_event_authority_v4(
    *,
    root: str | Path,
    ...
    parsed = tuple(
        sorted(
            ...
            key=lambda source: MASSIVE_ECONOMIC_EVENT_SOURCE_V4_KINDS.index(
                source.source_kind
            ),
        )
    )
    ...
    provisional = MassiveEconomicEventAuthorityV4(
...
__all__ = [
    ...
    "expected_effective_timestamp_contract_v4",
    "parse_massive_economic_event_source_v4",
    "sourced_economic_event_identity_v4",
