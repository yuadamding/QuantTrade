"""Governed two-H100 worker for the M03R-v13 predictive-only panel.
...
from rl_quant.training.top2000_m03r_v13_checkpoint import (
...
M03R_V13_STARTUP_SCHEMA = "rl-quant.top2000-dev.m03r-v13-two-h100-startup-v1"
M03R_V13_CAPACITY_TERMINAL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v13-two-h100-capacity-terminal-v1"
)
M03R_V13_FOLD_TERMINAL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v13-predictive-fold-terminal-v1"
)
M03R_V13_WORKER_TERMINAL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v13-predictive-worker-terminal-v1"
)
M03R_V13_WORKER_ERROR_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v13-predictive-worker-error-v1"
)
M03R_V13_PROGRESS_SCHEMA = "rl-quant.top2000-dev.m03r-v13-progress-v1"


class M03RV13PredictiveWorkflowError(RuntimeError):
    """The exact v13 package, distributed runtime, or output drifted."""
...
def _write_immutable_torch(path: Path, payload: dict[str, Any]) -> str:
    if path.exists() or path.is_symlink():
        raise M03RV13PredictiveWorkflowError("v13 artifact target already exists")
    path.parent.mkdir(mode=0o750, parents=True, exist_ok=True)
...
def _distributed_context() -> tuple[int, int, int, torch.device, bool]:
    try:
        ...
        raise M03RV13PredictiveWorkflowError(
            "v13 worker requires exact torchrun rank variables"
        ) from exc
    if (
        world_size != 2
        ...
        raise M03RV13PredictiveWorkflowError(
            "v13 requires one fresh non-resumed two-rank process group"
        )
    if not torch.cuda.is_available() or torch.cuda.device_count() != 2:
        raise M03RV13PredictiveWorkflowError(
            "v13 requires exactly two visible CUDA devices"
        )
    ...
    if (
        ...
        raise M03RV13PredictiveWorkflowError(
            "each v13 rank requires one NVIDIA H100 80GB HBM3"
        )
    owns = False
    ...
    if dist.get_world_size() != 2 or dist.get_rank() != rank:
        raise M03RV13PredictiveWorkflowError("v13 NCCL identity drifted")
...
def resolve_m03r_v13_completion_index(value: int | None) -> int:
    if value is None:
    ...
    if not isinstance(value, int) or isinstance(value, bool) or value not in range(2):
        raise M03RV13PredictiveWorkflowError(
            "v13 completion index must be 0 or 1"
        )
    return value
...
def _resolve_worker(
    ...
    if worker.setting_index != completion_index:
        raise M03RV13PredictiveWorkflowError("v13 completion mapping drifted")
    return worker
...
def _validate_runtime_package_members(
    ...
    for path, digest in expected.items():
        try:
            ...
            raise M03RV13PredictiveWorkflowError(
                "v13 runtime package member is unavailable"
            ) from exc
        if path.is_symlink() or not path.is_file() or status.st_size <= 0:
            raise M03RV13PredictiveWorkflowError(
                "v13 runtime package member is not a regular file"
            )
        if _file_sha256(path) != digest:
            raise M03RV13PredictiveWorkflowError(
                "v13 runtime package member hash drifted"
            )
...
def _qualification_artifact(
    result: M03RV13FoldQualificationResult,
) -> dict[str, Any]:
    return {
        "schema": "rl-quant.top2000-dev.m03r-v13-fold-qualification-artifact-v1",
...
def _validate_gathered_update(rows: list[Any], world_size: int) -> None:
    if len(rows) != world_size or any(not isinstance(row, dict) for row in rows):
        raise M03RV13PredictiveWorkflowError("v13 rank update evidence is malformed")
    shared = (
        "update_plan_sha256",
        "paired_input_sha256",
        "source_array_sha256",
    ...
    if (
        ...
        raise M03RV13PredictiveWorkflowError(
            "v13 distributed state, input, or origin coverage diverged"
        )
...
def _emit_progress(
    *,
    ...
    print(
        json.dumps(
            {
                "schema": M03R_V13_PROGRESS_SCHEMA,
...
def run_m03r_v13_predictive_worker(
    package_plan_path: str | Path,
    ...
    *,
    ...
    index = resolve_m03r_v13_completion_index(completion_index)
    ...
    try:
        ...
        if rank == 0:
            startup_sha = _write_immutable_json(
                ...
                {
                    "schema": M03R_V13_STARTUP_SCHEMA,
        ...
        if not isinstance(startup_sha, str):
            raise M03RV13PredictiveWorkflowError("v13 startup receipt missing")
        if startup_only:
            ...
            if rank == 0:
                unsigned = {
                    "schema": M03R_V13_CAPACITY_TERMINAL_SCHEMA,
                    "package_plan_sha256": package.package_plan_sha256,
                    "authorization_receipt_sha256": authorization.receipt_sha256,
                    "worker_plan_sha256": worker.receipt_sha256,
                    "setting_index": worker.setting_index,
        ...
        if cache.daily_ohlcv.shape[0] != TOP2000_M03R_V7_DEV_REQUIRED_STATE_ROWS:
            raise M03RV13PredictiveWorkflowError("v13 cache geometry drifted")
        ...
        if (
            ...
            raise M03RV13PredictiveWorkflowError("v13 risk identity drifted")
        ...
        for geometry in geometries:
            ...
            if model_state_sha256(training_policy) != worker.initial_parameter_state_sha256:
                raise M03RV13PredictiveWorkflowError(
                    "v13 fresh common initial parameter state drifted"
                )
            optimizer, partition = build_m03r_v13_optimizer(training_policy)
            ...
            if len(update_evidence) != geometry.optimizer_updates:
                raise M03RV13PredictiveWorkflowError(
                    "v13 fold update coverage drifted"
                )
            ...
            if (
                ...
                raise M03RV13PredictiveWorkflowError(
                    "v13 final rank model or optimizer states diverged"
                )
            source_root = _sha256(
            ...
            checkpoint_path = (
                ...
                / (
                    ...
                    f"{M03R_V13_SELECTED_HORIZON_SESSIONS:02d}-update-"
                    f"{geometry.optimizer_updates:04d}.pt"
                )
            )
            checkpoint_sha: str | None = None
            if rank == 0:
                checkpoint_sha = write_immutable_m03r_v13_alpha_checkpoint(
                    checkpoint_path,
                    training_policy,
                    fold_index=geometry.fold_index,
            ...
            if not isinstance(checkpoint_sha, str):
                raise M03RV13PredictiveWorkflowError(
                    "v13 checkpoint publication failed"
                )
            dist.barrier()
            del optimizer, partition, training_policy
            torch.cuda.empty_cache()

            risk_state = build_m03r_v13_qualification_risk_state(
                cache,
                geometry,
                risk_source,
                risk_binding,
            ...
            if len(set(risk_hashes)) != 1:
                raise M03RV13PredictiveWorkflowError(
                    "v13 qualified risk state diverged between ranks"
                )
            ...
            fold_qualification = run_m03r_v13_fold_qualification(
                cache,
                geometry,
                risk_source,
                risk_state,
            ...
            if len(set(trace_hashes)) != 1:
                raise M03RV13PredictiveWorkflowError(
                    "v13 qualification trace diverged between ranks"
                )
            artifact_path = (
                output
                / "fold-artifacts"
            ...
            if not isinstance(artifact_sha, str):
                raise M03RV13PredictiveWorkflowError(
                    "v13 qualification artifact publication failed"
                )
            if rank == 0:
                fold_traces.append(fold_qualification.trace)
                unsigned_fold = {
                    "schema": M03R_V13_FOLD_TERMINAL_SCHEMA,
                    "package_plan_sha256": package.package_plan_sha256,
                    "authorization_receipt_sha256": authorization.receipt_sha256,
                    "worker_plan_sha256": worker.receipt_sha256,
                    "setting_index": worker.setting_index,
        ...
        if rank == 0:
            ...
            bootstrap = build_m03r_v13_bootstrap_plan(
                tuple(row.origin_indices for row in traces)
            )
            predictive_qualification = qualify_m03r_v13_predictive_candidate(
                traces,
                bootstrap,
            )
            unsigned_terminal = {
                "schema": M03R_V13_WORKER_TERMINAL_SCHEMA,
                "package_plan_sha256": package.package_plan_sha256,
                "package_plan_file_sha256": expected_package_plan_file_sha256,
                "authorization_receipt_sha256": authorization.receipt_sha256,
                "worker_plan_sha256": worker.receipt_sha256,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        run_m03r_v13_predictive_worker(
            args.package_plan,
        ...
        if output_raw is not None and rank == "0":
            ...
            if output.is_dir() and not (output / "worker-error.json").exists():
                _write_immutable_json(
                    ...
                    {
                        "schema": M03R_V13_WORKER_ERROR_SCHEMA,
...
__all__ = [
    "M03R_V13_CAPACITY_TERMINAL_SCHEMA",
    "M03R_V13_FOLD_TERMINAL_SCHEMA",
    "M03R_V13_PROGRESS_SCHEMA",
    "M03R_V13_STARTUP_SCHEMA",
    "M03R_V13_WORKER_ERROR_SCHEMA",
    "M03R_V13_WORKER_TERMINAL_SCHEMA",
