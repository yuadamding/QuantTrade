"""Build and validate one immutable local M03R-v11 predictive package."""
...
M03R_V11_LOCAL_PACKAGE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v11-local-predictive-package-v1"
)
M03R_V11_SOURCE_MANIFEST_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v11-runtime-source-manifest-v1"
)
M03R_V11_EXECUTION_MANIFEST_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v11-execution-manifest-v1"
)
M03R_V11_TRANSFER_ROOT = "qt-m03r-v11-predictive-package-v1"
M03R_V11_RUNTIME_WORKER = "src/rl_quant/workflows/top2000_m03r_v11_predictive.py"
...
class M03RV11PackageBuildError(ValueError):
    """A v11 package member, identity, or no-clobber boundary drifted."""
...
def _regular(path: Path, name: str) -> Path:
    try:
        ...
        raise M03RV11PackageBuildError(f"{name} is unavailable") from exc
    if path.is_symlink() or not stat.S_ISREG(status.st_mode) or status.st_size <= 0:
        raise M03RV11PackageBuildError(f"{name} must be a nonempty regular file")
    return path
...
def _copy(source: Path, target: Path) -> str:
    ...
    if digest.hexdigest() != _file_sha256(source):
        raise M03RV11PackageBuildError("source changed during copy")
    return digest.hexdigest()
...
def _source_members(root: Path) -> tuple[Path, ...]:
    ...
    for path in sorted(candidates, key=lambda item: item.relative_to(root).as_posix()):
        ...
        if not resolved.is_relative_to(root):
            raise M03RV11PackageBuildError("source member leaves repository root")
        result.append(path)
    if len(result) != len(set(result)):
        raise M03RV11PackageBuildError("source inventory contains duplicates")
    return tuple(result)
...
def _inventory(root: Path) -> tuple[dict[str, Any], ...]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            raise M03RV11PackageBuildError("package inventory contains a symlink")
        if path.is_file():
            rows.append(
                {
    ...
    with tarfile.open(path, "r") as archive:
        for member in archive.getmembers():
            ...
            if (
                ...
                raise M03RV11PackageBuildError("archive contains an unsafe member")
            names.append(member.name)
    if len(names) != len(set(names)):
        raise M03RV11PackageBuildError("archive contains duplicate members")
    return tuple(names)


def _initial_policy() -> Top2000M03RV9PredictivePolicy:
    random.seed(17)
...
def build_m03r_v11_local_package(
    *,
    ...
    source_manifest_sha = _exclusive(
        ...
        _canonical(
            {
                "schema": M03R_V11_SOURCE_MANIFEST_SCHEMA,
                "protocol_sha256": M03R_V11_PROTOCOL_SHA256,
                "file_count": len(source_tuple),
                "files": source_tuple,
                "runtime_worker": M03R_V11_RUNTIME_WORKER,
    ...
    initial_state_sha, initial_state_file_sha = write_m03r_v11_initial_parameter_state(
    ...
    if not isinstance(projector, dict) or not isinstance(binding, dict):
        raise M03RV11PackageBuildError("projector manifest lacks typed payloads")
    ...
    artifacts = M03RV11PackageArtifacts(
        ...
        worker_source_sha256=_file_sha256(source / M03R_V11_RUNTIME_WORKER),
    ...
    plan_file_sha = write_m03r_v11_package_plan(plan_path, package)
    ...
    execution_sha = _exclusive(
        ...
        _canonical(
            {
                "schema": M03R_V11_EXECUTION_MANIFEST_SCHEMA,
                "protocol_sha256": M03R_V11_PROTOCOL_SHA256,
                "package_plan_sha256": package.package_plan_sha256,
                "package_plan_file_sha256": plan_file_sha,
                ...
                "runtime_entrypoint": M03R_V11_RUNTIME_ENTRYPOINT,
    ...
    unsigned = {
        "schema": M03R_V11_LOCAL_PACKAGE_SCHEMA,
...
def validate_m03r_v11_local_package(output_root: str | Path) -> dict[str, Any]:
    output = Path(output_root)
    receipt_path = _regular(output / "package-build-receipt.json", "package receipt")
    receipt = json.loads(receipt_path.read_bytes())
    if not isinstance(receipt, dict):
        raise M03RV11PackageBuildError("package receipt is not an object")
    unsigned = {key: value for key, value in receipt.items() if key != "receipt_sha256"}
    if (
        receipt.get("schema") != M03R_V11_LOCAL_PACKAGE_SCHEMA
        ...
        raise M03RV11PackageBuildError("package receipt semantics drifted")
    package_root = output / "package"
    if tuple(receipt.get("file_inventory", ())) != _inventory(package_root):
        raise M03RV11PackageBuildError("package inventory drifted")
    plan = load_m03r_v11_package_plan(
    ...
    if (
        ...
        raise M03RV11PackageBuildError("package authorization binding drifted")
    _safe_tar(package_root / "source.tar")
    return receipt


def build_m03r_v11_transfer_archive(
    output_root: str | Path,
    archive_path: str | Path,
) -> dict[str, Any]:
    receipt = validate_m03r_v11_local_package(output_root)
    root = Path(output_root)
    target = Path(archive_path)
    if target.exists() or target.is_symlink():
        raise M03RV11PackageBuildError("transfer archive already exists")
    ...
    with tarfile.open(target, "x", format=tarfile.PAX_FORMAT) as archive:
        for source in sorted(
            ...
            info = tarfile.TarInfo(f"{M03R_V11_TRANSFER_ROOT}/{relative}")
...
def validate_m03r_v11_transfer_archive(
    archive_path: str | Path,
    *,
    ...
    if _file_sha256(path) != expected_archive_sha256:
        raise M03RV11PackageBuildError("transfer archive hash drifted")
    _safe_tar(path, required_root=M03R_V11_TRANSFER_ROOT)
    receipt_name = f"{M03R_V11_TRANSFER_ROOT}/package-build-receipt.json"
    ...
    with tarfile.open(path, "r") as archive:
        for member in archive.getmembers():
            ...
            if stream is None:
                raise M03RV11PackageBuildError("transfer member is unreadable")
            digest = hashlib.sha256()
    ...
    if content is None:
        raise M03RV11PackageBuildError("transfer receipt is absent")
    if hashlib.sha256(content).hexdigest() != expected_package_receipt_file_sha256:
        raise M03RV11PackageBuildError("transfer package receipt hash drifted")
    receipt = json.loads(content)
    if not isinstance(receipt, dict):
        raise M03RV11PackageBuildError("transfer package receipt is malformed")
    unsigned = {key: value for key, value in receipt.items() if key != "receipt_sha256"}
    if receipt.get("schema") != M03R_V11_LOCAL_PACKAGE_SCHEMA or receipt.get(
        "receipt_sha256"
    ) != _sha256(unsigned):
        raise M03RV11PackageBuildError("transfer package receipt semantics drifted")
    expected = {
        receipt_name: (len(content), expected_package_receipt_file_sha256),
        **{
            f"{M03R_V11_TRANSFER_ROOT}/package/{row['path']}": (
                row["size"],
                row["sha256"],
            )
    ...
    if observed != expected:
        raise M03RV11PackageBuildError("transfer archive inventory drifted")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "build":
        ...
        validate_m03r_v11_local_package(args.output_root)
    else:
        build_m03r_v11_transfer_archive(args.output_root, args.archive_path)
    return 0
...
__all__ = [
    "M03R_V11_EXECUTION_MANIFEST_SCHEMA",
    "M03R_V11_LOCAL_PACKAGE_SCHEMA",
    "M03R_V11_RUNTIME_WORKER",
    "M03R_V11_SOURCE_MANIFEST_SCHEMA",
    "M03R_V11_TRANSFER_ROOT",
    "PINNED_QUANTTRADE_IMAGE",
