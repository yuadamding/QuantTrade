"""Pure-file preparation for the M03R-v15 Seadragon lifecycle.
...
class M03RV15SeadragonPrepareError(RuntimeError):
...
def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise M03RV15SeadragonPrepareError(f"{label} must be an object")
...
def _read_json(path: str | Path, expected_sha256: str, label: str) -> Mapping[str, Any]:
    ...
    if _file_sha256(source) != expected_sha256:
        raise M03RV15SeadragonPrepareError(f"{label} file hash drifted")
    try:
        return _mapping(json.loads(source.read_bytes()), label)
    except json.JSONDecodeError as exc:
        raise M03RV15SeadragonPrepareError(f"{label} is invalid JSON") from exc
...
def _load_live(path: str, expected_sha256: str) -> M03RV15LiveEvidence:
    ...
    try:
        ...
        return M03RV15LiveEvidence(**payload)
    except (KeyError, TypeError, ValueError) as exc:
        raise M03RV15SeadragonPrepareError("live evidence is invalid") from exc
...
def _load_template(path: str, expected_sha256: str) -> M03RV7KubernetesTemplateConfig:
    try:
        ...
        raise M03RV15SeadragonPrepareError("Kubernetes template is invalid") from exc
...
def _load_capacity(
    ...
    try:
        return M03RV15TwoH100CapacityQualification(
            **dict(_read_json(path, expected_sha256, "capacity qualification"))
        )
    except (TypeError, ValueError) as exc:
        raise M03RV15SeadragonPrepareError("capacity qualification is invalid") from exc


def _load_rendered(path: str, expected_sha256: str) -> M03RV15RenderedJob:
    try:
        return M03RV15RenderedJob(
            **dict(_read_json(path, expected_sha256, "rendered Job"))
        )
    except (TypeError, ValueError) as exc:
        raise M03RV15SeadragonPrepareError("rendered Job is invalid") from exc
...
def _render(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        raise M03RV15SeadragonPrepareError("now UTC is invalid") from exc
    if now.tzinfo is None:
        raise M03RV15SeadragonPrepareError("now UTC must be timezone-aware")
    if args.mode == "static":
        if args.capacity_qualification is not None:
            raise M03RV15SeadragonPrepareError(
                "static render cannot consume future qualification"
            )
        rendered = render_m03r_v15_suspended_static_job(
            package=package,
        ...
        rendered = render_m03r_v15_suspended_capacity_job(
            package=package,
            authorization=authorization,
            package_plan_file_sha256=args.package_plan_file_sha256,
            authorization_file_sha256=args.execution_authorization_file_sha256,
        ...
        rendered = render_m03r_v15_suspended_predictive_job(
            package=package,
            authorization=authorization,
            package_plan_file_sha256=args.package_plan_file_sha256,
            authorization_file_sha256=args.execution_authorization_file_sha256,
    ...
    if Path(args.manifest_output).exists() or Path(args.rendered_output).exists():
        raise M03RV15SeadragonPrepareError("render outputs must both be absent")
...
def _build_create(args: argparse.Namespace) -> None:
    ...
    if rendered.manifest != dict(manifest):
        raise M03RV15SeadragonPrepareError("rendered and manifest bytes disagree")
    config = operator.M03RV15CreateOperatorConfig(
        mode=rendered.mode,
...
def _build_capacity_attach(args: argparse.Namespace) -> None:
    ...
    config = lifecycle.M03RV15CapacityAttachConfig(
...
def _build_static_attach(args: argparse.Namespace) -> None:
    ...
    rendered = _load_rendered(args.rendered, args.rendered_file_sha256)
    binding = _binding(args)
    config = static_gate.M03RV15StaticAttachConfig(
        job_name=binding.job_name,
        run_id=binding.run_id,
        job_uid=binding.job_uid,
        rendered_path=str(Path(args.rendered).resolve()),
    ...
    if rendered.mode != "static":
        raise M03RV15SeadragonPrepareError("static attach requires static rendered Job")
    _write(args.output, asdict(config))
...
__all__ = ["M03RV15SeadragonPrepareError", "main"]
