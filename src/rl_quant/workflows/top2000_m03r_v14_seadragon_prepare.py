"""Pure-file preparation for the M03R-v14 Seadragon lifecycle.
...
from rl_quant.training import top2000_m03r_v14_seadragon_operator as operator
from rl_quant.training import top2000_m03r_v14_static_gate as static_gate
...
class M03RV14SeadragonPrepareError(RuntimeError):
...
def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise M03RV14SeadragonPrepareError(f"{label} must be an object")
...
def _read_json(path: str | Path, expected_sha256: str, label: str) -> Mapping[str, Any]:
    ...
    if _file_sha256(source) != expected_sha256:
        raise M03RV14SeadragonPrepareError(f"{label} file hash drifted")
    try:
        return _mapping(json.loads(source.read_bytes()), label)
    except json.JSONDecodeError as exc:
        raise M03RV14SeadragonPrepareError(f"{label} is invalid JSON") from exc
...
def _load_live(path: str, expected_sha256: str) -> M03RV14LiveEvidence:
    ...
    try:
        ...
        return M03RV14LiveEvidence(**payload)
    except (KeyError, TypeError, ValueError) as exc:
        raise M03RV14SeadragonPrepareError("live evidence is invalid") from exc
...
def _load_template(path: str, expected_sha256: str) -> M03RV7KubernetesTemplateConfig:
    try:
        ...
        raise M03RV14SeadragonPrepareError("Kubernetes template is invalid") from exc
...
def _load_capacity(
    ...
    try:
        return M03RV14TwoH100CapacityQualification(
            **dict(_read_json(path, expected_sha256, "capacity qualification"))
        )
    except (TypeError, ValueError) as exc:
        raise M03RV14SeadragonPrepareError("capacity qualification is invalid") from exc


def _load_rendered(path: str, expected_sha256: str) -> M03RV14RenderedJob:
    try:
        return M03RV14RenderedJob(
            **dict(_read_json(path, expected_sha256, "rendered Job"))
        )
    except (TypeError, ValueError) as exc:
        raise M03RV14SeadragonPrepareError("rendered Job is invalid") from exc
...
def _render(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        raise M03RV14SeadragonPrepareError("now UTC is invalid") from exc
    if now.tzinfo is None:
        raise M03RV14SeadragonPrepareError("now UTC must be timezone-aware")
    if args.mode == "static":
        if args.capacity_qualification is not None:
            raise M03RV14SeadragonPrepareError(
                "static render cannot consume future qualification"
            )
        rendered = render_m03r_v14_suspended_static_job(
            package=package,
        ...
        rendered = render_m03r_v14_suspended_capacity_job(
            package=package,
            authorization=authorization,
            package_plan_file_sha256=args.package_plan_file_sha256,
            authorization_file_sha256=args.execution_authorization_file_sha256,
        ...
        rendered = render_m03r_v14_suspended_predictive_job(
            package=package,
            authorization=authorization,
            package_plan_file_sha256=args.package_plan_file_sha256,
            authorization_file_sha256=args.execution_authorization_file_sha256,
    ...
    if Path(args.manifest_output).exists() or Path(args.rendered_output).exists():
        raise M03RV14SeadragonPrepareError("render outputs must both be absent")
...
def _build_create(args: argparse.Namespace) -> None:
    ...
    if rendered.manifest != dict(manifest):
        raise M03RV14SeadragonPrepareError("rendered and manifest bytes disagree")
    config = operator.M03RV14CreateOperatorConfig(
        mode=rendered.mode,
...
def _build_capacity_attach(args: argparse.Namespace) -> None:
    ...
    config = lifecycle.M03RV14CapacityAttachConfig(
...
def _build_static_attach(args: argparse.Namespace) -> None:
    ...
    rendered = _load_rendered(args.rendered, args.rendered_file_sha256)
    binding = _binding(args)
    config = static_gate.M03RV14StaticAttachConfig(
        job_name=binding.job_name,
        run_id=binding.run_id,
        job_uid=binding.job_uid,
        rendered_path=str(Path(args.rendered).resolve()),
    ...
    if rendered.mode != "static":
        raise M03RV14SeadragonPrepareError("static attach requires static rendered Job")
    _write(args.output, asdict(config))
...
def _build_predictive_attach(args: argparse.Namespace) -> None:
    ...
    binding = _binding(args)
    expected = tuple(
        lifecycle.M03RV14ExpectedCompletion(
    ...
    config = lifecycle.M03RV14AttachSupervisorConfig(
        job_name=binding.job_name,
        run_id=binding.run_id,
        job_uid=binding.job_uid,
        binding_path=str(Path(args.binding).resolve()),
...
__all__ = ["M03RV14SeadragonPrepareError", "main"]
