"""Governed two-H100 worker for the M03R-v15 predictive-only panel.
...
from rl_quant.training.top2000_m03r_v15_initial_state import (
    load_m03r_v15_initial_parameter_state,
)
...
from rl_quant.training.top2000_m03r_v15_qualification_runtime import (
    M03RV15FoldQualificationResult,
    build_m03r_v15_qualification_risk_state,
    run_m03r_v15_fold_qualification,
)
from rl_quant.training.top2000_m03r_v15_selection import (
    build_m03r_v15_bootstrap_plan,
    qualify_m03r_v15_predictive_candidate,
)
from rl_quant.training.top2000_m03r_v15_training_runtime import (
    move_and_bind_m03r_v15_sequence,
    run_m03r_v15_pretraining_fold_update,
)
...
M03R_V15_STARTUP_SCHEMA = "rl-quant.top2000-dev.m03r-v15-two-h100-startup-v1"
M03R_V15_CAPACITY_TERMINAL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-two-h100-capacity-terminal-v1"
)
M03R_V15_FOLD_TERMINAL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-predictive-fold-terminal-v1"
)
M03R_V15_WORKER_TERMINAL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-predictive-worker-terminal-v1"
)
M03R_V15_WORKER_ERROR_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-predictive-worker-error-v1"
)
M03R_V15_PROGRESS_SCHEMA = "rl-quant.top2000-dev.m03r-v15-progress-v1"


class M03RV15PredictiveWorkflowError(RuntimeError):
    """The exact v15 package, distributed runtime, or output drifted."""
...
def _write_immutable_torch(path: Path, payload: dict[str, Any]) -> str:
    if path.exists() or path.is_symlink():
        raise M03RV15PredictiveWorkflowError("v15 artifact target already exists")
    path.parent.mkdir(mode=0o750, parents=True, exist_ok=True)
...
def _distributed_context() -> tuple[int, int, int, torch.device, bool]:
    try:
        ...
        raise M03RV15PredictiveWorkflowError(
            "v15 worker requires exact torchrun rank variables"
        ) from exc
    if (
        world_size != 2
        ...
        raise M03RV15PredictiveWorkflowError(
            "v15 requires one fresh non-resumed two-rank process group"
        )
    if not torch.cuda.is_available() or torch.cuda.device_count() != 2:
        raise M03RV15PredictiveWorkflowError(
            "v15 requires exactly two visible CUDA devices"
        )
    ...
    if (
        ...
        raise M03RV15PredictiveWorkflowError(
            "each v15 rank requires one NVIDIA H100 80GB HBM3"
        )
    owns = False
    ...
    if dist.get_world_size() != 2 or dist.get_rank() != rank:
        raise M03RV15PredictiveWorkflowError("v15 NCCL identity drifted")
...
def _cpu_model_state(policy: torch.nn.Module) -> dict[str, torch.Tensor]:
    return {
...
def resolve_m03r_v15_completion_index(value: int | None) -> int:
    if value is None:
    ...
    if not isinstance(value, int) or isinstance(value, bool) or value not in range(2):
        raise M03RV15PredictiveWorkflowError(
            "v15 completion index must be 0 or 1"
        )
    return value


def _resolve_worker(
    package: M03RV15PackagePlan,
    completion_index: int,
) -> M03RV15PredictiveWorkerPlan:
    worker = package.panel.workers[completion_index]
    worker.validate()
    if worker.setting_index != completion_index:
        raise M03RV15PredictiveWorkflowError("v15 completion mapping drifted")
    return worker
...
def _validate_runtime_package_members(
    ...
    expected = {
        ...
        package_root
        / "source"
        / "src/rl_quant/workflows/top2000_m03r_v15_predictive.py": (
    ...
    for path, digest in expected.items():
        try:
            ...
            raise M03RV15PredictiveWorkflowError(
                "v15 runtime package member is unavailable"
            ) from exc
        if path.is_symlink() or not path.is_file() or status.st_size <= 0:
            raise M03RV15PredictiveWorkflowError(
                "v15 runtime package member is not a regular file"
            )
        if _file_sha256(path) != digest:
            raise M03RV15PredictiveWorkflowError(
                "v15 runtime package member hash drifted"
            )
...
def _qualification_artifact(
    result: M03RV15FoldQualificationResult,
) -> dict[str, Any]:
    return {
        "schema": "rl-quant.top2000-dev.m03r-v15-fold-qualification-artifact-v1",
...
def _rank_update_row(result: Any) -> dict[str, Any]:
    return {
        ...
        "mean_head_gradient_norm_before_clip": (
            result.step.mean_head_gradient_norm_before_clip
        ),
        "scale_head_gradient_norm_before_clip": (
            result.step.scale_head_gradient_norm_before_clip
        ),
        "learning_rate_multiplier": result.step.learning_rate_multiplier,
        "encoder_learning_rate": result.step.encoder_learning_rate,
        "mean_head_learning_rate": result.step.mean_head_learning_rate,
        "scale_head_learning_rate": result.step.scale_head_learning_rate,
        "encoder_gradient_clipped": result.step.encoder_gradient_clipped,
        "mean_head_gradient_clipped": result.step.mean_head_gradient_clipped,
        "scale_head_gradient_clipped": result.step.scale_head_gradient_clipped,
    }
...
def _validate_gathered_update(rows: list[Any], world_size: int) -> None:
    if len(rows) != world_size or any(not isinstance(row, dict) for row in rows):
        raise M03RV15PredictiveWorkflowError("v15 rank update evidence is malformed")
    shared = (
        "update_plan_sha256",
        "paired_input_sha256",
        "source_array_sha256",
        ...
        "mean_head_learning_rate",
        "scale_head_learning_rate",
    )
    if (
        ...
        raise M03RV15PredictiveWorkflowError(
            "v15 distributed state, input, or origin coverage diverged"
        )


def _emit_progress(
    *,
    worker: M03RV15PredictiveWorkerPlan,
    fold_index: int,
    ...
    print(
        json.dumps(
            {
                "schema": M03R_V15_PROGRESS_SCHEMA,
                ...
                "mean_head_gradient_norm_before_clip": row[
                    "mean_head_gradient_norm_before_clip"
                ],
                "scale_head_gradient_norm_before_clip": row[
                    "scale_head_gradient_norm_before_clip"
                ],
                "learning_rate_multiplier": row["learning_rate_multiplier"],
                "encoder_learning_rate": row["encoder_learning_rate"],
                "mean_head_learning_rate": row["mean_head_learning_rate"],
                "scale_head_learning_rate": row["scale_head_learning_rate"],
                "encoder_gradient_clipped": row["encoder_gradient_clipped"],
                "mean_head_gradient_clipped": row["mean_head_gradient_clipped"],
                "scale_head_gradient_clipped": row["scale_head_gradient_clipped"],
...
def run_m03r_v15_predictive_worker(
    package_plan_path: str | Path,
    ...
    *,
    ...
    structural_preflight = load_m03r_v15_structural_preflight(
    ...
    if authorization.package_plan_file_sha256 != expected_package_plan_file_sha256:
        raise M03RV15PredictiveWorkflowError(
            "v15 authorization and package-plan file disagree"
        )
    index = resolve_m03r_v15_completion_index(completion_index)
    ...
    try:
        ...
        if rank == 0:
            startup_sha = _write_immutable_json(
                ...
                {
                    "schema": M03R_V15_STARTUP_SCHEMA,
        ...
        if not isinstance(startup_sha, str):
            raise M03RV15PredictiveWorkflowError("v15 startup receipt missing")
        if startup_only:
            ...
            risk_source, written_risk = load_top2000_m03r_v9_risk_source(
                ...
                expected_manifest_file_sha256=(
                    worker.risk_source_manifest_file_sha256
                ),
            )
            ...
            if (
                ...
                raise M03RV15PredictiveWorkflowError(
                    "v15 capacity risk identity drifted"
                )
            structural_preflight.validate_for_package(
                ...
                projector_binding_sha256=risk_binding.binding_sha256,
            )
            ...
            capacity_optimizer, capacity_partition = build_m03r_v15_optimizer(
                capacity_policy
            )
            first_geometry = render_m03r_v15_fold_geometries(
                TOP2000_M03R_V7_DEV_REQUIRED_STATE_ROWS
            )[0]
            capacity_update = run_m03r_v15_pretraining_fold_update(
                cache,
                worker,
                package.schedule,
                first_geometry,
            ...
            update_rows = _gather(_rank_update_row(capacity_update), world_size)
            _validate_gathered_update(update_rows, world_size)
            post_update_model_sha = model_state_sha256(capacity_policy)
            post_update_optimizer_sha = optimizer_state_sha256(capacity_optimizer)
            if (
                len(set(_gather(post_update_model_sha, world_size))) != 1
                or len(set(_gather(post_update_optimizer_sha, world_size))) != 1
            ):
                raise M03RV15PredictiveWorkflowError(
                    "v15 disposable capacity update diverged between ranks"
                )
            risk_state = build_m03r_v15_qualification_risk_state(
                cache,
                first_geometry,
                risk_source,
                risk_binding,
            ...
            qualification_built = (
                build_top2000_hold30_development_sequence_from_loaded_cache(
                    cache,
                    state_start_index=(
                        first_geometry.qualification_episode_state_start
                    ),
                    state_stop_index_exclusive=(
                        first_geometry.qualification_episode_state_stop_exclusive
                    ),
                    max_state_rows=(
                        first_geometry.qualification_episode_state_stop_exclusive
                        - first_geometry.qualification_episode_state_start
            ...
            qualification_sequence = move_and_bind_m03r_v15_sequence(
                qualification_built.sequence,
                device=device,
                asset_axis_sha256=cache.action_hash,
            )
            local_origin = (
                first_geometry.qualification_origin_start_inclusive
                - first_geometry.qualification_episode_state_start
            )
            fill_index = local_origin + 1
            benchmark = qualification_sequence.benchmark_weights[fill_index]
            projection = project_m03r_v9_active_book(
                benchmark,
                benchmark,
                qualification_sequence.fill_membership[fill_index]
                & qualification_sequence.fill_availability[fill_index],
                qualification_sequence.risk_asset_caps[fill_index],
                qualification_sequence.risk_gross_max[fill_index],
                risk_state,
                origin_state_index=(
                    first_geometry.qualification_origin_start_inclusive
                ),
            ...
            if not torch.allclose(
                projection.projected_weights,
                benchmark,
                rtol=0.0,
                atol=2.0e-6,
            ):
                raise M03RV15PredictiveWorkflowError(
                    "v15 disposable qualification projection drifted"
                )
            peak_rows = _gather(
                {
                    "rank": rank,
            ...
            if rank == 0:
                unsigned = {
                    "schema": M03R_V15_CAPACITY_TERMINAL_SCHEMA,
                    "package_plan_sha256": package.package_plan_sha256,
                    "authorization_receipt_sha256": authorization.receipt_sha256,
                    "worker_plan_sha256": worker.receipt_sha256,
                    "setting_index": worker.setting_index,
                    ...
                    "disposable_update_step_receipt_sha256": (
                        capacity_update.step.receipt_sha256
                    ),
                    "disposable_rank_states_equal_after_update": True,
                    "qualification_risk_state_sha256": risk_state.state_sha256,
                    "disposable_qualification_projection_performed": True,
                    "peak_cuda_memory_by_rank": peak_rows,
        ...
        if cache.daily_ohlcv.shape[0] != TOP2000_M03R_V7_DEV_REQUIRED_STATE_ROWS:
            raise M03RV15PredictiveWorkflowError("v15 cache geometry drifted")
        risk_source, written_risk = load_top2000_m03r_v9_risk_source(
            Path(worker.risk_source_manifest_path),
        ...
        if (
            ...
            raise M03RV15PredictiveWorkflowError("v15 risk identity drifted")
        structural_preflight.validate_for_package(
            cache_sha256=package.artifacts.cache_artifact_sha256,
            cache_manifest_sha256=package.artifacts.cache_manifest_sha256,
            asset_axis_sha256=cache.action_hash,
        ...
        geometries = render_m03r_v15_fold_geometries(
            TOP2000_M03R_V7_DEV_REQUIRED_STATE_ROWS
        )
        fold_traces = []
        ...
        for geometry in geometries:
            ...
            if model_state_sha256(training_policy) != worker.initial_parameter_state_sha256:
                raise M03RV15PredictiveWorkflowError(
                    "v15 fresh common initial parameter state drifted"
                )
            optimizer, partition = build_m03r_v15_optimizer(training_policy)
            update_evidence: list[list[Any]] = []
            validation_receipts: list[M03RV15InnerValidationReceipt] = []
            ...
            for completed in range(geometry.optimizer_updates):
                result = run_m03r_v15_pretraining_fold_update(
                    cache,
                    worker,
                    package.schedule,
                    geometry,
                ...
                if (completed + 1) % geometry.training_block_count == 0:
                    epoch_index = completed // geometry.training_block_count
                    validation_batch, validation = run_m03r_v15_inner_validation(
                        cache,
                        geometry,
                        risk_source,
                        training_policy,
                    ...
                    gathered_validation = _gather(asdict(validation), world_size)
                    if len({_sha256(row) for row in gathered_validation}) != 1:
                        raise M03RV15PredictiveWorkflowError(
                            "v15 inner validation diverged between ranks"
                        )
                    validation_receipts.append(validation)
                    if selected_key is None or validation.selection_key > selected_key:
                        selected_key = validation.selection_key
                        selected_state = _cpu_model_state(training_policy)
                    del validation_batch
            if len(update_evidence) != geometry.optimizer_updates:
                raise M03RV15PredictiveWorkflowError(
                    "v15 fold update coverage drifted"
                )
            selection = select_m03r_v15_checkpoint(tuple(validation_receipts))
            if (
                selected_state is None
                or selected_key is None
                or selection.selected_model_state_sha256
                != state_dict_sha256(selected_state)
            ):
                raise M03RV15PredictiveWorkflowError(
                    "v15 selected validation model state was not retained"
                )
            training_policy.load_state_dict(selected_state, strict=True)
            selected_model_hashes = _gather(
            ...
            if (
                len(set(selected_model_hashes)) != 1
                or selected_model_hashes[0] != selection.selected_model_state_sha256
                or len(set(final_optimizer_hashes)) != 1
            ):
                raise M03RV15PredictiveWorkflowError(
                    "v15 selected rank model or terminal optimizer states diverged"
                )
            source_root = _sha256(
            ...
            checkpoint_path = (
                ...
                / (
                    ...
                    f"{M03R_V15_SELECTED_HORIZON_SESSIONS:02d}-selected-epoch-"
                    f"{selection.selected_epoch_index + 1:02d}.pt"
                )
            )
            checkpoint_sha: str | None = None
            if rank == 0:
                checkpoint_sha = write_immutable_m03r_v15_alpha_checkpoint(
                    checkpoint_path,
                    training_policy,
                    fold_index=geometry.fold_index,
                    completed_updates=geometry.optimizer_updates,
                    selected_epoch_index=selection.selected_epoch_index,
            ...
            if not isinstance(checkpoint_sha, str):
                raise M03RV15PredictiveWorkflowError(
                    "v15 checkpoint publication failed"
                )
            dist.barrier()
            ...
            if len(set(risk_hashes)) != 1:
                raise M03RV15PredictiveWorkflowError(
                    "v15 qualified risk state diverged between ranks"
                )
            loaded_policy = _new_policy(worker.setting_index, device)
            loaded = load_m03r_v15_alpha_checkpoint_for_evaluation(
                checkpoint_path,
                ...
                expected_selected_epoch_index=selection.selected_epoch_index,
                ...
                expected_checkpoint_selection_receipt_sha256=(
                    selection.receipt_sha256
                ),
                policy=loaded_policy,
            )
            fold_qualification = run_m03r_v15_fold_qualification(
                cache,
                geometry,
                risk_source,
                risk_state,
            ...
            if len(set(trace_hashes)) != 1:
                raise M03RV15PredictiveWorkflowError(
                    "v15 qualification trace diverged between ranks"
                )
            artifact_path = (
                output
                / "fold-artifacts"
            ...
            if not isinstance(artifact_sha, str):
                raise M03RV15PredictiveWorkflowError(
                    "v15 qualification artifact publication failed"
                )
            if rank == 0:
                fold_traces.append(fold_qualification.trace)
                unsigned_fold = {
                    "schema": M03R_V15_FOLD_TERMINAL_SCHEMA,
                    "package_plan_sha256": package.package_plan_sha256,
                    "authorization_receipt_sha256": authorization.receipt_sha256,
                    "worker_plan_sha256": worker.receipt_sha256,
                    "setting_index": worker.setting_index,
                    ...
                    "model_state_sha256": selected_model_hashes[0],
                    "optimizer_state_sha256": final_optimizer_hashes[0],
                    "selected_epoch_index": selection.selected_epoch_index,
                    "checkpoint_selection": asdict(selection),
                    "checkpoint_selection_sha256": selection.receipt_sha256,
        ...
        if rank == 0:
            ...
            bootstrap = build_m03r_v15_bootstrap_plan(
                tuple(row.origin_indices for row in traces)
            )
            predictive_qualification = qualify_m03r_v15_predictive_candidate(
                traces,
                bootstrap,
            )
            unsigned_terminal = {
                "schema": M03R_V15_WORKER_TERMINAL_SCHEMA,
                "package_plan_sha256": package.package_plan_sha256,
                "package_plan_file_sha256": expected_package_plan_file_sha256,
                "authorization_receipt_sha256": authorization.receipt_sha256,
                "worker_plan_sha256": worker.receipt_sha256,
        ...
        if rank == 0 and output.is_dir() and not (output / "worker-error.json").exists():
            _write_immutable_json(
                ...
                {
                    "schema": M03R_V15_WORKER_ERROR_SCHEMA,
                    "package_plan_sha256": package.package_plan_sha256,
                    "authorization_receipt_sha256": authorization.receipt_sha256,
                    "worker_plan_sha256": worker.receipt_sha256,
                    "setting_index": worker.setting_index,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        run_m03r_v15_predictive_worker(
            args.package_plan,
...
__all__ = [
    "M03R_V15_CAPACITY_TERMINAL_SCHEMA",
    "M03R_V15_FOLD_TERMINAL_SCHEMA",
    "M03R_V15_PROGRESS_SCHEMA",
    "M03R_V15_STARTUP_SCHEMA",
    "M03R_V15_WORKER_ERROR_SCHEMA",
    "M03R_V15_WORKER_TERMINAL_SCHEMA",
    "M03RV15PredictiveWorkflowError",
    "main",
    "resolve_m03r_v15_completion_index",
    "run_m03r_v15_predictive_worker",
]
