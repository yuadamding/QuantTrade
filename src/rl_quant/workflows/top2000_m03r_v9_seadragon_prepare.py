"""Pure-file preparation for the M03R-v9 Seadragon lifecycle.
...
class M03RV9SeadragonPrepareError(RuntimeError):
...
def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise M03RV9SeadragonPrepareError(f"{label} must be an object")
...
def _read_json(path: str | Path, expected_sha256: str, label: str) -> Mapping[str, Any]:
    ...
    if _file_sha256(source) != expected_sha256:
        raise M03RV9SeadragonPrepareError(f"{label} file hash drifted")
    try:
        return _mapping(json.loads(source.read_bytes()), label)
    except json.JSONDecodeError as exc:
        raise M03RV9SeadragonPrepareError(f"{label} is invalid JSON") from exc
...
def _load_live(path: str, expected_sha256: str) -> M03RV9LiveEvidence:
    ...
    try:
        ...
        return M03RV9LiveEvidence(**payload)
    except (KeyError, TypeError, ValueError) as exc:
        raise M03RV9SeadragonPrepareError("live evidence is invalid") from exc
...
def _load_template(path: str, expected_sha256: str) -> M03RV7KubernetesTemplateConfig:
    try:
        ...
        raise M03RV9SeadragonPrepareError("Kubernetes template is invalid") from exc
...
def _load_capacity(
    ...
    try:
        ...
        raise M03RV9SeadragonPrepareError("capacity qualification is invalid") from exc


def _load_rendered(path: str, expected_sha256: str) -> M03RV9RenderedJob:
    try:
        return M03RV9RenderedJob(
            **dict(_read_json(path, expected_sha256, "rendered Job"))
        )
    except (TypeError, ValueError) as exc:
        raise M03RV9SeadragonPrepareError("rendered Job is invalid") from exc
...
def _render(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        raise M03RV9SeadragonPrepareError("now UTC is invalid") from exc
    if now.tzinfo is None:
        raise M03RV9SeadragonPrepareError("now UTC must be timezone-aware")
    if args.mode == "capacity":
        if args.capacity_qualification is not None:
            raise M03RV9SeadragonPrepareError(
                "capacity render cannot consume future qualification"
            )
        rendered = render_m03r_v9_suspended_capacity_job(
            package=package, live=live, template=template, now_utc=now
        )
    else:
        if (
        ...
        rendered = render_m03r_v9_suspended_predictive_job(
            package=package,
    ...
    if Path(args.manifest_output).exists() or Path(args.rendered_output).exists():
        raise M03RV9SeadragonPrepareError("render outputs must both be absent")
...
def _build_create(args: argparse.Namespace) -> None:
    ...
    if rendered.manifest != dict(manifest):
        raise M03RV9SeadragonPrepareError("rendered and manifest bytes disagree")
...
def _parser() -> argparse.ArgumentParser:
    ...
    render.add_argument("--mode", choices=("capacity", "predictive"), required=True)
    render.add_argument("--package-plan", required=True)
    render.add_argument("--package-plan-sha256", required=True)
    ...
    for name, handler in (
        ...
        command.add_argument("--package-plan-sha256", required=True)
...
__all__ = ["M03RV9SeadragonPrepareError", "main"]
