"""Governed two-H100 worker for the corrected M03R-v12 predictive panel.

The entrypoint performs exactly 64 predictive updates for one of three paired
settings, writes the horizon-3 checkpoint before opening the qualification
...
M03R_V12_STARTUP_SCHEMA = "rl-quant.top2000-dev.m03r-v12-two-h100-startup-v1"
M03R_V12_CAPACITY_TERMINAL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v12-two-h100-capacity-terminal-v1"
)
M03R_V12_FOLD_TERMINAL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v12-predictive-fold-terminal-v1"
)
M03R_V12_WORKER_TERMINAL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v12-predictive-worker-terminal-v1"
)
M03R_V12_WORKER_ERROR_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v12-predictive-worker-error-v1"
)


class M03RV12PredictiveWorkflowError(RuntimeError):
...
def _write_immutable_torch(path: Path, payload: dict[str, Any]) -> str:
    if path.exists() or path.is_symlink():
        raise M03RV12PredictiveWorkflowError("v12 artifact target already exists")
    path.parent.mkdir(mode=0o750, parents=True, exist_ok=True)
...
def _distributed_context() -> tuple[int, int, int, torch.device, bool]:
    try:
        ...
        raise M03RV12PredictiveWorkflowError(
            "v12 worker requires exact torchrun rank variables"
        ) from exc
    if (
        world_size != 2
        ...
        raise M03RV12PredictiveWorkflowError(
            "v12 requires one fresh non-resumed two-rank process group"
        )
    if not torch.cuda.is_available() or torch.cuda.device_count() != 2:
        raise M03RV12PredictiveWorkflowError(
            "v12 requires exactly two visible CUDA devices"
        )
    ...
    if (
        ...
        raise M03RV12PredictiveWorkflowError(
            "each v12 rank requires one NVIDIA H100 80GB HBM3"
        )
    owns = False
    ...
    if dist.get_world_size() != 2 or dist.get_rank() != rank:
        raise M03RV12PredictiveWorkflowError("v12 NCCL identity drifted")
...
def resolve_m03r_v12_completion_index(value: int | None) -> int:
    if value is None:
    ...
    if not isinstance(value, int) or isinstance(value, bool) or value not in range(3):
        raise M03RV12PredictiveWorkflowError("v12 completion index must be 0, 1, or 2")
    return value
...
def _resolve_worker(
    ...
    if worker.setting_index != completion_index:
        raise M03RV12PredictiveWorkflowError("v12 completion mapping drifted")
    return worker
...
def _validate_runtime_package_members(
    ...
    expected = {
        ...
        package_root
        / "source"
        / "src/rl_quant/workflows/top2000_m03r_v12_predictive.py": (
    ...
    for path, digest in expected.items():
        try:
            ...
            raise M03RV12PredictiveWorkflowError(
                "v12 runtime package member is unavailable"
            ) from exc
        if path.is_symlink() or not path.is_file() or status.st_size <= 0:
            raise M03RV12PredictiveWorkflowError(
                "v12 runtime package member is not a regular file"
            )
        if _file_sha256(path) != digest:
            raise M03RV12PredictiveWorkflowError(
                "v12 runtime package member hash drifted"
            )


def _new_policy(
    setting_index: int, device: torch.device
) -> Top2000M03RV12PredictivePolicy:
...
def _qualification_artifact(lineage: Any) -> dict[str, Any]:
    return {
        "schema": "rl-quant.top2000-dev.m03r-v12-fold-qualification-artifact-v1",
...
def run_m03r_v12_predictive_worker(
    package_plan_path: str | Path,
    ...
    *,
    ...
    if authorization.package_plan_file_sha256 != expected_package_plan_file_sha256:
        raise M03RV12PredictiveWorkflowError(
            "v12 authorization and package-plan file disagree"
        )
    index = resolve_m03r_v12_completion_index(completion_index)
    ...
    try:
        ...
        if rank == 0:
            startup_sha = _write_immutable_json(
                ...
                {
                    "schema": M03R_V12_STARTUP_SCHEMA,
        ...
        if not isinstance(startup_sha, str):
            raise M03RV12PredictiveWorkflowError("v12 startup receipt missing")
        if startup_only:
            ...
            if rank == 0:
                unsigned = {
                    "schema": M03R_V12_CAPACITY_TERMINAL_SCHEMA,
                    "package_plan_sha256": package.package_plan_sha256,
                    "authorization_receipt_sha256": authorization.receipt_sha256,
                    "worker_plan_sha256": worker.receipt_sha256,
                    "setting_index": worker.setting_index,
        ...
        if cache.daily_ohlcv.shape[0] != TOP2000_M03R_V7_DEV_REQUIRED_STATE_ROWS:
            raise M03RV12PredictiveWorkflowError("v12 cache geometry drifted")
        ...
        if (
            ...
            raise M03RV12PredictiveWorkflowError("v12 risk identity drifted")
        ...
        for fold in folds:
            ...
            if (
                ...
                raise M03RV12PredictiveWorkflowError(
                    "v12 fresh common initial parameter state drifted"
                )
            optimizer, partition = build_m03r_v12_optimizer(training_policy)
            last_result = None
            for completed in range(worker.predictive_optimizer_updates):
                last_result = run_m03r_v12_pretraining_fold_update(
                    cache,
                    worker,
                    package.schedule,
                    fold,
            ...
            if last_result is None:
                raise M03RV12PredictiveWorkflowError("v12 produced no optimizer step")
            ...
            if (
                ...
                raise M03RV12PredictiveWorkflowError(
                    "v12 rank state or paired-input lineage diverged"
                )
            training_residual_root = _sha256(
                {"rank_ordered_residual_operator_roots": residual_roots}
            )
            horizon = M03R_V12_SELECTED_HORIZON_SESSIONS
            checkpoint_path = (
                output
                / "checkpoints"
            ...
            if rank == 0:
                checkpoint_sha = write_immutable_m03r_v12_alpha_checkpoint(
                    checkpoint_path,
                    training_policy,
            ...
            if not isinstance(checkpoint_sha, str):
                raise M03RV12PredictiveWorkflowError(
                    "v12 checkpoint publication failed"
                )
            dist.barrier()
            ...
            if len(set(risk_hashes)) != 1:
                raise M03RV12PredictiveWorkflowError(
                    "v12 qualified risk state diverged between ranks"
                )
            ...
            lineage = evaluate_m03r_v12_loaded_qualification_fold(
                cache,
                worker,
                fold,
                risk_source,
            ...
            if len(set(lineage_hashes)) != 1:
                raise M03RV12PredictiveWorkflowError(
                    "v12 qualification lineage diverged between ranks"
                )
            artifact_path = (
                output
                / "fold-artifacts"
            ...
            if not isinstance(artifact_sha, str):
                raise M03RV12PredictiveWorkflowError(
                    "v12 qualification artifact publication failed"
                )
            horizon_rows = {
                str(horizon): {
            ...
            fold_evidence.append(lineage.fold_evidence)
            del loaded_policy
            if rank == 0:
                unsigned_fold = {
                    "schema": M03R_V12_FOLD_TERMINAL_SCHEMA,
                    "package_plan_sha256": package.package_plan_sha256,
                    "authorization_receipt_sha256": authorization.receipt_sha256,
                    "worker_plan_sha256": worker.receipt_sha256,
                    "setting_index": worker.setting_index,
        ...
        if rank == 0:
            score_rows = tuple(row.score_session_index for row in fold_evidence)
            bootstrap = build_m03r_v12_bootstrap_plan(
                score_rows,
                bootstrap_seed=worker.seed,
            )
            qualification = qualify_m03r_v12_predictive_candidate(
                tuple(fold_evidence), bootstrap
            )
            selected = qualification if qualification.passed else None
            unsigned_terminal = {
                "schema": M03R_V12_WORKER_TERMINAL_SCHEMA,
                "package_plan_sha256": package.package_plan_sha256,
                "package_plan_file_sha256": expected_package_plan_file_sha256,
                "authorization_receipt_sha256": authorization.receipt_sha256,
                "worker_plan_sha256": worker.receipt_sha256,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        run_m03r_v12_predictive_worker(
            args.package_plan,
        ...
        if output_raw is not None and rank == "0":
            ...
            if output.is_dir() and not (output / "worker-error.json").exists():
                _write_immutable_json(
                    ...
                    {
                        "schema": M03R_V12_WORKER_ERROR_SCHEMA,
...
__all__ = [
    "M03R_V12_CAPACITY_TERMINAL_SCHEMA",
    "M03R_V12_FOLD_TERMINAL_SCHEMA",
    "M03R_V12_STARTUP_SCHEMA",
    "M03R_V12_WORKER_ERROR_SCHEMA",
    "M03R_V12_WORKER_TERMINAL_SCHEMA",
    "M03RV12PredictiveWorkflowError",
    "main",
    "resolve_m03r_v12_completion_index",
    "run_m03r_v12_predictive_worker",
]
