"""Same-image, zero-GPU static validation for the M03R-v15 package.
...
class M03RV15StaticValidationError(RuntimeError):
    """The immutable v15 package or zero-GPU process surface drifted."""
...
def _require_hash(path: Path, expected: str, label: str) -> None:
    if path.is_symlink() or not path.is_file() or _file_sha256(path) != expected:
        raise M03RV15StaticValidationError(f"{label} hash or file type drifted")
...
def validate_static_package(
    *,
    ...
    if package_root != Path(expected_package_root).resolve():
        raise M03RV15StaticValidationError("package root is not the bound mount")
    output = Path(output_root)
    if output.is_symlink() or not output.is_dir() or tuple(output.iterdir()):
        raise M03RV15StaticValidationError("static output root must start empty")
    if os.environ.get("NVIDIA_VISIBLE_DEVICES") != "none":
        raise M03RV15StaticValidationError("static process is not GPU-masked")
    expected = {
