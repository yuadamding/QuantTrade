"""Same-image, zero-GPU static validation for the M03R-v13 package.

This entrypoint reads immutable package metadata and hashes only.  It never
opens market tensors, initializes a model, trains, or accesses 2026 outcomes.
"""
...
class M03RV13StaticValidationError(RuntimeError):
    """The immutable v13 package or zero-GPU process surface drifted."""
...
def _require_hash(path: Path, expected: str, label: str) -> None:
    if path.is_symlink() or not path.is_file() or _file_sha256(path) != expected:
        raise M03RV13StaticValidationError(f"{label} hash or file type drifted")
...
def validate_static_package(
    *,
    ...
    if package_root != Path(expected_package_root).resolve():
        raise M03RV13StaticValidationError("package root is not the bound mount")
    output = Path(output_root)
    if output.is_symlink() or not output.is_dir() or tuple(output.iterdir()):
        raise M03RV13StaticValidationError("static output root must start empty")
    if os.environ.get("NVIDIA_VISIBLE_DEVICES") != "none":
        raise M03RV13StaticValidationError("static process is not GPU-masked")
    expected = {
    ...
    structural = load_m03r_v13_structural_preflight(
        package_root / "plans" / "real-data-structural-preflight.json",
