...
M03R_V16_LOCAL_PACKAGE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v16-local-predictive-package-v2"
)
...
def _freeze_source_tree(source_root: Path) -> None:
    """Make the manifest-owned tree non-writable before package imports."""
    ...
    for path in sorted(source_root.rglob("*"), reverse=True):
        ...
        if path.is_symlink() or not (
            ...
            raise M03RV16PackageBuildError("V16 source tree contains a special member")
        os.chmod(path, 0o555 if stat.S_ISDIR(status.st_mode) else 0o444)
...
def _verify_frozen_source_tree(
    ...
    *,
    ...
    verified = verify_m03r_v16_source_tree(
        package_root / "source",
        package_root / "source-manifest.json",
        expected_source_manifest_file_sha256=source_manifest_sha256,
        expected_runtime_worker_sha256=worker_source_sha256,
    )
    for path in (package_root / "source").rglob("*"):
        if path.name == "__pycache__" or path.suffix == ".pyc":
            raise M03RV16PackageBuildError(
                "package-owned V16 source contains generated bytecode"
            )
    return verified.source_tree_root_sha256
...
def build_m03r_v16_local_package(
    *,
    ...
    worker_source_sha = source_sha_by_path[M03R_V16_RUNTIME_WORKER]
    _freeze_source_tree(package_root / "source")
