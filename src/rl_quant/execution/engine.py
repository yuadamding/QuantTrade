"""Transition-P&L execution engine: cost/fill model, scalar path, and weight-aware leg path.
...
The scalar helpers stay torch-free arithmetic (``transition_pnl`` runs on tensors OR python scalars)
so the env step, eval loop, and pretraining-target builder can share ONE reward engine. The single
tensor-only helper is ``fill_indices`` (vectorized counterpart of scalar ``fill_index``).
...
Numeric-field validation lives in ``rl_quant.execution.validation``; this module imports the private
coercers from there so its config dataclasses enforce the same contracts as the rest of the framework.
"""
