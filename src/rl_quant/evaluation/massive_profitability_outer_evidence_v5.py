...
from rl_quant.evaluation.massive_profitability_evaluation_plan_v5 import (
    ...
    parse_massive_profitability_evaluation_plan_v5,
)
...
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SCHEMA = (
    "rl-quant.massive-profitability-outer-evidence-v5"
)
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_DATASET = (
    "massive-profitability-outer-evidence-v5"
)
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SOURCE_SCHEMA_SHA256 = semantic_sha256(
    {
        "schema": MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SCHEMA,
        "encoding": "canonical-json",
        "publication": "create-only-source-transaction",
    }
)
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SOURCE_SHA256 = file_sha256(Path(__file__))
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SPEC_SHA256 = semantic_sha256(
    {
        ...
        "sources": "authorized-root-replay-bound-evaluation-source-bundle-v6-only",
...
class MassiveProfitabilityOuterEvidenceV5Error(ValueError):
...
def _digest(name: str, value: object) -> str:
    if (
        ...
        raise MassiveProfitabilityOuterEvidenceV5Error(
...
class MassiveProfitabilityOuterEvidenceV5:
    ...
    schema: str = MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            self.schema != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SCHEMA
            ...
            != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SPEC_SHA256
            or self.implementation_source_sha256
            != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SOURCE_SHA256
            ...
            raise MassiveProfitabilityOuterEvidenceV5Error(
                "outer evidence V5 identity or authorization differs"
            )
        for value in (
            ...
            *self.stitched_capacity_receipts,
            ...
            _digest("outer evidence V5", value)
        if self.semantic_receipt_sha256 != semantic_sha256(self.semantic_unsigned()):
            raise MassiveProfitabilityOuterEvidenceV5Error(
                "outer evidence V5 semantic receipt differs"
            )
        ...
        if (
            ...
            != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SOURCE_SCHEMA_SHA256
            or self.loaded_source.receipt.entitlement_receipt_sha256
            != self.evaluation_source_bundle_semantic_receipt_sha256
        ):
            raise MassiveProfitabilityOuterEvidenceV5Error(
                "outer evidence V5 committed source differs"
            )


def _authorized_residual_v5(
    *,
    ...
    if (
        ...
        raise MassiveProfitabilityOuterEvidenceV5Error(
            "prediction V3 is outside EvaluationPlan V5"
        )
...
def materialize_massive_profitability_outer_evidence_v5(
    *,
    root: str | Path,
    artifact_id: str,
    evaluation_plan: MassiveProfitabilityEvaluationPlanV5,
    phase_plan: MassiveProfitabilityPhasePlanV2,
    evaluation_source_bundle: MassiveProfitabilityEvaluationSourceBundleV6,
    runtime_sources: MassiveProfitabilityEvaluationRuntimeSourcesV6,
    predictions: Sequence[MassiveProfitabilityOuterPredictionsV3],
    committed_at_ms: int,
) -> MassiveProfitabilityOuterEvidenceV5:
    """Derive stitched profitability only from the reconciled V6 source bundle."""

    evaluation_plan.validate()
    phase_plan.validate()
    reloaded_plan = parse_massive_profitability_evaluation_plan_v5(
        root=root, loaded_source=evaluation_plan.loaded_source
    )
    authorized_bundle = authorize_massive_profitability_evaluation_source_bundle_v6(
        root=root,
    ...
    if any(
        ...
        raise MassiveProfitabilityOuterEvidenceV5Error(
            "OuterEvidence V5 rejects non-V3 or unreplayed predictions"
        )
    expected = tuple(
        (fold_index, setting)
    ...
    if (
        ...
        != tuple(
            row.checkpoint_v3_source_receipt_sha256
            for row in authorized_replay.rows
        )
        ...
        raise MassiveProfitabilityOuterEvidenceV5Error(
            "outer evidence V5 roots or prediction inventory differ"
        )
    residuals = tuple(
        _authorized_residual_v5(
    ...
    semantic = {
        ...
        **statistics,
        ...
        "specification_sha256": MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SPEC_SHA256,
        "implementation_source_sha256": MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SOURCE_SHA256,
    ...
    if not artifact_id or any(not (value.isalnum() or value in "-_") for value in artifact_id):
        raise MassiveProfitabilityOuterEvidenceV5Error(
            "outer evidence V5 artifact ID is not path safe"
        )
    relative = f"massive-profitability/outer-evidence-v5/{artifact_id}.json"
    publish_massive_source_object(
        stream=BytesIO(canonical_json_file_bytes(semantic)),
        root=root,
        relative_payload_path=relative,
        dataset_id=MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_DATASET,
        source_object_key=relative,
        requested_at_ms=committed_at_ms,
        downloaded_at_ms=committed_at_ms,
        schema_sha256=MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SOURCE_SCHEMA_SHA256,
        entitlement_receipt_sha256=authorized_bundle.semantic_receipt_sha256,
        committed_at_ms=committed_at_ms,
        request_id=f"P0-OUTER-EVIDENCE-V5-{artifact_id}",
    )
    loaded = load_massive_source_bundle(
        root=root, relative_payload_path=relative, verified_at_ms=committed_at_ms
    )
    return parse_massive_profitability_outer_evidence_v5(
        root=root, loaded_source=loaded
    )


def parse_massive_profitability_outer_evidence_v5(
    *, root: str | Path, loaded_source: LoadedMassiveSourceObject
    ...
    try:
        ...
        raise MassiveProfitabilityOuterEvidenceV5Error(
            "outer evidence V5 source is not JSON"
        ) from exc
    if not isinstance(payload, dict) or raw != canonical_json_file_bytes(payload):
        raise MassiveProfitabilityOuterEvidenceV5Error(
            "outer evidence V5 source is not canonical JSON"
        )
    for name in (
    ...
    result = MassiveProfitabilityOuterEvidenceV5(**payload, loaded_source=loaded_source)
    result.validate()
    ...
    if canonical_json_file_bytes(expected) != raw:
        raise MassiveProfitabilityOuterEvidenceV5Error(
            "outer evidence V5 canonical bytes differ"
        )
    return result
...
__all__ = [
    ...
    "MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V5_SPEC_SHA256",
    "MassiveProfitabilityOuterEvidenceV5",
    "MassiveProfitabilityOuterEvidenceV5Error",
    "materialize_massive_profitability_outer_evidence_v5",
    "parse_massive_profitability_outer_evidence_v5",
]
