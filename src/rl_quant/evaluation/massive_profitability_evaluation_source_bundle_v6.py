...
from rl_quant.evaluation.massive_profitability_evaluation_plan_v5 import (
    MassiveProfitabilityEvaluationPlanV5,
)
...
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SCHEMA = (
    "rl-quant.massive-profitability-evaluation-source-bundle-v6"
)
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_DATASET = (
    "massive-profitability-evaluation-source-bundle-v6"
)
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SOURCE_SCHEMA_SHA256 = semantic_sha256(
    {
        "schema": MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SCHEMA,
        "encoding": "canonical-json",
        "publication": "create-only-source-transaction",
    }
)
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SOURCE_SHA256 = file_sha256(Path(__file__))
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SPEC_SHA256 = semantic_sha256(
    {
        "protocol": MASSIVE_FINALIZED_PROFITABILITY_P0_RECEIPT_SHA256,
        "phase": "phase-plan-v2",
        "evaluation": "evaluation-plan-v5",
        "roots": "package-reconciled-including-root-reexecuted-training-authority",
...
class MassiveProfitabilityEvaluationSourceBundleV6Error(ValueError):
    """The V6 source bundle differs from its typed root graph."""


@dataclass(frozen=True, slots=True)
class MassiveProfitabilityEvaluationRuntimeSourcesV6:
    ...
    evaluation_plan: MassiveProfitabilityEvaluationPlanV5
...
@dataclass(frozen=True, slots=True)
class MassiveProfitabilityEvaluationSourceDateV6:
    ...
    def validate(self) -> None:
        if (
            ...
            raise MassiveProfitabilityEvaluationSourceBundleV6Error(
                "source date V6 differs"
            )


@dataclass(frozen=True, slots=True)
class MassiveProfitabilityEvaluationSourceBundleV6:
    ...
    rows: tuple[MassiveProfitabilityEvaluationSourceDateV6, ...]
    ...
    schema: str = MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            self.schema != MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SCHEMA
            ...
            != MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SPEC_SHA256
            or self.implementation_source_sha256
            != MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SOURCE_SHA256
            ...
            raise MassiveProfitabilityEvaluationSourceBundleV6Error(
                "source bundle V6 identity or authorization differs"
            )
        ...
        if (
            ...
            != MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SOURCE_SCHEMA_SHA256
            or self.loaded_source.receipt.entitlement_receipt_sha256
            != self.evaluation_plan_semantic_receipt_sha256
        ):
            raise MassiveProfitabilityEvaluationSourceBundleV6Error(
                "source bundle V6 committed source differs"
            )


def _reconciled_rows(
    runtime: MassiveProfitabilityEvaluationRuntimeSourcesV6,
) -> tuple[MassiveProfitabilityEvaluationSourceDateV6, ...]:
    ...
    if set(feature_by_origin) != outer_origins or set(target_by_date) != {
        ...
        raise MassiveProfitabilityEvaluationSourceBundleV6Error(
            "source bundle V6 accounting inventory contains nonouter rows"
        )
    result = []
    for row in base:
        ...
        item = MassiveProfitabilityEvaluationSourceDateV6(
...
def materialize_massive_profitability_evaluation_source_bundle_v6(
    *,
    root: str | Path,
    artifact_id: str,
    runtime_sources: MassiveProfitabilityEvaluationRuntimeSourcesV6,
    committed_at_ms: int,
) -> MassiveProfitabilityEvaluationSourceBundleV6:
    authorized_replay = authorize_massive_profitability_training_replay_authority_v1(
        root=root,
        replay_authority=runtime_sources.training_replay_authority,
    ...
    if not qualified:
        raise MassiveProfitabilityEvaluationSourceBundleV6Error(
            "source bundle V6 roots are not qualified"
        )
    semantic = {
        "schema": MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SCHEMA,
        ...
        "specification_sha256": MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SPEC_SHA256,
        "implementation_source_sha256": MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SOURCE_SHA256,
    ...
    if not artifact_id or any(not (value.isalnum() or value in "-_") for value in artifact_id):
        raise MassiveProfitabilityEvaluationSourceBundleV6Error(
            "source bundle V6 artifact ID is not path safe"
        )
    relative = f"massive-profitability/evaluation-source-bundle-v6/{artifact_id}.json"
    publish_massive_source_object(
        stream=BytesIO(canonical_json_file_bytes(semantic)),
        root=root,
        relative_payload_path=relative,
        dataset_id=MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_DATASET,
        source_object_key=relative,
        requested_at_ms=committed_at_ms,
        downloaded_at_ms=committed_at_ms,
        schema_sha256=MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V6_SOURCE_SCHEMA_SHA256,
        entitlement_receipt_sha256=roots.evaluation_plan.semantic_receipt_sha256,
        committed_at_ms=committed_at_ms,
        request_id=f"P0-EVALUATION-SOURCE-V6-{artifact_id}",
    )
    loaded = load_massive_source_bundle(
        root=root, relative_payload_path=relative, verified_at_ms=committed_at_ms
    )
    parsed = parse_massive_profitability_evaluation_source_bundle_v6(
        root=root, loaded_source=loaded
    )
    return authorize_massive_profitability_evaluation_source_bundle_v6(
        root=root, source_bundle=parsed, runtime_sources=runtime_sources
    )


def parse_massive_profitability_evaluation_source_bundle_v6(
    *, root: str | Path, loaded_source: LoadedMassiveSourceObject
    ...
    if not isinstance(payload, dict) or raw != canonical_json_file_bytes(payload):
        raise MassiveProfitabilityEvaluationSourceBundleV6Error(
            "source bundle V6 is not canonical JSON"
        )
    rows = tuple(
        MassiveProfitabilityEvaluationSourceDateV6(**row)
        for row in payload.pop("rows")
    )
    payload["outer_test_session_dates"] = tuple(payload["outer_test_session_dates"])
    result = MassiveProfitabilityEvaluationSourceBundleV6(
        **payload,
        rows=rows,
    ...
    if canonical_json_file_bytes(expected) != raw:
        raise MassiveProfitabilityEvaluationSourceBundleV6Error(
            "source bundle V6 canonical bytes differ"
        )
    return result


def authorize_massive_profitability_evaluation_source_bundle_v6(
    *,
    root: str | Path,
    source_bundle: MassiveProfitabilityEvaluationSourceBundleV6,
    runtime_sources: MassiveProfitabilityEvaluationRuntimeSourcesV6,
) -> MassiveProfitabilityEvaluationSourceBundleV6:
    parsed = parse_massive_profitability_evaluation_source_bundle_v6(
    ...
    if (
        ...
        raise MassiveProfitabilityEvaluationSourceBundleV6Error(
            "source bundle V6 does not reconcile to its runtime roots"
        )
    result = replace(
        parsed,
...
__all__ = [
    ...
    "MassiveProfitabilityEvaluationSourceBundleV6",
    "MassiveProfitabilityEvaluationSourceBundleV6Error",
    "MassiveProfitabilityEvaluationSourceDateV6",
    "authorize_massive_profitability_evaluation_source_bundle_v6",
    "materialize_massive_profitability_evaluation_source_bundle_v6",
    "parse_massive_profitability_evaluation_source_bundle_v6",
]
