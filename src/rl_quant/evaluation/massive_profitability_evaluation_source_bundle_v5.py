"""Training-replay-bound component-root reconciliation for outer evaluation."""
...
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SCHEMA = (
    "rl-quant.massive-profitability-evaluation-source-bundle-v5"
)
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_DATASET = (
    "massive-profitability-evaluation-source-bundle-v5"
)
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SOURCE_SCHEMA_SHA256 = semantic_sha256(
    {
        "schema": MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SCHEMA,
        "encoding": "canonical-json",
        "publication": "create-only-source-transaction",
    }
)
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SOURCE_SHA256 = file_sha256(Path(__file__))
MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SPEC_SHA256 = semantic_sha256(
    {
        "protocol": MASSIVE_FINALIZED_PROFITABILITY_P0_RECEIPT_SHA256,
        "phase": "phase-plan-v2",
        "evaluation": "evaluation-plan-v4",
...
class MassiveProfitabilityEvaluationSourceBundleV5Error(ValueError):
    """The V4 source bundle differs from its typed root graph."""


@dataclass(frozen=True, slots=True)
class MassiveProfitabilityEvaluationRuntimeSourcesV5:
...
@dataclass(frozen=True, slots=True)
class MassiveProfitabilityEvaluationSourceDateV5:
    ...
    def validate(self) -> None:
        if (
            ...
            raise MassiveProfitabilityEvaluationSourceBundleV5Error(
                "source date V5 differs"
            )


@dataclass(frozen=True, slots=True)
class MassiveProfitabilityEvaluationSourceBundleV5:
    ...
    rows: tuple[MassiveProfitabilityEvaluationSourceDateV5, ...]
    ...
    schema: str = MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            self.schema != MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SCHEMA
            ...
            != MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SPEC_SHA256
            or self.implementation_source_sha256
            != MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SOURCE_SHA256
            ...
            raise MassiveProfitabilityEvaluationSourceBundleV5Error(
                "source bundle V5 identity or authorization differs"
            )
        ...
        if (
            ...
            != MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SOURCE_SCHEMA_SHA256
            or self.loaded_source.receipt.entitlement_receipt_sha256
            != self.evaluation_plan_semantic_receipt_sha256
        ):
            raise MassiveProfitabilityEvaluationSourceBundleV5Error(
                "source bundle V5 committed source differs"
            )


def _reconciled_rows(
    runtime: MassiveProfitabilityEvaluationRuntimeSourcesV5,
) -> tuple[MassiveProfitabilityEvaluationSourceDateV5, ...]:
    ...
    if set(feature_by_origin) != outer_origins or set(target_by_date) != {
        ...
        raise MassiveProfitabilityEvaluationSourceBundleV5Error(
            "source bundle V5 accounting inventory contains nonouter rows"
        )
    result = []
    for row in base:
        ...
        item = MassiveProfitabilityEvaluationSourceDateV5(
...
def materialize_massive_profitability_evaluation_source_bundle_v5(
    *,
    root: str | Path,
    artifact_id: str,
    ...
    if not qualified:
        raise MassiveProfitabilityEvaluationSourceBundleV5Error(
            "source bundle V5 roots are not qualified"
        )
    semantic = {
        "schema": MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SCHEMA,
        ...
        "specification_sha256": MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SPEC_SHA256,
        "implementation_source_sha256": MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SOURCE_SHA256,
    ...
    if not artifact_id or any(not (value.isalnum() or value in "-_") for value in artifact_id):
        raise MassiveProfitabilityEvaluationSourceBundleV5Error(
            "source bundle V5 artifact ID is not path safe"
        )
    relative = f"massive-profitability/evaluation-source-bundle-v5/{artifact_id}.json"
    publish_massive_source_object(
        stream=BytesIO(canonical_json_file_bytes(semantic)),
        root=root,
        relative_payload_path=relative,
        dataset_id=MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_DATASET,
        source_object_key=relative,
        requested_at_ms=committed_at_ms,
        downloaded_at_ms=committed_at_ms,
        schema_sha256=MASSIVE_PROFITABILITY_EVALUATION_SOURCE_BUNDLE_V5_SOURCE_SCHEMA_SHA256,
        entitlement_receipt_sha256=roots.evaluation_plan.semantic_receipt_sha256,
        committed_at_ms=committed_at_ms,
        request_id=f"P0-EVALUATION-SOURCE-V5-{artifact_id}",
    )
    loaded = load_massive_source_bundle(
        root=root, relative_payload_path=relative, verified_at_ms=committed_at_ms
    )
    parsed = parse_massive_profitability_evaluation_source_bundle_v5(
        root=root, loaded_source=loaded
    )
    return authorize_massive_profitability_evaluation_source_bundle_v5(
        root=root, source_bundle=parsed, runtime_sources=runtime_sources
    )


def parse_massive_profitability_evaluation_source_bundle_v5(
    *, root: str | Path, loaded_source: LoadedMassiveSourceObject
    ...
    if not isinstance(payload, dict) or raw != canonical_json_file_bytes(payload):
        raise MassiveProfitabilityEvaluationSourceBundleV5Error(
            "source bundle V5 is not canonical JSON"
        )
    rows = tuple(
        MassiveProfitabilityEvaluationSourceDateV5(**row)
        for row in payload.pop("rows")
    )
    payload["outer_test_session_dates"] = tuple(payload["outer_test_session_dates"])
    result = MassiveProfitabilityEvaluationSourceBundleV5(
        **payload,
        rows=rows,
    ...
    if canonical_json_file_bytes(expected) != raw:
        raise MassiveProfitabilityEvaluationSourceBundleV5Error(
            "source bundle V5 canonical bytes differ"
        )
    return result


def authorize_massive_profitability_evaluation_source_bundle_v5(
    *,
    root: str | Path,
    source_bundle: MassiveProfitabilityEvaluationSourceBundleV5,
    runtime_sources: MassiveProfitabilityEvaluationRuntimeSourcesV5,
) -> MassiveProfitabilityEvaluationSourceBundleV5:
    parsed = parse_massive_profitability_evaluation_source_bundle_v5(
    ...
    if (
        ...
        raise MassiveProfitabilityEvaluationSourceBundleV5Error(
            "source bundle V5 does not reconcile to its runtime roots"
        )
    result = replace(
        parsed,
...
__all__ = [
    ...
    "MassiveProfitabilityEvaluationSourceBundleV5",
    "MassiveProfitabilityEvaluationSourceBundleV5Error",
    "MassiveProfitabilityEvaluationSourceDateV5",
    "authorize_massive_profitability_evaluation_source_bundle_v5",
    "materialize_massive_profitability_evaluation_source_bundle_v5",
    "parse_massive_profitability_evaluation_source_bundle_v5",
]
