"""Embargoed source-bundle-bound outer profitability evidence."""
...
from rl_quant.evaluation.massive_profitability_evaluation_plan_v3 import (
    ...
    parse_massive_profitability_evaluation_plan_v3,
)
...
from rl_quant.evaluation.massive_profitability_predictions_v2 import (
...
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SCHEMA = (
    "rl-quant.massive-profitability-outer-evidence-v3"
)
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_DATASET = (
    "massive-profitability-outer-evidence-v3"
)
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SOURCE_SCHEMA_SHA256 = semantic_sha256(
    {
        "schema": MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SCHEMA,
        "encoding": "canonical-json",
        "publication": "create-only-source-transaction",
    }
)
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SOURCE_SHA256 = file_sha256(Path(__file__))
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SPEC_SHA256 = semantic_sha256(
    {
        "protocol": MASSIVE_FINALIZED_PROFITABILITY_P0_RECEIPT_SHA256,
        "phase": "embargoed-phase-plan-v2",
        "tournament": "tournament-plan-and-predictions-v2",
        "sources": "authorized-reconciled-evaluation-source-bundle-v4-only",
...
class MassiveProfitabilityOuterEvidenceV3Error(ValueError):
...
def _digest(name: str, value: object) -> str:
    if (
        ...
        raise MassiveProfitabilityOuterEvidenceV3Error(
...
class MassiveProfitabilityOuterEvidenceV3:
    ...
    schema: str = MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            self.schema != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SCHEMA
            ...
            != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SPEC_SHA256
            or self.implementation_source_sha256
            != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SOURCE_SHA256
            ...
            raise MassiveProfitabilityOuterEvidenceV3Error(
                "outer evidence V3 identity or authorization differs"
            )
        for value in (
            ...
            *self.stitched_capacity_receipts,
            ...
            _digest("outer evidence V3", value)
        if self.semantic_receipt_sha256 != semantic_sha256(self.semantic_unsigned()):
            raise MassiveProfitabilityOuterEvidenceV3Error(
                "outer evidence V3 semantic receipt differs"
            )
        ...
        if (
            ...
            != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SOURCE_SCHEMA_SHA256
            or self.loaded_source.receipt.entitlement_receipt_sha256
            != self.evaluation_source_bundle_semantic_receipt_sha256
        ):
            raise MassiveProfitabilityOuterEvidenceV3Error(
                "outer evidence V3 committed source differs"
            )


def _authorized_residual_v3(
    *,
    evaluation_plan: MassiveProfitabilityEvaluationPlanV3,
    prediction: MassiveProfitabilityOuterPredictionsV2,
    ...
    if (
        ...
        raise MassiveProfitabilityOuterEvidenceV3Error(
            "prediction V2 is outside EvaluationPlan V2"
        )
...
def materialize_massive_profitability_outer_evidence_v3(
    *,
    root: str | Path,
    artifact_id: str,
    evaluation_plan: MassiveProfitabilityEvaluationPlanV3,
    phase_plan: MassiveProfitabilityPhasePlanV2,
    evaluation_source_bundle: MassiveProfitabilityEvaluationSourceBundleV4,
    runtime_sources: MassiveProfitabilityEvaluationRuntimeSourcesV4,
    predictions: Sequence[MassiveProfitabilityOuterPredictionsV2],
    committed_at_ms: int,
) -> MassiveProfitabilityOuterEvidenceV3:
    """Derive stitched profitability only from the reconciled V3 source bundle."""

    evaluation_plan.validate()
    phase_plan.validate()
    reloaded_plan = parse_massive_profitability_evaluation_plan_v3(
        root=root, loaded_source=evaluation_plan.loaded_source
    )
    authorized_bundle = authorize_massive_profitability_evaluation_source_bundle_v4(
        root=root,
    ...
    if (
        ...
        raise MassiveProfitabilityOuterEvidenceV3Error(
            "outer evidence V3 roots or prediction inventory differ"
        )
    residuals = tuple(
        _authorized_residual_v3(
    ...
    semantic = {
        ...
        **statistics,
        ...
        "specification_sha256": MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SPEC_SHA256,
        "implementation_source_sha256": MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SOURCE_SHA256,
    ...
    if not artifact_id or any(not (value.isalnum() or value in "-_") for value in artifact_id):
        raise MassiveProfitabilityOuterEvidenceV3Error(
            "outer evidence V3 artifact ID is not path safe"
        )
    relative = f"massive-profitability/outer-evidence-v3/{artifact_id}.json"
    publish_massive_source_object(
        stream=BytesIO(canonical_json_file_bytes(semantic)),
        root=root,
        relative_payload_path=relative,
        dataset_id=MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_DATASET,
        source_object_key=relative,
        requested_at_ms=committed_at_ms,
        downloaded_at_ms=committed_at_ms,
        schema_sha256=MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SOURCE_SCHEMA_SHA256,
        entitlement_receipt_sha256=authorized_bundle.semantic_receipt_sha256,
        committed_at_ms=committed_at_ms,
        request_id=f"P0-OUTER-EVIDENCE-V3-{artifact_id}",
    )
    loaded = load_massive_source_bundle(
        root=root, relative_payload_path=relative, verified_at_ms=committed_at_ms
    )
    return parse_massive_profitability_outer_evidence_v3(
        root=root, loaded_source=loaded
    )


def parse_massive_profitability_outer_evidence_v3(
    *, root: str | Path, loaded_source: LoadedMassiveSourceObject
    ...
    try:
        ...
        raise MassiveProfitabilityOuterEvidenceV3Error(
            "outer evidence V3 source is not JSON"
        ) from exc
    if not isinstance(payload, dict) or raw != canonical_json_file_bytes(payload):
        raise MassiveProfitabilityOuterEvidenceV3Error(
            "outer evidence V3 source is not canonical JSON"
        )
    for name in (
    ...
    result = MassiveProfitabilityOuterEvidenceV3(**payload, loaded_source=loaded_source)
    result.validate()
    ...
    if canonical_json_file_bytes(expected) != raw:
        raise MassiveProfitabilityOuterEvidenceV3Error(
            "outer evidence V3 canonical bytes differ"
        )
    return result
...
__all__ = [
    ...
    "MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V3_SPEC_SHA256",
    "MassiveProfitabilityOuterEvidenceV3",
    "MassiveProfitabilityOuterEvidenceV3Error",
    "materialize_massive_profitability_outer_evidence_v3",
    "parse_massive_profitability_outer_evidence_v3",
]
