"""Root-replay-authority-bound evaluation plan for the embargoed V2 tournament."""
...
MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SCHEMA = (
    "rl-quant.massive-profitability-evaluation-plan-v6"
)
MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_DATASET = (
    "massive-profitability-evaluation-plan-v6"
)
MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SOURCE_SCHEMA_SHA256 = semantic_sha256(
    {
        "schema": MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SCHEMA,
        "encoding": "canonical-json",
        "publication": "create-only-source-transaction",
    }
)
MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SOURCE_SHA256 = file_sha256(Path(__file__))
MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SPEC_SHA256 = semantic_sha256(
    {
        ...
        "predictions": "root-promoted-prediction-replay-authority-v1",
        "training_replay": "root-promoted-training-replay-authority-v1",
...
class MassiveProfitabilityEvaluationPlanV6Error(ValueError):
...
def _digest(name: str, value: object) -> str:
    if (
        ...
        raise MassiveProfitabilityEvaluationPlanV6Error(
...
class MassiveProfitabilityEvaluationPlanV6:
    ...
    prediction_replay_inventory_sha256: str
    ...
    schema: str = MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            self.schema != MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SCHEMA
            or keys != expected
            ...
            != MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SPEC_SHA256
            or self.implementation_source_sha256
            != MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SOURCE_SHA256
            ...
            raise MassiveProfitabilityEvaluationPlanV6Error(
                "evaluation plan V6 identity or authorization differs"
            )
        ...
        for value in (
            ...
            self.prediction_replay_authority_semantic_receipt_sha256,
            self.prediction_replay_authority_source_receipt_sha256,
            self.prediction_replay_inventory_sha256,
            ...
            *self.fixed_horizon_scaling_receipts,
            ...
            _digest("evaluation plan V6", value)
        self.loaded_source.validate()
        if (
            self.loaded_source.receipt.dataset_id
            != MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_DATASET
            or self.loaded_source.receipt.schema_sha256
            != MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SOURCE_SCHEMA_SHA256
            or self.loaded_source.receipt.entitlement_receipt_sha256
            != self.tournament_plan_receipt_sha256
        ):
            raise MassiveProfitabilityEvaluationPlanV6Error(
                "evaluation plan V6 committed source differs"
            )


def materialize_massive_profitability_evaluation_plan_v6(
    *,
    root: str | Path,
    artifact_id: str,
    ...
    prediction_runtime = resolve_massive_profitability_prediction_replay_authority_v1(
        root=root,
        replay_authority=prediction_replay_authority,
        ...
        features=features,
        targets=targets,
        tournament_plan=reloaded,
    )
    authorized_prediction_replay = prediction_runtime.authority
    authorized_replay = prediction_runtime.training_replay_authority
    ordered = prediction_runtime.predictions
    if any(
        not isinstance(row, MassiveProfitabilityOuterPredictionsV3) for row in ordered
    ):
        raise MassiveProfitabilityEvaluationPlanV6Error(
            "EvaluationPlan V6 rejects every prediction generation except V3"
        )
    replay_pairs = tuple(
        (
            value.checkpoint_v2_source_receipt_sha256,
            value.run_v2_receipt_sha256,
        )
        for value in authorized_replay.rows
    )
    ...
    if (
        ...
        or not authorized_replay.runtime_root_replayed
        or not authorized_replay.outer_evaluation_authorized
        or not authorized_prediction_replay.runtime_predictions_replayed
        or not authorized_prediction_replay.outer_evaluation_authorized
        or authorized_prediction_replay.training_replay_authority_semantic_receipt_sha256
        != authorized_replay.semantic_receipt_sha256
        or authorized_prediction_replay.data_gate_semantic_receipt_sha256
        != data_gate.semantic_receipt_sha256
        or authorized_prediction_replay.phase_plan_semantic_receipt_sha256
        != phase_plan.semantic_receipt_sha256
        or authorized_prediction_replay.tournament_plan_receipt_sha256
        != reloaded.receipt_sha256
        or authorized_replay.data_gate_semantic_receipt_sha256
        != data_gate.semantic_receipt_sha256
        or authorized_replay.phase_plan_semantic_receipt_sha256
        != phase_plan.semantic_receipt_sha256
        or authorized_replay.tournament_plan_receipt_sha256 != reloaded.receipt_sha256
        or replay_pairs != prediction_pairs
    ):
        raise MassiveProfitabilityEvaluationPlanV6Error(
            "evaluation V6 roots do not share one embargoed tournament"
        )
    inventory = []
    for prediction in ordered:
        prediction.validate()
        ...
        if (
            ...
            or prediction.tournament_plan_receipt_sha256
            ...
            raise MassiveProfitabilityEvaluationPlanV6Error(
                "prediction V3 is detached from the frozen V6 evaluation roots"
            )
        body = {
    ...
    semantic = {
        ...
        "training_replay_authority_source_receipt_sha256": authorized_replay.loaded_source.receipt_sha256,
        ...
        "prediction_replay_inventory_sha256": authorized_prediction_replay.row_inventory_sha256,
        ...
        "training_replay_checkpoint_receipts": tuple(
            value.checkpoint_v3_source_receipt_sha256
            for value in authorized_replay.rows
        ),
        "training_replay_run_receipts": tuple(
            value.replayed_run_v3_semantic_receipt_sha256
            for value in authorized_replay.rows
        ),
        ...
        "specification_sha256": MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SPEC_SHA256,
        "implementation_source_sha256": MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SOURCE_SHA256,
    ...
    if not artifact_id or any(
        ...
        raise MassiveProfitabilityEvaluationPlanV6Error(
            "evaluation plan V6 artifact ID is not path safe"
        )
    relative = f"massive-profitability/evaluation-plan-v6/{artifact_id}.json"
    publish_massive_source_object(
        stream=BytesIO(canonical_json_file_bytes(semantic)),
        root=root,
        relative_payload_path=relative,
        dataset_id=MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_DATASET,
        source_object_key=relative,
        requested_at_ms=committed_at_ms,
        downloaded_at_ms=committed_at_ms,
        schema_sha256=MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SOURCE_SCHEMA_SHA256,
        entitlement_receipt_sha256=tournament_plan.receipt_sha256,
        committed_at_ms=committed_at_ms,
        request_id=f"P0-EVALUATION-PLAN-V6-{artifact_id}",
    )
    loaded = load_massive_source_bundle(
        root=root, relative_payload_path=relative, verified_at_ms=committed_at_ms
    )
    return parse_massive_profitability_evaluation_plan_v6(
        root=root, loaded_source=loaded
    )


def parse_massive_profitability_evaluation_plan_v6(
    *, root: str | Path, loaded_source: LoadedMassiveSourceObject
    ...
    if not isinstance(payload, dict) or raw != canonical_json_file_bytes(payload):
        raise MassiveProfitabilityEvaluationPlanV6Error(
            "evaluation plan V6 source is not canonical JSON"
        )
    predictions = tuple(
        MassiveProfitabilityEvaluationPredictionV1(
    ...
    result = MassiveProfitabilityEvaluationPlanV6(
    ...
    if canonical_json_file_bytes(expected) != raw:
        raise MassiveProfitabilityEvaluationPlanV6Error(
            "evaluation plan V6 canonical bytes differ"
        )
    return result
...
__all__ = [
    ...
    "MASSIVE_PROFITABILITY_EVALUATION_PLAN_V6_SPEC_SHA256",
    "MassiveProfitabilityEvaluationPlanV6",
    "MassiveProfitabilityEvaluationPlanV6Error",
    "materialize_massive_profitability_evaluation_plan_v6",
    "parse_massive_profitability_evaluation_plan_v6",
]
