"""Root-training-replay-authority-bound outer profitability evidence."""
...
from rl_quant.evaluation.massive_profitability_evaluation_source_bundle_v7 import (
    ...
    _authorize_massive_profitability_evaluation_source_bundle_v7_from_runtime,
...
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SCHEMA = (
    "rl-quant.massive-profitability-outer-evidence-v6"
)
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_DATASET = (
    "massive-profitability-outer-evidence-v6"
)
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SOURCE_SCHEMA_SHA256 = semantic_sha256(
    {
        "schema": MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SCHEMA,
        "encoding": "canonical-json",
        "publication": "create-only-source-transaction",
    }
)
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SOURCE_SHA256 = file_sha256(Path(__file__))
MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SPEC_SHA256 = semantic_sha256(
    {
        ...
        "sources": "authorized-training-and-prediction-replay-bound-source-bundle-v7-only",
...
class MassiveProfitabilityOuterEvidenceV6Error(ValueError):
...
def _digest(name: str, value: object) -> str:
    if (
        ...
        raise MassiveProfitabilityOuterEvidenceV6Error(
...
class MassiveProfitabilityOuterEvidenceV6:
    ...
    schema: str = MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            self.schema != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SCHEMA
            ...
            != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SPEC_SHA256
            or self.implementation_source_sha256
            != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SOURCE_SHA256
            ...
            raise MassiveProfitabilityOuterEvidenceV6Error(
                "outer evidence V6 identity or authorization differs"
            )
        for value in (
            ...
            *self.stitched_capacity_receipts,
            ...
            _digest("outer evidence V6", value)
        if self.semantic_receipt_sha256 != semantic_sha256(self.semantic_unsigned()):
            raise MassiveProfitabilityOuterEvidenceV6Error(
                "outer evidence V6 semantic receipt differs"
            )
        ...
        if (
            ...
            != MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SOURCE_SCHEMA_SHA256
            or self.loaded_source.receipt.entitlement_receipt_sha256
            != self.evaluation_source_bundle_semantic_receipt_sha256
        ):
            raise MassiveProfitabilityOuterEvidenceV6Error(
                "outer evidence V6 committed source differs"
            )


def _authorized_residual_v6(
    *,
    ...
    if (
        ...
        raise MassiveProfitabilityOuterEvidenceV6Error(
            "prediction V3 is outside EvaluationPlan V6"
        )
...
def materialize_massive_profitability_outer_evidence_v6(
    *,
    root: str | Path,
    artifact_id: str,
    evaluation_plan: MassiveProfitabilityEvaluationPlanV6,
    phase_plan: MassiveProfitabilityPhasePlanV2,
    evaluation_source_bundle: MassiveProfitabilityEvaluationSourceBundleV7,
    runtime_sources: MassiveProfitabilityEvaluationRuntimeSourcesV7,
    committed_at_ms: int,
) -> MassiveProfitabilityOuterEvidenceV6:
    """Derive stitched profitability only from the reconciled V7 source bundle."""

    evaluation_plan.validate()
    phase_plan.validate()
    reloaded_plan = parse_massive_profitability_evaluation_plan_v6(
    ...
    parsed_bundle = parse_massive_profitability_evaluation_source_bundle_v7(
        root=root, loaded_source=evaluation_source_bundle.loaded_source
    )
    authorized_bundle = (
        _authorize_massive_profitability_evaluation_source_bundle_v7_from_runtime(
    ...
    if any(
        ...
        raise MassiveProfitabilityOuterEvidenceV6Error(
            "OuterEvidence V6 rejects non-V3 or unreplayed predictions"
        )
    expected = tuple(
        (fold_index, setting)
    ...
    replay_pairs = tuple(
        (row.checkpoint_v2_source_receipt_sha256, row.run_v2_receipt_sha256)
        for row in authorized_replay.rows
    )
    ...
    if (
        ...
        != phase_plan.semantic_receipt_sha256
        or evaluation_plan.training_replay_authority_semantic_receipt_sha256
        != authorized_replay.semantic_receipt_sha256
        or evaluation_plan.prediction_replay_authority_semantic_receipt_sha256
        != authorized_prediction_replay.semantic_receipt_sha256
        or authorized_bundle.prediction_replay_authority_semantic_receipt_sha256
        != authorized_prediction_replay.semantic_receipt_sha256
        or evaluation_plan.training_replay_checkpoint_receipts
        != tuple(
            row.checkpoint_v3_source_receipt_sha256 for row in authorized_replay.rows
        )
        or evaluation_plan.training_replay_run_receipts
        != tuple(
        ...
        raise MassiveProfitabilityOuterEvidenceV6Error(
            "outer evidence V6 roots or prediction inventory differ"
        )
    residuals = tuple(
        _authorized_residual_v6(
    ...
    semantic = {
        ...
        "prediction_replay_authority_semantic_receipt_sha256": (
            authorized_prediction_replay.semantic_receipt_sha256
        ),
        ...
        **statistics,
        ...
        "specification_sha256": MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SPEC_SHA256,
        "implementation_source_sha256": MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SOURCE_SHA256,
    ...
    if not artifact_id or any(
        ...
        raise MassiveProfitabilityOuterEvidenceV6Error(
            "outer evidence V6 artifact ID is not path safe"
        )
    relative = f"massive-profitability/outer-evidence-v6/{artifact_id}.json"
    publish_massive_source_object(
        stream=BytesIO(canonical_json_file_bytes(semantic)),
        root=root,
        relative_payload_path=relative,
        dataset_id=MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_DATASET,
        source_object_key=relative,
        requested_at_ms=committed_at_ms,
        downloaded_at_ms=committed_at_ms,
        schema_sha256=MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SOURCE_SCHEMA_SHA256,
        entitlement_receipt_sha256=authorized_bundle.semantic_receipt_sha256,
        committed_at_ms=committed_at_ms,
        request_id=f"P0-OUTER-EVIDENCE-V6-{artifact_id}",
    )
    loaded = load_massive_source_bundle(
        root=root, relative_payload_path=relative, verified_at_ms=committed_at_ms
    )
    return parse_massive_profitability_outer_evidence_v6(
        root=root, loaded_source=loaded
    )


def parse_massive_profitability_outer_evidence_v6(
    *, root: str | Path, loaded_source: LoadedMassiveSourceObject
    ...
    try:
        ...
        raise MassiveProfitabilityOuterEvidenceV6Error(
            "outer evidence V6 source is not JSON"
        ) from exc
    if not isinstance(payload, dict) or raw != canonical_json_file_bytes(payload):
        raise MassiveProfitabilityOuterEvidenceV6Error(
            "outer evidence V6 source is not canonical JSON"
        )
    for name in (
    ...
    result = MassiveProfitabilityOuterEvidenceV6(**payload, loaded_source=loaded_source)
    result.validate()
    ...
    if canonical_json_file_bytes(expected) != raw:
        raise MassiveProfitabilityOuterEvidenceV6Error(
            "outer evidence V6 canonical bytes differ"
        )
    return result
...
__all__ = [
    ...
    "MASSIVE_PROFITABILITY_OUTER_EVIDENCE_V6_SPEC_SHA256",
    "MassiveProfitabilityOuterEvidenceV6",
    "MassiveProfitabilityOuterEvidenceV6Error",
    "materialize_massive_profitability_outer_evidence_v6",
    "parse_massive_profitability_outer_evidence_v6",
]
