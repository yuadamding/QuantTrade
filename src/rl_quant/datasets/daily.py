...
def build_daily_episodes(records: list[dict], episode_len: int, stride: int | None = None) -> list[dict]:
    """records: a DATE-SORTED list of per-day dicts, each with the end-of-day context + day-open:
        {market [d], per_stock [A,d], news_raw [A,M,1], news_mask [A,M], day_open [A]}.
    ...
    return [{"market": market[s:s + L], "per_stock": per_stock[s:s + L],
             "news_raw": news_raw[s:s + L], "news_mask": news_mask[s:s + L],
