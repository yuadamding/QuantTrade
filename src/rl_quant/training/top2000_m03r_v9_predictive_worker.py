"""Immutable three-setting worker plan for M03R-v9 predictive qualification.

This plan authorizes exactly three Indexed completions, each requesting two
H100s and training six folds at seed 17 for exactly 64 predictive updates.
It deliberately contains no economic-policy update or economic Job surface.
"""

from __future__ import annotations

import hashlib
import json
from dataclasses import asdict, dataclass

from rl_quant.protocol.hold30_alpha_m03r_v9_top2000_dev import (
    M03R_V9_DESIGN_ID,
    M03R_V9_PREDICTIVE_SPEC,
    M03R_V9_PROTOCOL_GENERATION,
    M03R_V9_PROTOCOL_SHA256,
    M03R_V9_SETTING_IDS,
    M03RV9HorizonBinding,
    resolve_m03r_v9_setting,
)

M03R_V9_PREDICTIVE_WORKER_PLAN_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v9-predictive-worker-plan-v1"
)
M03R_V9_PREDICTIVE_PANEL_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v9-predictive-panel-plan-v1"
)


class M03RV9PredictiveWorkerError(ValueError):
    """The v9 predictive worker plan or assignment drifted."""


def _sha256(value: object) -> str:
    return hashlib.sha256(
        json.dumps(
            value,
            allow_nan=False,
            ensure_ascii=False,
            separators=(",", ":"),
            sort_keys=True,
        ).encode("utf-8")
    ).hexdigest()


def _digest(name: str, value: str) -> str:
    if (
        not isinstance(value, str)
        or len(value) != 64
        or any(character not in "0123456789abcdef" for character in value)
    ):
        raise M03RV9PredictiveWorkerError(f"{name} must be a lowercase SHA-256")
    return value


@dataclass(frozen=True, slots=True)
class M03RV9PredictiveWorkerPlan:
    setting_index: int
    setting_id: str
    cache_path: str
    cache_sha256: str
    risk_source_manifest_path: str
    risk_source_manifest_file_sha256: str
    projector_manifest_path: str
    projector_manifest_file_sha256: str
    projector_manifest_sha256: str
    projector_binding_sha256: str
    source_manifest_sha256: str
    source_archive_sha256: str
    output_root: str
    horizon_bindings: tuple[M03RV9HorizonBinding, ...] = (
        M03RV9HorizonBinding(21, 21, 21),
        M03RV9HorizonBinding(30, 30, 30),
    )
    seed: int = 17
    fold_count: int = 6
    expected_world_size: int = 2
    h100s_per_worker: int = 2
    token_dim: int = 128
    raw_stock_chunk: int = 512
    activation_checkpointing: bool = True
    mixed_precision: str = "bfloat16"
    predictive_optimizer_updates: int = 64
    early_stopping_enabled: bool = False
    qualification_evaluation_updates: tuple[int, ...] = (64,)
    economic_optimizer_updates: int = 0
    protocol_generation: str = M03R_V9_PROTOCOL_GENERATION
    design_id: str = M03R_V9_DESIGN_ID
    protocol_sha256: str = M03R_V9_PROTOCOL_SHA256
    development_only: bool = True
    reportable: bool = False
    promotion_eligible: bool = False
    schema: str = M03R_V9_PREDICTIVE_WORKER_PLAN_SCHEMA

    def validate(self) -> None:
        setting = resolve_m03r_v9_setting(self.setting_index)
        for name in (
            "cache_sha256",
            "risk_source_manifest_file_sha256",
            "projector_manifest_file_sha256",
            "projector_manifest_sha256",
            "projector_binding_sha256",
            "source_manifest_sha256",
            "source_archive_sha256",
        ):
            _digest(name, getattr(self, name))
        for binding in self.horizon_bindings:
            binding.__post_init__()
        if (
            self.setting_id != setting.setting_id
            or self.setting_id != M03R_V9_SETTING_IDS[self.setting_index]
            or not self.cache_path
            or not self.risk_source_manifest_path
            or not self.projector_manifest_path
            or not self.output_root
            or tuple(
                binding.economic_execution_horizon for binding in self.horizon_bindings
            )
            != (21, 30)
            or self.seed != M03R_V9_PREDICTIVE_SPEC.seed
            or self.fold_count != M03R_V9_PREDICTIVE_SPEC.chronological_fold_count
            or self.expected_world_size != 2
            or self.h100s_per_worker != M03R_V9_PREDICTIVE_SPEC.h100s_per_worker
            or self.token_dim != 128
            or self.raw_stock_chunk != 512
            or not self.activation_checkpointing
            or self.mixed_precision != "bfloat16"
            or self.predictive_optimizer_updates
            != M03R_V9_PREDICTIVE_SPEC.maximum_optimizer_updates
            or self.early_stopping_enabled
            or self.qualification_evaluation_updates
            != M03R_V9_PREDICTIVE_SPEC.qualification_evaluation_updates
            or self.economic_optimizer_updates != 0
            or self.protocol_generation != M03R_V9_PROTOCOL_GENERATION
            or self.design_id != M03R_V9_DESIGN_ID
            or self.protocol_sha256 != M03R_V9_PROTOCOL_SHA256
            or not self.development_only
            or self.reportable
            or self.promotion_eligible
            or self.schema != M03R_V9_PREDICTIVE_WORKER_PLAN_SCHEMA
        ):
            raise M03RV9PredictiveWorkerError("v9 worker plan drifted")

    @property
    def receipt_sha256(self) -> str:
        self.validate()
        return _sha256(asdict(self))


@dataclass(frozen=True, slots=True)
class M03RV9PredictivePanelPlan:
    workers: tuple[M03RV9PredictiveWorkerPlan, ...]
    indexed_completions: int = 3
    parallelism: int = 3
    h100s_per_completion: int = 2
    maximum_h100_requests: int = 6
    economic_panel_authorized: bool = False
    protocol_sha256: str = M03R_V9_PROTOCOL_SHA256
    schema: str = M03R_V9_PREDICTIVE_PANEL_SCHEMA

    def validate(self) -> None:
        for worker in self.workers:
            worker.validate()
        if (
            len(self.workers) != 3
            or tuple(worker.setting_index for worker in self.workers) != (0, 1, 2)
            or tuple(worker.setting_id for worker in self.workers)
            != M03R_V9_SETTING_IDS
            or len({worker.receipt_sha256 for worker in self.workers}) != 3
            or self.indexed_completions != 3
            or self.parallelism != 3
            or self.h100s_per_completion != 2
            or self.maximum_h100_requests != 6
            or self.parallelism * self.h100s_per_completion
            != self.maximum_h100_requests
            or self.economic_panel_authorized
            or self.protocol_sha256 != M03R_V9_PROTOCOL_SHA256
            or self.schema != M03R_V9_PREDICTIVE_PANEL_SCHEMA
        ):
            raise M03RV9PredictiveWorkerError("v9 predictive panel plan drifted")

    @property
    def receipt_sha256(self) -> str:
        self.validate()
        return _sha256(asdict(self))


def resolve_m03r_v9_predictive_setting_index(completion_index: int) -> int:
    if (
        isinstance(completion_index, bool)
        or not isinstance(completion_index, int)
        or completion_index not in range(3)
    ):
        raise M03RV9PredictiveWorkerError(
            "JOB_COMPLETION_INDEX must be exactly 0, 1, or 2"
        )
    return completion_index


__all__ = [
    "M03R_V9_PREDICTIVE_PANEL_SCHEMA",
    "M03R_V9_PREDICTIVE_WORKER_PLAN_SCHEMA",
    "M03RV9PredictivePanelPlan",
    "M03RV9PredictiveWorkerError",
    "M03RV9PredictiveWorkerPlan",
    "resolve_m03r_v9_predictive_setting_index",
]
