"""Executable paired fold-update runtime for M03R-v15 predictive training."""
...
class M03RV15TrainingRuntimeError(ValueError):
    """The v15 executable fold update drifted before mutation."""


@dataclass(frozen=True, slots=True)
class M03RV15FoldUpdateResult:
    update_plan: M03RV15TrainingUpdatePlan
    paired_input: M03RV15PairedInputBinding
    step: M03RV15AlphaStepReceipt
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise M03RV15TrainingRuntimeError("v15 fold-update receipt chain drifted")


def move_and_bind_m03r_v15_sequence(
...
def run_m03r_v15_pretraining_fold_update(
    ...
    *,
    ...
    device: torch.device,
) -> M03RV15FoldUpdateResult:
    """Build and execute one setting-neutral, full-context v15 update."""
    ...
    if (
        policy.v15_setting.setting_index != worker.setting_index
        ...
        raise M03RV15TrainingRuntimeError(
            "v15 worker, schedule, cache, risk, rank, fold, or initial state drifted"
        )
    update_plan = render_m03r_v15_training_update_plan(
        schedule,
        geometry,
    ...
    paired_input = build_m03r_v15_paired_input_binding(
        update_plan,
    ...
    step = train_m03r_v15_predictive_batch_update(
        policy,
        batch,
        optimizer,
        partition,
    ...
    result = M03RV15FoldUpdateResult(update_plan, paired_input, step)
    result.validate()
    return result
