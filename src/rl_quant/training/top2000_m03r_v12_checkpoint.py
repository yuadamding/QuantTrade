"""Immutable three-head checkpoint boundary for M03R-v12."""
...
M03R_V12_ALPHA_CHECKPOINT_SCHEMA = "rl-quant.top2000-dev.m03r-v12-alpha-checkpoint-v2"
_MAX_CHECKPOINT_BYTES = 8 * 1024**3
EvaluationResult = TypeVar("EvaluationResult")


class M03RV12CheckpointError(ValueError):
    """The v12 checkpoint artifact or exact reload drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV12CheckpointError(f"{name} must be a lowercase SHA-256")
    return value
...
class M03RV12LoadedCheckpoint:
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise M03RV12CheckpointError("v12 loaded checkpoint drifted")
        for name, value in (
...
def write_immutable_m03r_v12_alpha_checkpoint(
    path: str | Path,
    ...
    *,
    ...
    if target.exists() or target.is_symlink():
        raise M03RV12CheckpointError("v12 checkpoint target already exists")
    if (
        fold_index not in range(6)
        or selected_horizon_sessions not in M03R_V12_ELIGIBLE_EXECUTION_HORIZONS
        or policy.selected_horizon_sessions != selected_horizon_sessions
    ):
        raise M03RV12CheckpointError("v12 checkpoint cursor drifted")
    for name, value in (
    ...
    payload = {
        "schema": M03R_V12_ALPHA_CHECKPOINT_SCHEMA,
        "protocol_sha256": M03R_V12_PROTOCOL_SHA256,
        "setting_index": policy.v12_setting.setting_index,
...
def load_m03r_v12_alpha_checkpoint_for_evaluation(
    path: str | Path,
    *,
    ...
    source = Path(path)
    if (
        expected_selected_horizon_sessions not in M03R_V12_ELIGIBLE_EXECUTION_HORIZONS
        or policy.selected_horizon_sessions != expected_selected_horizon_sessions
    ):
        raise M03RV12CheckpointError("v12 checkpoint selected horizon drifted")
    expected_file_sha256 = _digest("expected_file_sha256", expected_file_sha256)
    try:
        descriptor = os.open(source, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0))
    except OSError as exc:
        raise M03RV12CheckpointError(
            "v12 checkpoint must be a readable regular non-symlink"
        ) from exc
    try:
        ...
        if (
            ...
            raise M03RV12CheckpointError("v12 checkpoint size or type is invalid")
        digest = hashlib.sha256()
        while block := os.read(descriptor, 1024 * 1024):
            digest.update(block)
        if digest.hexdigest() != expected_file_sha256:
            raise M03RV12CheckpointError("v12 checkpoint file hash drifted")
        ...
        if (
            ...
            raise M03RV12CheckpointError("v12 checkpoint changed while read")
    finally:
        os.close(descriptor)
    if not isinstance(payload, Mapping):
        raise M03RV12CheckpointError("v12 checkpoint payload is not a mapping")
    expected = {
        "schema": M03R_V12_ALPHA_CHECKPOINT_SCHEMA,
        "protocol_sha256": M03R_V12_PROTOCOL_SHA256,
        "setting_index": expected_setting_index,
        "setting_id": M03R_V12_SETTING_IDS[expected_setting_index],
    ...
    if any(payload.get(name) != value for name, value in expected.items()):
        raise M03RV12CheckpointError("v12 checkpoint immutable identity drifted")
    ...
    if (
        ...
        raise M03RV12CheckpointError("v12 checkpoint model state drifted")
    try:
        policy.load_state_dict(dict(state), strict=True)
        stored_identity = M03RV12HeadIdentity(**dict(head_row))
    except (RuntimeError, TypeError, ValueError) as exc:
        raise M03RV12CheckpointError("v12 checkpoint strict load failed") from exc
    observed_identity = policy.v12_head_identity()
    if (
        state_dict_sha256(policy.state_dict()) != payload["model_state_sha256"]
        or observed_identity != stored_identity
    ):
        raise M03RV12CheckpointError("loaded v12 semantic state drifted")
    result = M03RV12LoadedCheckpoint(
        setting_index=expected_setting_index,
        setting_id=M03R_V12_SETTING_IDS[expected_setting_index],
        fold_index=expected_fold_index,
        completed_updates=64,
...
def write_reload_evaluate_m03r_v12_checkpoint(
    path: str | Path,
    policy: Top2000M03RV12PredictivePolicy,
    fresh_policy_factory: Callable[[], Top2000M03RV12PredictivePolicy],
    evaluator: Callable[
        [Top2000M03RV12PredictivePolicy, M03RV12LoadedCheckpoint], EvaluationResult
    ],
    *,
    fold_index: int,
    selected_horizon_sessions: int,
    ...
    source_array_sha256: str,
    asset_axis_sha256: str,
) -> tuple[M03RV12LoadedCheckpoint, EvaluationResult]:
    file_sha256 = write_immutable_m03r_v12_alpha_checkpoint(
        path,
        policy,
        fold_index=fold_index,
        selected_horizon_sessions=selected_horizon_sessions,
    ...
    loaded = load_m03r_v12_alpha_checkpoint_for_evaluation(
        ...
        expected_setting_index=loaded_policy.v12_setting.setting_index,
...
__all__ = [
    "M03R_V12_ALPHA_CHECKPOINT_SCHEMA",
