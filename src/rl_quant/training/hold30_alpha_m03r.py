"""Objective primitives for the M03R active-alpha Hold-30 generation.
...
All result-moving multipliers and factor limits must be supplied by an
immutable experiment plan before :func:`m03r_active_objective` will run.
"""
...
class M03RObjectiveConfig:
    ...
    absolute_factor_exposure_limits: tuple[float, ...]
    ...
    def __post_init__(self) -> None:
        ...
        if not self.factor_names or any(
            not isinstance(name, str) or not name for name in self.factor_names
        ):
            raise M03RObjectiveError("factor_names must be a non-empty tuple")
        if len(set(self.factor_names)) != len(self.factor_names):
            raise M03RObjectiveError("factor_names must be unique")
        if len(self.absolute_factor_exposure_limits) != len(self.factor_names):
            raise M03RObjectiveError(
                "absolute_factor_exposure_limits must align with factor_names"
            )
        for index, value in enumerate(self.absolute_factor_exposure_limits):
            if (
                ...
                raise M03RObjectiveError(
                    f"factor exposure limit {index} must be finite and nonnegative"
                )
        if (
...
class M03RObjectiveInputs:
    ...
    active_factor_exposure: torch.Tensor
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.active_factor_exposure, torch.Tensor)
            or self.active_factor_exposure.ndim != 2
            or self.active_factor_exposure.shape[0] != shape[0]
            or not self.active_factor_exposure.is_floating_point()
            or not bool(torch.isfinite(self.active_factor_exposure).all())
        ):
            raise M03RObjectiveError(
                "active_factor_exposure must be finite [session, factor]"
            )
...
def m03r_active_objective(
    ...
    *,
    ...
    if inputs.active_factor_exposure.shape[1] != len(config.factor_names):
        raise M03RObjectiveError(
            "active_factor_exposure width must match the frozen factor inventory"
        )
    ...
    factors = inputs.active_factor_exposure[mask]
    ...
    limits = factors.new_tensor(config.absolute_factor_exposure_limits)
    factor_penalty = torch.relu(factors.abs() - limits).square().mean()
