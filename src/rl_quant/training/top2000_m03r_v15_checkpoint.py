"""Immutable direct-h3 checkpoint round trip for M03R-v15."""
...
M03R_V15_ALPHA_CHECKPOINT_SCHEMA = "rl-quant.top2000-dev.m03r-v15-alpha-checkpoint-v1"
_MAX_CHECKPOINT_BYTES = 8 * 1024**3
EvaluationResult = TypeVar("EvaluationResult")


class M03RV15CheckpointError(ValueError):
    """The v15 checkpoint artifact or exact reload drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV15CheckpointError(f"{name} must be a lowercase SHA-256")
    return value
...
def _expected_updates(fold_index: int) -> int:
    if isinstance(fold_index, bool) or fold_index not in range(6):
        raise M03RV15CheckpointError("v15 checkpoint fold index drifted")
    try:
        return render_m03r_v15_fold_geometries(1001)[fold_index].optimizer_updates
    except (IndexError, TypeError) as exc:
        raise M03RV15CheckpointError("v15 checkpoint fold index drifted") from exc


@dataclass(frozen=True, slots=True)
class M03RV15LoadedCheckpoint:
    setting_index: int
    setting_id: str
    fold_index: int
    ...
    head_identity: M03RV15HeadIdentity
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.selected_epoch_index not in range(8)
            ...
            raise M03RV15CheckpointError("v15 loaded checkpoint drifted")
        for name, value in (
...
def write_immutable_m03r_v15_alpha_checkpoint(
    path: str | Path,
    ...
    *,
    ...
    if target.exists() or target.is_symlink():
        raise M03RV15CheckpointError("v15 checkpoint target already exists")
    if (
        fold_index not in range(6)
        or completed_updates != _expected_updates(fold_index)
        or selected_epoch_index not in range(8)
        or policy.selected_horizon_sessions
        != M03R_V15_SELECTED_HORIZON_SESSIONS
    ):
        raise M03RV15CheckpointError("v15 checkpoint cursor drifted")
    for name, value in (
    ...
    identity = policy.v15_head_identity()
    payload = {
        "schema": M03R_V15_ALPHA_CHECKPOINT_SCHEMA,
...
def load_m03r_v15_alpha_checkpoint_for_evaluation(
    path: str | Path,
    *,
    ...
    expected_selected_epoch_index: int,
    ...
    policy: Top2000M03RV15PredictivePolicy,
) -> M03RV15LoadedCheckpoint:
    source = Path(path)
    if (
        policy.selected_horizon_sessions
        != M03R_V15_SELECTED_HORIZON_SESSIONS
        or expected_completed_updates != _expected_updates(expected_fold_index)
        or expected_selected_epoch_index not in range(8)
    ):
        raise M03RV15CheckpointError("v15 checkpoint horizon or update drifted")
    expected_file_sha256 = _digest("expected_file_sha256", expected_file_sha256)
    try:
        descriptor = os.open(source, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0))
    except OSError as exc:
        raise M03RV15CheckpointError(
            "v15 checkpoint must be a readable regular non-symlink"
        ) from exc
    try:
        ...
        if (
            ...
            raise M03RV15CheckpointError("v15 checkpoint size or type is invalid")
        digest = hashlib.sha256()
        while block := os.read(descriptor, 1024 * 1024):
            digest.update(block)
        if digest.hexdigest() != expected_file_sha256:
            raise M03RV15CheckpointError("v15 checkpoint file hash drifted")
        ...
        if (
            ...
            raise M03RV15CheckpointError("v15 checkpoint changed while read")
    finally:
        os.close(descriptor)
    if not isinstance(payload, Mapping):
        raise M03RV15CheckpointError("v15 checkpoint payload is not a mapping")
    if expected_setting_index not in range(len(M03R_V15_SETTING_IDS)):
        raise M03RV15CheckpointError("v15 expected setting index drifted")
    expected = {
        "schema": M03R_V15_ALPHA_CHECKPOINT_SCHEMA,
        "protocol_sha256": M03R_V15_PROTOCOL_SHA256,
        "setting_index": expected_setting_index,
        "setting_id": M03R_V15_SETTING_IDS[expected_setting_index],
        "fold_index": expected_fold_index,
        "completed_updates": expected_completed_updates,
        "selected_epoch_index": expected_selected_epoch_index,
        "selected_horizon_sessions": M03R_V15_SELECTED_HORIZON_SESSIONS,
        ...
        "checkpoint_selection_receipt_sha256": _digest(
    ...
    if any(payload.get(name) != value for name, value in expected.items()):
        raise M03RV15CheckpointError("v15 checkpoint immutable identity drifted")
    ...
    if (
        ...
        raise M03RV15CheckpointError("v15 checkpoint model state drifted")
    try:
        policy.load_state_dict(dict(state), strict=True)
        stored_identity = M03RV15HeadIdentity(**dict(head_row))
    except (RuntimeError, TypeError, ValueError) as exc:
        raise M03RV15CheckpointError("v15 checkpoint strict load failed") from exc
    observed_identity = policy.v15_head_identity()
    if (
        state_dict_sha256(policy.state_dict()) != payload["model_state_sha256"]
        or observed_identity != stored_identity
    ):
        raise M03RV15CheckpointError("loaded v15 semantic state drifted")
    result = M03RV15LoadedCheckpoint(
        setting_index=expected_setting_index,
        setting_id=M03R_V15_SETTING_IDS[expected_setting_index],
        fold_index=expected_fold_index,
        completed_updates=expected_completed_updates,
...
def write_reload_evaluate_m03r_v15_checkpoint(
    path: str | Path,
    policy: Top2000M03RV15PredictivePolicy,
    fresh_policy_factory: Callable[[], Top2000M03RV15PredictivePolicy],
    evaluator: Callable[
        [Top2000M03RV15PredictivePolicy, M03RV15LoadedCheckpoint], EvaluationResult
    ],
    *,
    fold_index: int,
    completed_updates: int,
    ...
    asset_axis_sha256: str,
    checkpoint_selection_receipt_sha256: str,
) -> tuple[M03RV15LoadedCheckpoint, EvaluationResult]:
    file_sha256 = write_immutable_m03r_v15_alpha_checkpoint(
        path,
        policy,
        fold_index=fold_index,
        completed_updates=completed_updates,
    ...
    loaded = load_m03r_v15_alpha_checkpoint_for_evaluation(
        ...
        expected_setting_index=loaded_policy.v15_setting.setting_index,
        expected_fold_index=fold_index,
        expected_completed_updates=completed_updates,
        expected_selected_epoch_index=selected_epoch_index,
...
__all__ = [
    "M03R_V15_ALPHA_CHECKPOINT_SCHEMA",
