"""Immutable common initial parameter bytes shared by both M03R-v15 settings."""
...
M03R_V15_INITIAL_STATE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-common-initial-parameter-state-v1"
)
_MAX_INITIAL_STATE_BYTES = 4 * 1024**3


class M03RV15InitialStateError(ValueError):
    """The common v15 initialization is unavailable or has drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV15InitialStateError(f"{name} must be a lowercase SHA-256")
    return value


def write_m03r_v15_initial_parameter_state(
    path: str | Path,
    ...
    if target.exists() or target.is_symlink():
        raise M03RV15InitialStateError("v15 initial-state target already exists")
    state = {
    ...
    payload = {
        "schema": M03R_V15_INITIAL_STATE_SCHEMA,
        "protocol_sha256": M03R_V15_PROTOCOL_SHA256,
        "output_contract_sha256": M03R_V15_ALPHA_OUTPUT_CONTRACT_SHA256,
...
def load_m03r_v15_initial_parameter_state(
    path: str | Path,
    policy: Top2000M03RV15PredictivePolicy,
    *,
    ...
    try:
        ...
        raise M03RV15InitialStateError(
            "v15 initial-state artifact is unavailable"
        ) from exc
    try:
        ...
        if (
            ...
            raise M03RV15InitialStateError(
                "v15 initial-state artifact type or size is invalid"
            )
        digest = hashlib.sha256()
        while block := os.read(descriptor, 1024 * 1024):
            digest.update(block)
        if digest.hexdigest() != expected_file_sha256:
            raise M03RV15InitialStateError("v15 initial-state file hash drifted")
        ...
        if (
            ...
            raise M03RV15InitialStateError(
                "v15 initial-state artifact changed while read"
            )
    finally:
        os.close(descriptor)
    if not isinstance(payload, Mapping):
        raise M03RV15InitialStateError("v15 initial-state payload is malformed")
    state = payload.get("state_dict")
    if (
        payload.get("schema") != M03R_V15_INITIAL_STATE_SCHEMA
        or payload.get("protocol_sha256") != M03R_V15_PROTOCOL_SHA256
        ...
        or payload.get("v14_state_reused") is not False
        ...
        raise M03RV15InitialStateError("v15 initial-state semantics drifted")
    observed_architecture = _architecture_sha256(policy.state_dict())
    if observed_architecture != expected_architecture_sha256:
        raise M03RV15InitialStateError("v15 policy architecture drifted")
    try:
        policy.load_state_dict(dict(state), strict=True)
    except (RuntimeError, TypeError, ValueError) as exc:
        raise M03RV15InitialStateError(
            "v15 initial-state does not strictly match the policy"
        ) from exc
    if model_state_sha256(policy) != expected_state_sha256:
        raise M03RV15InitialStateError("v15 loaded initial parameter state drifted")


__all__ = [
    "M03R_V15_INITIAL_STATE_SCHEMA",
    "M03RV15InitialStateError",
