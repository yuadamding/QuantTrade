"""Joint six-fold block inference and fail-closed stop gate for M03R-v14."""
...
from rl_quant.training.top2000_m03r_v14_runtime import M03RV14SimpleSleeveTrace

M03R_V14_BOOTSTRAP_PLAN_SCHEMA = "rl-quant.top2000-dev.m03r-v14-bootstrap-plan-v1"
M03R_V14_QUALIFICATION_SCHEMA = "rl-quant.top2000-dev.m03r-v14-qualification-v1"


class M03RV14SelectionError(ValueError):
    """The v14 chronology, bootstrap, or predictive stop gate drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV14SelectionError(f"{name} must be a lowercase SHA-256")
    return value
...
def _draw_indices(
    ...
    *,
    ...
    generator.manual_seed(
        M03R_V14_PREDICTIVE_SPEC.bootstrap_seed + block_sessions * 1_000_003
    )
    ...
    for fold_length in fold_lengths:
        ...
        starts = torch.randint(
            fold_length,
            (M03R_V14_PREDICTIVE_SPEC.bootstrap_replicates, blocks),
            generator=generator,
            dtype=torch.int64,
        )
        local = (starts.unsqueeze(-1) + block_offset) % fold_length
        rows.append(
            local.reshape(M03R_V14_PREDICTIVE_SPEC.bootstrap_replicates, -1)[
                :, :fold_length
            ]
            + offset
        )
...
class M03RV14BootstrapPlan:
    chronology_sha256: str
    fold_lengths: tuple[int, ...]
    draw_sha256_by_block: tuple[str, str, str]
    block_sessions: tuple[int, int, int] = (10, 21, 30)
    replicates: int = M03R_V14_PREDICTIVE_SPEC.bootstrap_replicates
    bootstrap_seed: int = M03R_V14_PREDICTIVE_SPEC.bootstrap_seed
    protocol_sha256: str = M03R_V14_PROTOCOL_SHA256
    schema: str = M03R_V14_BOOTSTRAP_PLAN_SCHEMA

    def validate(self) -> None:
        if (
            self.fold_lengths != (63, 63, 63, 63, 63, 63)
            or self.block_sessions != (10, 21, 30)
            or self.replicates != M03R_V14_PREDICTIVE_SPEC.bootstrap_replicates
            or self.bootstrap_seed != M03R_V14_PREDICTIVE_SPEC.bootstrap_seed
            ...
            or self.schema != M03R_V14_BOOTSTRAP_PLAN_SCHEMA
        ):
            raise M03RV14SelectionError("v14 bootstrap plan drifted")
...
def build_m03r_v14_bootstrap_plan(
    origin_indices_by_fold: tuple[torch.Tensor, ...],
) -> M03RV14BootstrapPlan:
    if len(origin_indices_by_fold) != 6:
        raise M03RV14SelectionError("v14 bootstrap requires six folds")
    chronology: list[int] = []
    ...
    for row in origin_indices_by_fold:
        if (
            ...
            raise M03RV14SelectionError("v14 fold chronology is not ordered/disjoint")
...
@dataclass(frozen=True, slots=True)
class M03RV14PredictiveQualification:
    setting_index: int
    setting_id: str
    ...
    protocol_sha256: str = M03R_V14_PROTOCOL_SHA256
    schema: str = M03R_V14_QUALIFICATION_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.protocol_sha256 != M03R_V14_PROTOCOL_SHA256
            or self.schema != M03R_V14_QUALIFICATION_SCHEMA
        ):
            raise M03RV14SelectionError("v14 predictive qualification drifted")
...
def qualify_m03r_v14_predictive_candidate(
    traces: tuple[M03RV14SimpleSleeveTrace, ...],
    bootstrap: M03RV14BootstrapPlan,
) -> M03RV14PredictiveQualification:
    if len(traces) != 6:
        raise M03RV14SelectionError("v14 qualification requires six fold traces")
    ...
    if (
        ...
        raise M03RV14SelectionError("v14 fold identities or chronology drifted")
    gross = torch.cat(
        tuple(
    ...
    provisional = M03RV14PredictiveQualification(
        setting_index=setting,
        setting_id=M03R_V14_SETTING_IDS[setting],
        ...
        positive_date_fraction_fold_count=sum(
            value > M03R_V14_PREDICTIVE_SPEC.minimum_positive_ic_date_fraction
    ...
    result = M03RV14PredictiveQualification(
        **{
            **asdict(provisional),
            "passed": passed,
...
__all__ = [
    "M03R_V14_BOOTSTRAP_PLAN_SCHEMA",
    "M03R_V14_QUALIFICATION_SCHEMA",
    "M03RV14BootstrapPlan",
    "M03RV14PredictiveQualification",
    "M03RV14SelectionError",
