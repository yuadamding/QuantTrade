"""Deterministic corrected simple-sleeve runtime for M03R-v12."""
...
from rl_quant.training.top2000_m03r_v12_policy import M03RV12AlphaDistribution
...
M03R_V12_SIMPLE_SLEEVE_TRACE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v12-simple-sleeve-trace-v1"
)


class M03RV12RuntimeError(ValueError):
    """The corrected deterministic v12 sleeve is malformed."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV12RuntimeError(f"{name} must be a lowercase SHA-256")
    return value


@dataclass(frozen=True, slots=True)
class M03RV12SimpleSleeveTrace:
    setting_index: int
    setting_id: str
    fold_index: int
    ...
    v12_shared_residual_operator_used: bool = True
    v12_cost_aware_allocator_used: bool = True
    economic_optimizer_updates: int = 0
    protocol_sha256: str = M03R_V12_PROTOCOL_SHA256
    schema: str = M03R_V12_SIMPLE_SLEEVE_TRACE_SCHEMA
    ...
    def unsigned_payload(self) -> dict[str, Any]:
        return {
            ...
            "v12_shared_residual_operator_used": (
                self.v12_shared_residual_operator_used
            ),
            "v12_cost_aware_allocator_used": self.v12_cost_aware_allocator_used,
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or not self.v12_shared_residual_operator_used
            or not self.v12_cost_aware_allocator_used
            or self.economic_optimizer_updates != 0
            or self.protocol_sha256 != M03R_V12_PROTOCOL_SHA256
            or self.schema != M03R_V12_SIMPLE_SLEEVE_TRACE_SCHEMA
            or self.trace_sha256 != _sha256(self.unsigned_payload())
        ):
            raise M03RV12RuntimeError("v12 simple-sleeve trace drifted")
        for name, value in (
            ("checkpoint_file_sha256", self.checkpoint_file_sha256),
            ("checkpoint_model_state_sha256", self.checkpoint_model_state_sha256),
            ("source_receipt_sha256", self.source_receipt_sha256),
...
def run_m03r_v12_simple_sleeve(
    sequence: Hold30Sequence,
    alpha_distributions: tuple[M03RV12AlphaDistribution, ...],
    ...
    *,
    ...
    """Run one hazard-free chronology with exact v12 residual operators."""
    ...
    if (
        ...
        raise M03RV12RuntimeError("v12 sleeve source axes or chronology drifted")
    risk_state.validate()
    ...
    for transition_index, (distribution, operator) in enumerate(
        ...
        if (
            ...
            raise M03RV12RuntimeError(
                "v12 distribution or shared residual operator drifted"
            )

        returns = sequence.asset_returns[transition_index]
        gross = (weights * returns).sum(-1)
        if bool((gross <= -1.0).any()):
            raise M03RV12RuntimeError("v12 simple sleeve lost all value")
        ...
        proposal = build_cost_aware_active_proposal_v4(
            action_anchor,
            sequence.benchmark_weights[fill_index],
        ...
        projection = project_m03r_v9_active_book(
            proposal.proposal.requested_weights,
            sequence.benchmark_weights[fill_index],
            fill_mask,
            sequence.risk_asset_caps[fill_index],
            sequence.risk_gross_max[fill_index],
        ...
        requested_weights.append(proposal.proposal.requested_weights.squeeze(0))
