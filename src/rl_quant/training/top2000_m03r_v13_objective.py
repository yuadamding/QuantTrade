"""Direct h3 rank/economic objective for M03R-v13."""
...
M03R_V13_RANK_SCORE_STANDARD_DEVIATION_FLOOR = 0.25


class M03RV13ObjectiveError(ValueError):
    """The v13 direct h3 objective inputs drifted."""


@dataclass(frozen=True, slots=True)
class M03RV13PredictiveBatch:
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise M03RV13ObjectiveError("v13 predictive batch drifted")


@dataclass(frozen=True, slots=True)
class M03RV13PredictiveLoss:
    total: torch.Tensor
...
def _rank_loss(
    ...
    for date_index in range(prediction.shape[0]):
        ...
        if setting.ranking_objective == "none" or int(mask.sum()) < 2:
            ...
            continue
        selected = prediction[date_index, mask]
        centered = selected - selected.mean()
        prediction_scale = torch.sqrt(
            centered.square()
            .mean()
            .clamp_min(M03R_V13_RANK_SCORE_STANDARD_DEVIATION_FLOOR**2)
        )
...
def m03r_v13_predictive_loss(
    batch: M03RV13PredictiveBatch,
) -> M03RV13PredictiveLoss:
    batch.validate()
    ...
    horizon_scale = 0.02 * math.sqrt(M03R_V13_SELECTED_HORIZON_SESSIONS)
    robust = _date_mean(
    ...
    distributional = _date_mean(
        0.5
        * (
            torch.exp(-2.0 * log_scale) * (target - mean).square()
            + 2.0 * log_scale
        ),
        valid,
    ).mean()
    ...
    if not bool(torch.isfinite(total)):
        raise M03RV13ObjectiveError("v13 predictive loss is non-finite")
    return M03RV13PredictiveLoss(
        total=total,
        ranking=ranking,
...
__all__ = [
    "M03R_V13_RANK_SCORE_STANDARD_DEVIATION_FLOOR",
    "M03RV13ObjectiveError",
