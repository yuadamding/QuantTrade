"""Executable paired-update runtime for M03R-v11 predictive training.
...
from rl_quant.training.top2000_m03r_v11_fold import (
...
class M03RV11TrainingRuntimeError(ValueError):
    """The executable v11 fold-update lineage drifted."""


@dataclass(frozen=True, slots=True)
class M03RV11FoldUpdateResult:
    training_shard: M03RV11TrainingShardPlan
    paired_input: M03RV11PairedInputReceipt
    step_receipt: M03RV11AlphaStepReceipt
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise M03RV11TrainingRuntimeError("v11 fold update lineage drifted")
...
def run_m03r_v11_pretraining_fold_update(
    ...
    *,
    ...
    device: torch.device,
) -> M03RV11FoldUpdateResult:
    """Build and execute one setting-neutral, tensor-bound v11 update."""
    ...
    if (
        ...
        raise M03RV11TrainingRuntimeError(
            "v11 worker, cache, risk, rank, fold, or initial state drifted"
        )

    shard = render_m03r_v11_training_shard_plan(
        worker,
        schedule,
        fold,
    ...
    paired = build_m03r_v11_paired_input_receipt(
        shard,
    ...
    step = train_m03r_v11_alpha_pretraining_update(
        policy,
        batch,
        optimizer,
        partition,
    ...
    result = M03RV11FoldUpdateResult(shard, paired, step)
    result.validate()
    return result
