"""Immutable common initial parameter bytes for all M03R-v16 settings."""
...
class M03RV16InitialStateError(ValueError):
    """The common V16 initialization is unavailable or has drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV16InitialStateError(f"{name} must be a lowercase SHA-256")
    return value


def write_m03r_v16_initial_parameter_state(
    path: str | Path,
    policy: Top2000M03RV16PredictivePolicy,
) -> tuple[str, str, str]:
    target = Path(path)
    if target.exists() or target.is_symlink():
        raise M03RV16InitialStateError("V16 initial-state target already exists")
    state = {
...
def load_m03r_v16_initial_parameter_state(
    path: str | Path,
    policy: Top2000M03RV16PredictivePolicy,
    *,
    ...
    try:
        ...
        raise M03RV16InitialStateError(
            "V16 initial-state artifact is unavailable"
        ) from exc
    try:
        ...
        if (
            ...
            raise M03RV16InitialStateError(
                "V16 initial-state artifact type or size is invalid"
            )
        digest = hashlib.sha256()
        while block := os.read(descriptor, 1024 * 1024):
            digest.update(block)
        if digest.hexdigest() != expected_file_sha256:
            raise M03RV16InitialStateError("V16 initial-state file hash drifted")
        ...
        if (
            ...
            raise M03RV16InitialStateError(
                "V16 initial-state artifact changed while read"
            )
    finally:
        os.close(descriptor)
    if not isinstance(payload, Mapping):
        raise M03RV16InitialStateError("V16 initial-state payload is malformed")
    ...
    if (
        ...
        or payload.get("v15_state_reused") is not False
        ...
        or _architecture_sha256(policy.state_dict()) != expected_architecture_sha256
    ):
        raise M03RV16InitialStateError("V16 initial-state semantics drifted")
    try:
        policy.load_state_dict(dict(state), strict=True)
    except (RuntimeError, TypeError, ValueError) as exc:
        raise M03RV16InitialStateError(
            "V16 initial state does not strictly match the policy"
        ) from exc
    if model_state_sha256(policy) != expected_state_sha256:
        raise M03RV16InitialStateError("V16 loaded initial parameter state drifted")
