"""One-attempt suspended-Job preparation for M03R-v12 on Seadragon.

This operator is the sole v12 create surface.  It performs one server dry-run,
...
CREATE_CONFIG_SCHEMA: Final = "rl-quant.top2000-dev.m03r-v12-create-config-v1"
_SHA256_RE = re.compile(r"^[0-9a-f]{64}$")


class M03RV12SeadragonOperatorError(RuntimeError):
    """The one-attempt suspended preparation failed closed."""


class M03RV12CreateAttachRequired(M03RV12SeadragonOperatorError):
    """Create may have been accepted; the exact identity must not be retried."""


def _require_sha256(name: str, value: str) -> None:
    if _SHA256_RE.fullmatch(value) is None:
        raise M03RV12SeadragonOperatorError(f"{name} must be a lowercase SHA-256")
...
def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise M03RV12SeadragonOperatorError(f"{label} must be an object")
...
def _project_path(path: str, label: str) -> Path:
    ...
    if not value.is_absolute():
        raise M03RV12SeadragonOperatorError(f"{label} must be absolute")
    try:
        value.relative_to(root)
    except ValueError as exc:
        raise M03RV12SeadragonOperatorError(
...
class M03RV12CreateOperatorConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise M03RV12SeadragonOperatorError("v12 create config identity drifted")
        for name in (
            "rendered_path",
            "manifest_path",
            "evidence_root",
...
class CreateTransport(Protocol):
    ...
    def create_once(self, manifest_path: Path) -> M03RV12CreateAttempt: ...
...
class OneCreateKubectl:
    ...
    def _pods(self, selector: str) -> tuple[Mapping[str, Any], ...]:
        ...
        if not isinstance(items, list):
            raise M03RV12SeadragonOperatorError("PodList items are invalid")
    ...
    def create_once(self, manifest_path: Path) -> M03RV12CreateAttempt:
        if self._create_attempted:
            raise M03RV12SeadragonOperatorError("create request must never be retried")
        self._create_attempted = True
        completed = self._run(
            ("create", "-f", str(manifest_path), "-o", "json"), check=False
        )
        return M03RV12CreateAttempt(
    ...
    def delete(self, request: M03RV7ExactJobCleanupRequest, options_path: Path) -> None:
        if request.job_name != self.job_name or request.namespace != self.namespace:
            raise M03RV12SeadragonOperatorError("cleanup target is not this Job")
...
class _CleanupAdapter:
    ...
    def get_pod_log(self, pod_name: str, *, limit_bytes: int) -> bytes:
        ...
        raise M03RV12SeadragonOperatorError("preactivation cleanup has no Pod logs")

    def activate(self, request: Any) -> Mapping[str, Any]:
        del request
        raise M03RV12SeadragonOperatorError("create operator cannot activate")
...
def _load_config(path: Path, expected_sha256: str) -> M03RV12CreateOperatorConfig:
    ...
    try:
        return M03RV12CreateOperatorConfig(**dict(payload))
    except (TypeError, ValueError) as exc:
        raise M03RV12SeadragonOperatorError("v12 create config is invalid") from exc


def _load_rendered(
    config: M03RV12CreateOperatorConfig,
) -> M03RV12RenderedJob:
    ...
    if (
        ...
        raise M03RV12SeadragonOperatorError("rendered or manifest file hash drifted")
    ...
    try:
        rendered = M03RV12RenderedJob(**rendered_payload)
    except (TypeError, ValueError) as exc:
        raise M03RV12SeadragonOperatorError("rendered Job receipt is invalid") from exc
...
def _bind_rendered(
    *,
    ...
    return bind_m03r_v12_admitted_suspended_job(
        rendered=rendered,
...
def _synthetic_dry_read(
    dry: Mapping[str, Any], config: M03RV12CreateOperatorConfig
) -> dict[str, Any]:
...
def _validate_created_identity(
    ...
    *,
    ...
    if (
        ...
        raise M03RV12SeadragonOperatorError("created Job identity drifted")
    return uid
...
def _publish_attach(
    ...
    *,
    ...
    common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-create-attach-required-v1",
            "phase": phase,
...
def prepare_suspended_job_once(
    ...
    *,
    ...
    if _file_sha256(source) != config.operator_source_sha256:
        raise M03RV12SeadragonOperatorError("operator source hash drifted")
    ...
    for ordinal in ("first", "second"):
        ...
        common._exclusive_json(
            ...
            {
                "schema": "rl-quant.top2000-dev.m03r-v12-precreate-absence-v1",
                "job_name": config.job_name,
                "job_absent": True,
                "name_scoped_pods": [],
            },
        ...
        if ordinal == "first":
            sleep(0.1)
    attempt: M03RV12CreateAttempt | None = None
    ...
    common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-create-outcome-v1",
    ...
    if attempt is not None and attempt.stdout.strip():
        try:
            ...
            raise M03RV12CreateAttachRequired(
    ...
    if attempt is not None and attempt.returncode == 0 and response_uid is None:
        error = M03RV12SeadragonOperatorError(
    ...
    if observed is None:
        ...
        common._exclusive_json(
            ...
            {
                "schema": "rl-quant.top2000-dev.m03r-v12-create-reconciliation-v1",
                "outcome": (
    ...
    try:
        ...
        common._exclusive_json(
            ...
            {
                "schema": "rl-quant.top2000-dev.m03r-v12-postaccept-cleanup-error-v1",
    ...
    try:
        ...
        common._exclusive_json(
            ...
            {
                "schema": "rl-quant.top2000-dev.m03r-v12-prepare-success-v1",
                "job_name": config.job_name,
                "job_uid": uid,
                "run_id": config.run_id,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except M03RV12CreateAttachRequired as exc:
        print(
            json.dumps(
...
__all__ = [
    ...
    "M03RV12CreateAttachRequired",
    "M03RV12CreateAttempt",
    "M03RV12CreateOperatorConfig",
    "M03RV12SeadragonOperatorError",
    "OneCreateKubectl",
