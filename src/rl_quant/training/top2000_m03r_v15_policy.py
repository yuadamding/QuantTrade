"""Single direct-to-execution three-session policy output for M03R-v15."""
...
M03R_V15_POLICY_SCHEMA = "rl-quant.top2000-dev.m03r-v15-predictive-policy-v1"
M03R_V15_OUTPUT_SCHEMA = "rl-quant.top2000-dev.m03r-v15-raw-h3-output-v1"
M03R_V15_ALPHA_OUTPUT_CONTRACT_SHA256 = hashlib.sha256(
    (
        "one-three-session-raw-mean-and-log-scale;raw-output-exposes-no-rank-"
        "or-execution-alias;action-operator-creates-the-executable-score;cash-zero;"
...
class M03RV15PolicyError(ValueError):
    """The v15 direct predictive output or identity drifted."""
...
class M03RV15RawH3Prediction:
    ...
    schema: str = M03R_V15_OUTPUT_SCHEMA

    def validate(self) -> None:
        if (
            not isinstance(self.raw_mean, torch.Tensor)
            or self.raw_mean.ndim != 2
            or not self.raw_mean.is_floating_point()
            or not bool(torch.isfinite(self.raw_mean).all())
            or not isinstance(self.raw_log_scale, torch.Tensor)
            or tuple(self.raw_log_scale.shape) != tuple(self.raw_mean.shape)
            or self.raw_log_scale.dtype != self.raw_mean.dtype
            or self.raw_log_scale.device != self.raw_mean.device
            or not bool(torch.isfinite(self.raw_log_scale).all())
            or self.selected_horizon_sessions
            != M03R_V15_SELECTED_HORIZON_SESSIONS
            or self.protocol_sha256 != M03R_V15_PROTOCOL_SHA256
            or self.schema != M03R_V15_OUTPUT_SCHEMA
        ):
            raise M03RV15PolicyError("v15 raw h3 prediction drifted")

    @property
    def economic_scale(self) -> torch.Tensor:
        self.validate()
        return torch.exp(self.raw_log_scale)
            ...
            json.dumps(
                {
                    ...
                    "raw_mean_sha256": _tensor_sha256(self.raw_mean),
                    "raw_log_scale_sha256": _tensor_sha256(self.raw_log_scale),
                },
                separators=(",", ":"),
                sort_keys=True,
            ).encode("ascii")
...
@dataclass(frozen=True, slots=True)
class M03RV15HeadIdentity:
    setting_id: str
    selected_alpha_horizon: int
    economic_mean_head_state_sha256: str
    economic_scale_head_state_sha256: str
    output_contract_sha256: str = M03R_V15_ALPHA_OUTPUT_CONTRACT_SHA256

    def validate(self) -> None:
        resolve_m03r_v15_setting(self.setting_id)
        for value in (
            ...
            if len(value) != 64 or any(
                ...
                raise M03RV15PolicyError("v15 head identity has a malformed digest")
        if (
            self.selected_alpha_horizon != M03R_V15_SELECTED_HORIZON_SESSIONS
            or self.output_contract_sha256
            != M03R_V15_ALPHA_OUTPUT_CONTRACT_SHA256
        ):
            raise M03RV15PolicyError("v15 head identity drifted")
...
class Top2000M03RV15PredictivePolicy(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.v15_setting: M03RV15PredictiveSetting = resolve_m03r_v15_setting(setting)
        if (
            isinstance(selected_horizon_sessions, bool)
            or selected_horizon_sessions != M03R_V15_SELECTED_HORIZON_SESSIONS
        ):
            raise M03RV15PolicyError("v15 model selected horizon drifted")
        ...
        if source_alpha is None:
            raise M03RV15PolicyError("reviewed source policy omitted its alpha head")
        ...
        nn.init.constant_(
            ...
            math.log(0.02 * math.sqrt(M03R_V15_SELECTED_HORIZON_SESSIONS)),
        )
    ...
    def predictive_output(
        ...
        if (
            ...
            raise M03RV15PolicyError("v15 state and availability axes are invalid")
        with torch.autocast(
        ...
        result = M03RV15RawH3Prediction(mean, log_scale)
        result.validate()
        return result

    def v15_head_identity(self) -> M03RV15HeadIdentity:
        result = M03RV15HeadIdentity(
            setting_id=self.v15_setting.setting_id,
...
__all__ = [
    "M03R_V15_ALPHA_OUTPUT_CONTRACT_SHA256",
    "M03R_V15_ENCODER_PARAMETER_PREFIXES",
    "M03R_V15_OUTPUT_SCHEMA",
    "M03R_V15_POLICY_SCHEMA",
    "M03RV15RawH3Prediction",
    "M03RV15HeadIdentity",
    "M03RV15PolicyError",
