"""Immutable two-setting predictive worker and panel plans for M03R-v13."""
...
M03R_V13_WORKER_PLAN_SCHEMA = "rl-quant.top2000-dev.m03r-v13-worker-plan-v1"
M03R_V13_PANEL_PLAN_SCHEMA = "rl-quant.top2000-dev.m03r-v13-panel-plan-v1"
M03R_V13_FOLD_UPDATE_COUNTS = tuple(
    geometry.optimizer_updates for geometry in render_m03r_v13_fold_geometries(1001)
)


class M03RV13PredictiveWorkerError(ValueError):
    """The v13 worker or paired panel identity drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV13PredictiveWorkerError(f"{name} must be a lowercase SHA-256")
    return value


@dataclass(frozen=True, slots=True)
class M03RV13PredictiveWorkerPlan:
    setting_index: int
    setting_id: str
    output_root: str
    cache_path: str
    ...
    fold_optimizer_updates: tuple[int, ...] = M03R_V13_FOLD_UPDATE_COUNTS
    seed: int = M03R_V13_PREDICTIVE_SPEC.seed
    fold_count: int = M03R_V13_PREDICTIVE_SPEC.chronological_fold_count
    expected_world_size: int = M03R_V13_PREDICTIVE_SPEC.expected_world_size
    ...
    schema: str = M03R_V13_WORKER_PLAN_SCHEMA

    def validate(self) -> None:
        setting = resolve_m03r_v13_setting(self.setting_index)
        for name in (
        ...
        if (
            ...
            or self.fold_optimizer_updates != M03R_V13_FOLD_UPDATE_COUNTS
            or self.seed != M03R_V13_PREDICTIVE_SPEC.seed
            or self.fold_count != M03R_V13_PREDICTIVE_SPEC.chronological_fold_count
            or self.expected_world_size
            != M03R_V13_PREDICTIVE_SPEC.expected_world_size
            ...
            or self.schema != M03R_V13_WORKER_PLAN_SCHEMA
        ):
            raise M03RV13PredictiveWorkerError("v13 worker plan drifted")
...
@dataclass(frozen=True, slots=True)
class M03RV13PredictivePanelPlan:
    workers: tuple[M03RV13PredictiveWorkerPlan, ...]
    indexed_completions: int = 2
    parallelism: int = 2
    h100s_per_completion: int = 2
    maximum_h100_requests: int = M03R_V13_PREDICTIVE_SPEC.maximum_h100_requests
    economic_panel_authorized: bool = False
    package_authorized: bool = False
    kubernetes_launch_authorized: bool = False
    protocol_sha256: str = M03R_V13_PROTOCOL_SHA256
    schema: str = M03R_V13_PANEL_PLAN_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            len(self.workers) != len(M03R_V13_SETTING_IDS)
            ...
            or self.kubernetes_launch_authorized
            or self.protocol_sha256 != M03R_V13_PROTOCOL_SHA256
            or self.schema != M03R_V13_PANEL_PLAN_SCHEMA
        ):
            raise M03RV13PredictiveWorkerError("v13 panel plan drifted")
...
__all__ = [
    "M03R_V13_FOLD_UPDATE_COUNTS",
    "M03R_V13_PANEL_PLAN_SCHEMA",
    "M03R_V13_WORKER_PLAN_SCHEMA",
    "M03RV13PredictivePanelPlan",
    "M03RV13PredictiveWorkerError",
