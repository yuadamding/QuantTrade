"""Content-bound local package plan for the M03R-v11 predictive panel."""
...
M03R_V11_PACKAGE_SCHEMA = "rl-quant.top2000-dev.m03r-v11-predictive-package-v1"
M03R_V11_PACKAGE_FILE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v11-predictive-package-file-v1"
)
M03R_V11_EXECUTION_AUTHORIZATION_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v11-predictive-execution-authorization-v1"
)
M03R_V11_PACKAGE_SOURCE_PYTHONPATH = "/mnt/package/source/src"
M03R_V11_RUNTIME_ENTRYPOINT = "rl_quant.workflows.top2000_m03r_v11_predictive"
_MAX_PACKAGE_PLAN_BYTES = 4 * 1024 * 1024
_DIGEST = re.compile(r"^[0-9a-f]{64}$")
_IMAGE = re.compile(r"^[^@\s]+@sha256:([0-9a-f]{64})$")


class M03RV11PackageError(ValueError):
    """The v11 source, data, risk, image, or panel package drifted."""
...
def _digest(name: str, value: str) -> str:
    if _DIGEST.fullmatch(value) is None:
        raise M03RV11PackageError(f"{name} must be one lowercase SHA-256")
    return value


@dataclass(frozen=True, slots=True)
class M03RV11PackageArtifacts:
    ...
    def validate(self) -> None:
        ...
        if match is None or match.group(1) != self.image_digest_sha256:
            raise M03RV11PackageError("v11 image reference must be digest pinned")


@dataclass(frozen=True, slots=True)
class M03RV11PackagePlan:
    artifacts: M03RV11PackageArtifacts
    schedule: M03RV11PanelEpisodeSchedule
    panel: M03RV11PredictivePanelPlan
    plan_directory: str
    package_plan_sha256: str
    source_pythonpath: str = M03R_V11_PACKAGE_SOURCE_PYTHONPATH
    protocol_generation: str = M03R_V11_PROTOCOL_GENERATION
    design_id: str = M03R_V11_DESIGN_ID
    ...
    schema: str = M03R_V11_PACKAGE_SCHEMA
    ...
    def validate(self) -> None:
        ...
        for completion, worker in enumerate(self.panel.workers):
            if (
                ...
                raise M03RV11PackageError(
                    "v11 worker does not bind exact package artifacts"
                )
        if (
            ...
            or self.source_pythonpath != M03R_V11_PACKAGE_SOURCE_PYTHONPATH
            or self.protocol_generation != M03R_V11_PROTOCOL_GENERATION
            or self.design_id != M03R_V11_DESIGN_ID
            ...
            or self.schema != M03R_V11_PACKAGE_SCHEMA
            or self.package_plan_sha256 != _sha256(self.unsigned_payload())
        ):
            raise M03RV11PackageError("v11 package plan identity drifted")


@dataclass(frozen=True, slots=True)
class M03RV11ExecutionAuthorization:
    package_plan_sha256: str
    ...
    worker_source_sha256: str
    image_reference: str
    runtime_entrypoint: str = M03R_V11_RUNTIME_ENTRYPOINT
    ...
    promotion_eligible: bool = False
    protocol_sha256: str = M03R_V11_PROTOCOL_SHA256
    schema: str = M03R_V11_EXECUTION_AUTHORIZATION_SCHEMA

    def validate(self, package: M03RV11PackagePlan) -> None:
        package.validate()
        for name in (
            "package_plan_sha256",
        ...
        if (
            ...
            or self.runtime_entrypoint != M03R_V11_RUNTIME_ENTRYPOINT
            ...
            or self.promotion_eligible
            or self.protocol_sha256 != M03R_V11_PROTOCOL_SHA256
            or self.schema != M03R_V11_EXECUTION_AUTHORIZATION_SCHEMA
        ):
            raise M03RV11PackageError("v11 execution authorization drifted")
...
def write_m03r_v11_execution_authorization(
    path: str | Path,
    ...
    if target.exists() or target.is_symlink():
        raise M03RV11PackageError("v11 execution authorization already exists")
    payload = {
...
def load_m03r_v11_execution_authorization(
    path: str | Path,
    *,
    ...
    try:
        ...
        raise M03RV11PackageError(
            "v11 execution authorization must be a regular non-symlink"
        ) from exc
    try:
        ...
        if (
            ...
            raise M03RV11PackageError("v11 execution authorization is invalid")
        content = bytearray()
        ...
        if digest.hexdigest() != expected_file_sha256:
            raise M03RV11PackageError("v11 execution authorization hash drifted")
        if (
            before.st_dev,
            before.st_ino,
            before.st_size,
            ...
            raise M03RV11PackageError("v11 execution authorization changed while read")
    finally:
        os.close(descriptor)
    ...
    if not isinstance(payload, dict) or not isinstance(
        ...
        raise M03RV11PackageError("v11 execution authorization schema drifted")
    authorization = M03RV11ExecutionAuthorization(**payload["authorization"])
    authorization.validate(package)
    if payload.get("receipt_sha256") != authorization.receipt_sha256:
        raise M03RV11PackageError("v11 execution authorization receipt drifted")
    return authorization


def build_m03r_v11_package_plan(
    artifacts: M03RV11PackageArtifacts,
    schedule: M03RV11PanelEpisodeSchedule,
) -> M03RV11PackagePlan:
    artifacts.validate()
    ...
    if schedule.cache_sha256 != artifacts.cache_artifact_sha256:
        raise M03RV11PackageError("v11 schedule and packaged cache disagree")
    workers = tuple(
        M03RV11PredictiveWorkerPlan(
            setting_index=index,
            setting_id=M03R_V11_SETTING_IDS[index],
    ...
    provisional = M03RV11PackagePlan(
        artifacts=artifacts,
        schedule=schedule,
        panel=M03RV11PredictivePanelPlan(workers),
...
def write_m03r_v11_package_plan(path: str | Path, plan: M03RV11PackagePlan) -> str:
    plan.validate()
    target = Path(path)
    if target.exists() or target.is_symlink():
        raise M03RV11PackageError("v11 package plan target already exists")
    payload = {
        "schema": M03R_V11_PACKAGE_FILE_SCHEMA,
        "plan": asdict(plan),
...
def load_m03r_v11_package_plan(
    path: str | Path,
    *,
    expected_file_sha256: str,
) -> M03RV11PackagePlan:
    ...
    try:
        ...
        raise M03RV11PackageError(
            "v11 package plan must be a readable regular non-symlink"
        ) from exc
    try:
        before = os.fstat(descriptor)
        if (
            ...
            raise M03RV11PackageError("v11 package plan size or type is invalid")
        content = bytearray()
        digest = hashlib.sha256()
        while block := os.read(descriptor, 1024 * 1024):
            digest.update(block)
        ...
        after = os.fstat(descriptor)
        if digest.hexdigest() != expected_file_sha256:
            raise M03RV11PackageError("v11 package plan file hash drifted")
        if (
            before.st_dev,
            before.st_ino,
            before.st_size,
            ...
            raise M03RV11PackageError("v11 package plan changed while read")
    finally:
        os.close(descriptor)
    payload = json.loads(bytes(content))
    if (
        not isinstance(payload, dict)
        or payload.get("schema") != M03R_V11_PACKAGE_FILE_SCHEMA
        or not isinstance(payload.get("plan"), dict)
    ):
        raise M03RV11PackageError("v11 package plan file schema drifted")
    row = payload["plan"]
    artifacts = M03RV11PackageArtifacts(**row["artifacts"])
    schedule = M03RV11PanelEpisodeSchedule(
        **{
            **row["schedule"],
    ...
    if payload.get("package_plan_sha256") != plan.package_plan_sha256:
        raise M03RV11PackageError("v11 package file and plan disagree")
    return plan
...
__all__ = [
    "M03R_V11_EXECUTION_AUTHORIZATION_SCHEMA",
    "M03R_V11_PACKAGE_FILE_SCHEMA",
    "M03R_V11_PACKAGE_SCHEMA",
    "M03R_V11_PACKAGE_SOURCE_PYTHONPATH",
