"""Joint block inference and corrected predictive qualification for M03R-v12."""
...
M03R_V12_BOOTSTRAP_PLAN_SCHEMA = "rl-quant.top2000-dev.m03r-v12-bootstrap-plan-v1"
M03R_V12_FOLD_EVIDENCE_SCHEMA = "rl-quant.top2000-dev.m03r-v12-fold-evidence-v1"
M03R_V12_QUALIFICATION_SCHEMA = "rl-quant.top2000-dev.m03r-v12-qualification-v1"


class M03RV12SelectionError(ValueError):
    """The v12 chronology, inference, or predictive gate drifted."""
...
def _torch_module() -> Any:
    try:
        ...
        raise M03RV12SelectionError(
            "PyTorch is required for v12 statistical tensor operations"
        ) from exc
    return torch
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV12SelectionError(f"{name} must be a lowercase SHA-256")
    return value
...
class M03RV12BootstrapPlan:
    ...
    rule: str = M03R_V12_BOOTSTRAP_RULE
    protocol_sha256: str = M03R_V12_PROTOCOL_SHA256
    schema: str = M03R_V12_BOOTSTRAP_PLAN_SCHEMA
    ...
    def validate(self) -> None:
        if (
            ...
            or self.replicates != M03R_V12_PREDICTIVE_SPEC.bootstrap_replicates
            or self.rule != M03R_V12_BOOTSTRAP_RULE
            or self.protocol_sha256 != M03R_V12_PROTOCOL_SHA256
            or self.schema != M03R_V12_BOOTSTRAP_PLAN_SCHEMA
        ):
            raise M03RV12SelectionError("v12 bootstrap plan drifted")
        ...
        if self.draw_sha256_by_block != expected:
            raise M03RV12SelectionError("v12 bootstrap draws drifted")
...
def build_m03r_v12_bootstrap_plan(
    ...
    *,
    ...
    if len(score_session_index_by_fold) != 6:
        raise M03RV12SelectionError("v12 bootstrap requires six folds")
    ...
    for row in score_session_index_by_fold:
        if (
            ...
            raise M03RV12SelectionError("v12 fold chronology is not disjoint ordered")
    ...
    draw_rows = tuple(
        _tensor_sha256(
            _draw_indices(
                fold_lengths,
                replicates=M03R_V12_PREDICTIVE_SPEC.bootstrap_replicates,
                block_sessions=block,
                seed=bootstrap_seed,
            )
        )
...
class M03RV12FoldEvidence:
    ...
    protocol_sha256: str = M03R_V12_PROTOCOL_SHA256
    schema: str = M03R_V12_FOLD_EVIDENCE_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.horizon_sessions not in set(M03R_V12_ELIGIBLE_EXECUTION_HORIZONS)
            ...
            or self.protocol_sha256 != M03R_V12_PROTOCOL_SHA256
            or self.schema != M03R_V12_FOLD_EVIDENCE_SCHEMA
        ):
            raise M03RV12SelectionError("v12 fold evidence drifted")
        for name, value in (
...
def build_m03r_v12_fold_evidence(
    *,
    setting_index: int,
    fold_index: int,
    horizon_sessions: int,
    ...
    setting = resolve_m03r_v12_setting(setting_index)
    arrays = (
        score_session_index,
        gross_active_return,
...
@dataclass(frozen=True, slots=True)
class M03RV12PredictiveQualification:
    setting_index: int
    setting_id: str
    horizon_sessions: int
    ...
    protocol_sha256: str = M03R_V12_PROTOCOL_SHA256
    schema: str = M03R_V12_QUALIFICATION_SCHEMA

    def validate(self) -> None:
        expected_pass = (
            self.mean_rank_ic >= M03R_V12_PREDICTIVE_SPEC.minimum_mean_spearman_rank_ic
        ...
        if (
            ...
            or self.setting_index not in range(len(M03R_V12_SETTING_IDS))
            ...
            or self.protocol_sha256 != M03R_V12_PROTOCOL_SHA256
            or self.schema != M03R_V12_QUALIFICATION_SCHEMA
        ):
            raise M03RV12SelectionError("v12 predictive qualification drifted")
...
def qualify_m03r_v12_predictive_candidate(
    folds: tuple[M03RV12FoldEvidence, ...],
    bootstrap: M03RV12BootstrapPlan,
) -> M03RV12PredictiveQualification:
    torch = _torch_module()
    if len(folds) != 6:
        raise M03RV12SelectionError("v12 qualification requires six folds")
    ...
    if (
        ...
        raise M03RV12SelectionError("v12 fold identities or common chronology drifted")
    ...
    prediction_dispersion_ok = all(
        ...
        >= M03R_V12_PREDICTIVE_SPEC.minimum_prediction_cross_sectional_std
        for row in ordered
    )
    ratio_min, ratio_max = (
        M03R_V12_PREDICTIVE_SPEC.prediction_target_dispersion_ratio_range
    )
    ...
    result = M03RV12PredictiveQualification(
        setting_index=setting_index,
...
__all__ = [
    "M03R_V12_BOOTSTRAP_PLAN_SCHEMA",
    "M03R_V12_FOLD_EVIDENCE_SCHEMA",
    "M03R_V12_QUALIFICATION_SCHEMA",
    "M03RV12BootstrapPlan",
    "M03RV12FoldEvidence",
    "M03RV12PredictiveQualification",
    "M03RV12SelectionError",
