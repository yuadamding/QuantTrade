"""Context-matched fold geometry and paired epoch schedule for M03R-v15."""
...
M03R_V15_FOLD_SCHEMA = "rl-quant.top2000-dev.m03r-v15-fold-geometry-v1"
M03R_V15_PANEL_SCHEDULE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-panel-episode-schedule-v1"
)
M03R_V15_TRAINING_UPDATE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-training-update-v1"
)
M03R_V15_PAIRED_INPUT_BINDING_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-paired-input-binding-v1"
)
M03R_V15_REQUIRED_STATE_ROWS = 1001
M03R_V15_FIRST_QUALIFICATION_START = 469
M03R_V15_FOLD_ADVANCE = 93
M03R_V15_MINIMUM_LOCAL_ORIGIN = (
    M03R_V15_PREDICTIVE_SPEC.observation_context_sessions - 1
)
M03R_V15_MAXIMUM_LOCAL_TRAINING_ORIGIN = (
    M03R_V15_PREDICTIVE_SPEC.episode_state_rows
    - M03R_V15_SELECTED_HORIZON_SESSIONS
    - 2
)


class M03RV15FoldError(ValueError):
    """The v15 full-context fold or paired update schedule drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV15FoldError(f"{name} is not a lowercase SHA-256")
    return value
...
def _complementary_rank_shards(
    ...
    if (
        len(origins) < M03R_V15_PREDICTIVE_SPEC.expected_world_size
        or any(later <= earlier for earlier, later in pairwise(origins))
    ):
        raise M03RV15FoldError("v15 origins cannot form complementary rank shards")
    first = origins[::2]
    second = origins[1::2]
    if not first or not second:
        raise M03RV15FoldError("v15 paired origin set is too small")
    if (
        set(first).intersection(second)
        or tuple(sorted((*first, *second))) != origins
    ):
        raise M03RV15FoldError("v15 rank shards do not cover the global origins")
    return first, second


@dataclass(frozen=True, slots=True)
class M03RV15FoldGeometry:
    fold_index: int
    ...
    inner_validation_episode_state_start: int
    inner_validation_episode_state_stop_exclusive: int
    ...
    geometry_rule: str = M03R_V15_FOLD_GEOMETRY_RULE
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_FOLD_SCHEMA

    def validate(self) -> None:
        spec = M03R_V15_PREDICTIVE_SPEC
        expected_qualification_start = (
            M03R_V15_FIRST_QUALIFICATION_START
            + M03R_V15_FOLD_ADVANCE * self.fold_index
        )
        if (
            isinstance(self.fold_index, bool)
            or self.fold_index not in range(spec.chronological_fold_count)
            or self.cache_state_rows != M03R_V15_REQUIRED_STATE_ROWS
            ...
            or self.training_origin_start_inclusive != M03R_V15_MINIMUM_LOCAL_ORIGIN
            or self.training_origin_stop_exclusive
            != self.inner_validation_origin_start_inclusive
            - spec.inner_validation_embargo_sessions
            or self.training_origin_stop_exclusive
            ...
            - self.inner_validation_origin_stop_exclusive
            != M03R_V15_SELECTED_HORIZON_SESSIONS + 1
            or self.inner_validation_target_stop_exclusive
            != self.training_target_stop_exclusive
            or self.inner_validation_episode_state_stop_exclusive
            != self.inner_validation_target_stop_exclusive
            or self.inner_validation_episode_state_start
            != self.inner_validation_episode_state_stop_exclusive
            - spec.episode_state_rows
            or self.inner_validation_origin_start_inclusive
            - self.inner_validation_episode_state_start
            < M03R_V15_MINIMUM_LOCAL_ORIGIN
            or self.inner_validation_origin_stop_exclusive
            - 1
            - self.inner_validation_episode_state_start
            ...
            or self.geometry_rule != M03R_V15_FOLD_GEOMETRY_RULE
            or self.protocol_sha256 != M03R_V15_PROTOCOL_SHA256
            or self.schema != M03R_V15_FOLD_SCHEMA
        ):
            raise M03RV15FoldError("v15 fold geometry drifted")
        if self.fold_index == spec.chronological_fold_count - 1 and (
            self.qualification_target_stop_exclusive != self.cache_state_rows
        ):
            raise M03RV15FoldError("v15 final fold does not consume the cache boundary")
    ...
    def training_block_count(self) -> int:
        return math.ceil(
            ...
            / M03R_V15_PREDICTIVE_SPEC.origins_per_update
        )

    @property
    def optimizer_updates(self) -> int:
        return self.training_block_count * M03R_V15_PREDICTIVE_SPEC.training_epochs
...
def render_m03r_v15_fold_geometries(
    state_rows: int,
) -> tuple[M03RV15FoldGeometry, ...]:
    if state_rows != M03R_V15_REQUIRED_STATE_ROWS:
        raise M03RV15FoldError("v15 requires the exact 1001-state pre-2026 cache")
    spec = M03R_V15_PREDICTIVE_SPEC
    rows: list[M03RV15FoldGeometry] = []
    for fold_index in range(spec.chronological_fold_count):
        qualification_start = (
            M03R_V15_FIRST_QUALIFICATION_START
            + M03R_V15_FOLD_ADVANCE * fold_index
        )
        qualification_stop = qualification_start + spec.qualification_origins_per_fold
        target_stop = (
            qualification_stop + M03R_V15_SELECTED_HORIZON_SESSIONS + 1
        )
        training_stop = qualification_start - spec.purge_sessions
        inner_validation_stop = (
            training_stop - M03R_V15_SELECTED_HORIZON_SESSIONS - 1
        )
        inner_validation_start = (
            inner_validation_stop - spec.inner_validation_origins_per_fold
        )
        optimizer_origin_stop = (
            inner_validation_start - spec.inner_validation_embargo_sessions
        )
        row = M03RV15FoldGeometry(
            fold_index=fold_index,
            cache_state_rows=state_rows,
            training_state_start=0,
            training_target_stop_exclusive=training_stop,
            training_origin_start_inclusive=M03R_V15_MINIMUM_LOCAL_ORIGIN,
            training_origin_stop_exclusive=optimizer_origin_stop,
            inner_validation_origin_start_inclusive=inner_validation_start,
            inner_validation_origin_stop_exclusive=inner_validation_stop,
            inner_validation_target_stop_exclusive=training_stop,
            inner_validation_episode_state_start=(
                training_stop - spec.episode_state_rows
            ),
            inner_validation_episode_state_stop_exclusive=training_stop,
...
@dataclass(frozen=True, slots=True)
class M03RV15PanelEpisodeSchedule:
    protocol_common_data_sha256: str
    cache_sha256: str
    asset_axis_sha256: str
    fold_geometry_sha256: tuple[str, ...]
    seed: int = M03R_V15_PREDICTIVE_SPEC.seed
    training_epochs: int = M03R_V15_PREDICTIVE_SPEC.training_epochs
    origins_per_update: int = M03R_V15_PREDICTIVE_SPEC.origins_per_update
    schedule_rule: str = M03R_V15_EPISODE_SCHEDULE_RULE
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_PANEL_SCHEDULE_SCHEMA

    def validate(self) -> None:
        geometries = render_m03r_v15_fold_geometries(
            M03R_V15_REQUIRED_STATE_ROWS
        )
        if (
            ...
            or self.training_epochs != M03R_V15_PREDICTIVE_SPEC.training_epochs
            or self.origins_per_update != M03R_V15_PREDICTIVE_SPEC.origins_per_update
            or self.schedule_rule != M03R_V15_EPISODE_SCHEDULE_RULE
            or self.protocol_sha256 != M03R_V15_PROTOCOL_SHA256
            or self.schema != M03R_V15_PANEL_SCHEDULE_SCHEMA
        ):
            raise M03RV15FoldError("v15 panel episode schedule drifted")
        for name, value in (
...
def _epoch_block_order(
    ...
    *,
    ...
    if epoch_index not in range(M03R_V15_PREDICTIVE_SPEC.training_epochs):
        raise M03RV15FoldError("v15 epoch cursor drifted")
    return tuple(
        sorted(
            ...
            key=lambda block_index: hashlib.sha256(
                (
                    ...
                    f"{epoch_index}:{block_index}:paired-v15"
                ).encode("ascii")
            ).digest(),
        )
    )
...
@dataclass(frozen=True, slots=True)
class M03RV15TrainingUpdatePlan:
    setting_index: int
    fold_index: int
    ...
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_TRAINING_UPDATE_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            self.setting_index not in range(len(M03R_V15_SETTINGS))
            or geometry is None
            ...
            or self.episode_stop_exclusive
            ...
            > M03R_V15_MAXIMUM_LOCAL_TRAINING_ORIGIN
            or any(
                origin + M03R_V15_SELECTED_HORIZON_SESSIONS + 1
                > geometry.inner_validation_origin_start_inclusive
            ...
            or self.protocol_sha256 != M03R_V15_PROTOCOL_SHA256
            or self.schema != M03R_V15_TRAINING_UPDATE_SCHEMA
        ):
            raise M03RV15FoldError("v15 training update plan drifted")
...
@dataclass(frozen=True, slots=True)
class M03RV15PairedInputBinding:
    fold_index: int
    completed_update: int
    epoch_index: int
    epoch_block_index: int
    ...
    asset_axis_sha256: str
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_PAIRED_INPUT_BINDING_SCHEMA
    ...
    def validate(self) -> None:
        if (
            ...
            or self.rank_origins != _complementary_rank_shards(self.global_origins)
            or self.protocol_sha256 != M03R_V15_PROTOCOL_SHA256
            or self.schema != M03R_V15_PAIRED_INPUT_BINDING_SCHEMA
        ):
            raise M03RV15FoldError("v15 paired input binding drifted")
        for name, value in (
...
def build_m03r_v15_paired_input_binding(
    ...
    *,
    ...
    result = M03RV15PairedInputBinding(
...
def render_m03r_v15_training_update_plan(
    schedule: M03RV15PanelEpisodeSchedule,
    geometry: M03RV15FoldGeometry,
    *,
    setting_index: int,
    ...
    geometry.validate()
    if (
        setting_index not in range(len(M03R_V15_SETTINGS))
        ...
        raise M03RV15FoldError("v15 update request drifted")
    ...
    if not origins:
        raise M03RV15FoldError("v15 update selected an empty origin block")
    episode_start = max(
        0,
        origins[-1] - M03R_V15_MAXIMUM_LOCAL_TRAINING_ORIGIN,
    )
    result = M03RV15TrainingUpdatePlan(
        ...
        episode_stop_exclusive=(
            episode_start + M03R_V15_PREDICTIVE_SPEC.episode_state_rows
        ),
...
__all__ = [
    "M03R_V15_FIRST_QUALIFICATION_START",
    "M03R_V15_FOLD_ADVANCE",
    "M03R_V15_FOLD_SCHEMA",
    "M03R_V15_MAXIMUM_LOCAL_TRAINING_ORIGIN",
    "M03R_V15_MINIMUM_LOCAL_ORIGIN",
    "M03R_V15_PANEL_SCHEDULE_SCHEMA",
    "M03R_V15_PAIRED_INPUT_BINDING_SCHEMA",
    ...
    "M03RV15PairedInputBinding",
    "M03RV15TrainingUpdatePlan",
    "build_m03r_v15_paired_input_binding",
