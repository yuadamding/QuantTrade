"""Receipt-gated same-image zero-GPU validation for M03R-v15.
...
from rl_quant.training.top2000_m03r_v15_kubernetes import M03RV15RenderedJob
from rl_quant.training.top2000_m03r_v15_static_contract import (
    M03R_V15_STATIC_RESULT_SCHEMA,
)

M03R_V15_STATIC_GATE_SCHEMA = "rl-quant.top2000-dev.m03r-v15-static-gate-v1"
M03R_V15_STATIC_ATTACH_CONFIG_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v15-static-attach-config-v1"
)
...
class M03RV15StaticGateError(RuntimeError):
    """The static admission, Pod, or application evidence drifted."""


class M03RV15StaticAttachOnlyKubectl(common.AttachOnlyKubectl):
...
def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise M03RV15StaticGateError(f"{label} must be an object")
...
def _identity(job: Mapping[str, Any]) -> tuple[str, str, str, str]:
    ...
    if not all(isinstance(value, str) and value for value in values):
        raise M03RV15StaticGateError("Job identity is incomplete")
...
def _resources(container: Mapping[str, Any]) -> dict[str, str]:
    ...
    if set(resources) != {"requests", "limits"}:
        raise M03RV15StaticGateError("static resource sides drifted")
    ...
    for side in ("requests", "limits"):
        ...
        if set(values) != names:
            raise M03RV15StaticGateError("static resource-key inventory drifted")
        for name in names:
    ...
    if (
        ...
        raise M03RV15StaticGateError("static GPU request and limit must both be zero")
    return result
...
def _validate_static_pod_spec(
    ...
    *,
    ...
    if (
        ...
        raise M03RV15StaticGateError("static surface must contain one container")
    ...
    if set(container) != allowed_container_keys:
        raise M03RV15StaticGateError("static container gained an executable surface")
    for key in allowed_container_keys - {"resources", "env"}:
        if container.get(key) != desired_container.get(key):
            raise M03RV15StaticGateError(
    ...
    if (
        ...
        raise M03RV15StaticGateError("static NVIDIA visibility mask drifted")
    ...
    if set(pod_spec) - allowed_keys:
        raise M03RV15StaticGateError("static Pod gained an unallowlisted field")
    for key in base_keys - {"containers"}:
        if pod_spec.get(key) != desired_pod_spec.get(key):
            raise M03RV15StaticGateError(f"static protected Pod field drifted: {key}")
    placement = (
    ...
    if not (neutral or dry or created):
        raise M03RV15StaticGateError("static placement/CPU/memory profile drifted")
    if (
        profile["requests.ephemeral-storage"] != "1Gi"
        or profile["limits.ephemeral-storage"] != "4Gi"
    ):
        raise M03RV15StaticGateError("static ephemeral-storage profile drifted")
    ...
    if actual:
        if tolerations not in ([], standard):
            raise M03RV15StaticGateError("actual static tolerations drifted")
        if (
            ...
            raise M03RV15StaticGateError("actual static scheduling defaults drifted")
    elif tolerations:
        raise M03RV15StaticGateError("pre-Pod static tolerations must be empty")
    return {
        "resources": profile,
        "gpu_requests": 0,
        "gpu_limits": 0,
...
def bind_m03r_v15_static_admitted_suspended_job(
    *,
    rendered: M03RV15RenderedJob,
    ...
    if rendered.mode != "static" or rendered.gpus_per_completion != 0:
        raise M03RV15StaticGateError("static binder received a non-static Job")
    ...
    if first_identity[:3] != second_identity[:3] or attached_owned_pod_uids:
        raise M03RV15StaticGateError("static Job identity changed or already owns Pods")
    ...
    if (
        ...
        raise M03RV15StaticGateError("static Job metadata drifted")
    ...
    if (
        ...
        raise M03RV15StaticGateError("static template metadata drifted")
    ...
    if not isinstance(run_id, str) or not run_id:
        raise M03RV15StaticGateError("static run ID is absent")
    fields = {
        "job_name": job_name,
        "namespace": namespace,
        "job_uid": job_uid,
...
def validate_m03r_v15_static_actual_pod(
    *,
    pod: Mapping[str, Any],
    ...
    if (
        ...
        raise M03RV15StaticGateError("actual static Pod ownership drifted")
    desired_metadata = _mapping(rendered.manifest.get("metadata"), "desired metadata")
    expected_labels = {
    ...
    if metadata.get("labels") != expected_labels:
        raise M03RV15StaticGateError("actual static Pod labels drifted")
    ...
    if (
        ...
        raise M03RV15StaticGateError("actual static Pod annotations drifted")
    ...
    if actual_spec.get("hostname") != f"{job_name}-0":
        raise M03RV15StaticGateError("actual static indexed hostname drifted")
    ...
    if any(
        ...
        raise M03RV15StaticGateError("actual static container differs from the Job")
    if actual_spec.get("volumes") != created_spec.get("volumes"):
        raise M03RV15StaticGateError("actual static volumes differ from the Job")
    status = _mapping(pod.get("status"), "actual Pod status")
    statuses = status.get("containerStatuses")
    if not isinstance(statuses, list) or len(statuses) != 1:
        raise M03RV15StaticGateError("actual static container status is absent")
    ...
    if (
        ...
        raise M03RV15StaticGateError("actual static image or exit status drifted")
    return {
        "pod_name": pod_name,
        "pod_uid": pod_uid,
        "job_name": job_name,
...
def validate_m03r_v15_static_log(
    log: bytes, *, rendered: M03RV15RenderedJob
) -> dict[str, Any]:
    if len(log) > 32768:
        raise M03RV15StaticGateError("static log exceeds the bounded contract")
    try:
        ...
        raise M03RV15StaticGateError("static log is not one canonical result") from exc
    annotations = _mapping(rendered.manifest.get("metadata"), "desired metadata").get(
        "annotations", {}
    )
    if (
        not isinstance(value, dict)
        or value.get("schema") != M03R_V15_STATIC_RESULT_SCHEMA
        ...
        raise M03RV15StaticGateError("static application result drifted")
    return value


@dataclass(frozen=True, slots=True)
class M03RV15StaticGateReceipt:
    ...
    schema: str = M03R_V15_STATIC_GATE_SCHEMA


@dataclass(frozen=True, slots=True)
class M03RV15StaticAttachConfig:
    job_name: str
    run_id: str
    job_uid: str
    rendered_path: str
    ...
    schema: str = M03R_V15_STATIC_ATTACH_CONFIG_SCHEMA
    ...
    def __post_init__(self) -> None:
        for name in (
            ...
            if re.fullmatch(r"[0-9a-f]{64}", cast(str, getattr(self, name))) is None:
                raise M03RV15StaticGateError(f"{name} must be a SHA-256")
        ...
        for name in (
            ...
            if not value.is_absolute() or not value.is_relative_to(root):
                raise M03RV15StaticGateError(f"{name} leaves the project root")
        if (
            self.schema != M03R_V15_STATIC_ATTACH_CONFIG_SCHEMA
            ...
            raise M03RV15StaticGateError("static attach config drifted")
...
) -> M03RV15StaticAttachConfig:
    if _file_sha(path) != expected_file_sha256:
        raise M03RV15StaticGateError("static config file hash drifted")
    try:
        return M03RV15StaticAttachConfig(**json.loads(path.read_bytes()))
    except (TypeError, ValueError, json.JSONDecodeError) as exc:
        raise M03RV15StaticGateError("static attach config is invalid") from exc


def _load_static_rendered(config: M03RV15StaticAttachConfig) -> M03RV15RenderedJob:
    ...
    if _file_sha(path) != config.rendered_file_sha256:
        raise M03RV15StaticGateError("static rendered file hash drifted")
    try:
        rendered = M03RV15RenderedJob(**json.loads(path.read_bytes()))
    except (TypeError, ValueError, json.JSONDecodeError) as exc:
        raise M03RV15StaticGateError("static rendered receipt is invalid") from exc
    ...
    if (
        ...
        raise M03RV15StaticGateError("static rendered identity drifted")
    return rendered


def _validate_static_job_identity(
    job: Mapping[str, Any], config: M03RV15StaticAttachConfig
) -> None:
    ...
    if (
        ...
        raise M03RV15StaticGateError("static Job identity drifted")


def run_m03r_v15_static_gate(
    ...
    *,
    ...
    if _file_sha(source) != config.static_source_sha256:
        raise M03RV15StaticGateError("static gate source hash drifted")
    ...
    if (
        ...
        raise M03RV15StaticGateError("static package authorization drifted")
    ...
    if (
        ...
        raise M03RV15StaticGateError("static binding/activation identity drifted")
    root = common._directory_no_symlink(
        Path(config.evidence_root), label="static evidence root"
    )
    if any(root.iterdir()):
        raise M03RV15StaticGateError("static evidence root must start empty")
    ...
    if _file_sha(dry_path) != config.server_dry_run_file_sha256:
        raise M03RV15StaticGateError("static server dry-run hash drifted")
    live = transport or M03RV15StaticAttachOnlyKubectl(
    ...
    try:
        ...
        common._exclusive_json(
            ...
            {
                "schema": "rl-quant.top2000-dev.m03r-v15-static-preactivation-v1",
        ...
        if (
            ...
            raise M03RV15StaticGateError("static activation did not unsuspend")
        activated = True
        common._exclusive_json(
            root / "activation.json",
            {
                "schema": "rl-quant.top2000-dev.m03r-v15-static-activation-v1",
                "activation_request_sha256": activation.request_sha256,
        ...
        while monotonic() < hard_deadline:
            job = live.get_job()
            if job is None:
                raise M03RV15StaticGateError("static Job disappeared before terminal")
            _validate_static_job_identity(job, config)
            pods = live.get_owned_pods()
        ...
        if terminal_job is None or len(terminal_pods) != 1:
            raise M03RV15StaticGateError("static Job did not complete before hard wall")
        ...
        pod = terminal_pods[0]
        proof = validate_m03r_v15_static_actual_pod(
            pod=pod,
        ...
        validate_m03r_v15_static_log(raw_log, rendered=rendered)
        ...
        cleaned = True
        receipt = M03RV15StaticGateReceipt(
            ...
            try:
                ...
                if not (root / "static-cleanup-attach-required.json").exists():
                    common._exclusive_json(
                        ...
                        {
                            "schema": "rl-quant.top2000-dev.m03r-v15-static-cleanup-attach-required-v1",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    receipt = run_m03r_v15_static_gate(args.config, args.config_sha256)
    print(
        json.dumps(
...
__all__ = [
    "M03R_V15_STATIC_ATTACH_CONFIG_SCHEMA",
    "M03R_V15_STATIC_GATE_SCHEMA",
    "M03RV15StaticAttachConfig",
    "M03RV15StaticGateError",
    "M03RV15StaticGateReceipt",
    "bind_m03r_v15_static_admitted_suspended_job",
    "main",
    "run_m03r_v15_static_gate",
    "validate_m03r_v15_static_actual_pod",
    "validate_m03r_v15_static_log",
]
