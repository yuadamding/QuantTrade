"""Real-cache structural qualification for every M03R-v13 scheduled origin."""
...
M03R_V13_STRUCTURAL_PREFLIGHT_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v13-real-cache-structural-preflight-v1"
)
M03R_V13_STRUCTURAL_PREFLIGHT_FILE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v13-real-cache-structural-preflight-file-v1"
)
_MAX_PREFLIGHT_BYTES = 4 * 1024**2


class M03RV13PreflightError(ValueError):
    """The exact data-dependent v13 structural gate failed or drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV13PreflightError(f"{name} must be a lowercase SHA-256")
    return value


def _scheduled_origins() -> tuple[int, ...]:
    origins: set[int] = set()
    for geometry in render_m03r_v13_fold_geometries(1001):
...
class M03RV13StructuralPreflightReceipt:
    cache_sha256: str
    ...
    schema: str = M03R_V13_STRUCTURAL_PREFLIGHT_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.schema != M03R_V13_STRUCTURAL_PREFLIGHT_SCHEMA
        ):
            raise M03RV13PreflightError("v13 structural preflight receipt drifted")
        for name, value in (
...
def run_m03r_v13_structural_preflight(
    ...
    if (
        ...
        raise M03RV13PreflightError(
            "v13 real cache, risk source, or asset axis drifted"
        )
    ...
    for origin in _scheduled_origins():
        ...
        target = build_m03r_v11_residual_operator(
        ...
        action = build_m03r_v11_residual_operator(
    ...
    geometries = render_m03r_v13_fold_geometries(1001)
    origins = _scheduled_origins()
    result = M03RV13StructuralPreflightReceipt(
...
def write_m03r_v13_structural_preflight(
    path: str | Path,
    receipt: M03RV13StructuralPreflightReceipt,
) -> str:
    receipt.validate()
    target = Path(path)
    if target.exists() or target.is_symlink():
        raise M03RV13PreflightError("v13 preflight target already exists")
    encoded = (
        json.dumps(
            {
                "schema": M03R_V13_STRUCTURAL_PREFLIGHT_FILE_SCHEMA,
...
def load_m03r_v13_structural_preflight(
    path: str | Path,
    *,
    ...
    try:
        ...
        raise M03RV13PreflightError("v13 preflight file is unavailable") from exc
    try:
        ...
        if (
            ...
            raise M03RV13PreflightError("v13 preflight file type or size drifted")
        ...
        if (
            ...
            raise M03RV13PreflightError("v13 preflight file changed while read")
    finally:
        os.close(descriptor)
    if len(raw) != before.st_size or hashlib.sha256(raw).hexdigest() != expected_file_sha256:
        raise M03RV13PreflightError("v13 preflight file hash drifted")
    try:
        payload = json.loads(raw)
        row = dict(payload["receipt"])
        row["fold_geometry_sha256"] = tuple(row["fold_geometry_sha256"])
        receipt = M03RV13StructuralPreflightReceipt(**row)
    except (KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
        raise M03RV13PreflightError("v13 preflight file is malformed") from exc
    if (
        payload.get("schema") != M03R_V13_STRUCTURAL_PREFLIGHT_FILE_SCHEMA
        or payload.get("receipt_sha256") != expected_receipt_sha256
        or receipt.receipt_sha256 != expected_receipt_sha256
    ):
        raise M03RV13PreflightError("v13 preflight receipt drifted")
    return receipt
...
__all__ = [
    "M03R_V13_STRUCTURAL_PREFLIGHT_FILE_SCHEMA",
    "M03R_V13_STRUCTURAL_PREFLIGHT_SCHEMA",
    "M03RV13PreflightError",
    "M03RV13StructuralPreflightReceipt",
    "load_m03r_v13_structural_preflight",
    "run_m03r_v13_structural_preflight",
    "write_m03r_v13_structural_preflight",
]
