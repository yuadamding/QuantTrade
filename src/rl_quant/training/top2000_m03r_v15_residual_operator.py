"""Stable reusable exposure-null operators for M03R-v15 executable scores."""
...
M03R_V15_RESIDUAL_OPERATOR_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-qr-map-residual-operator-v1"
)
M03R_V15_RESIDUAL_RESULT_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-qr-map-residual-result-v1"
)


class M03RV15ResidualOperatorError(ValueError):
    """The reusable v15 residual operator is invalid or drifted."""
...
class M03RV15ResidualOperator:
    ...
    operator_rule: str = M03R_V15_EXECUTABLE_SCORE_RULE
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_RESIDUAL_OPERATOR_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.operator_rule != M03R_V15_EXECUTABLE_SCORE_RULE
            or self.protocol_sha256 != M03R_V15_PROTOCOL_SHA256
            or self.schema != M03R_V15_RESIDUAL_OPERATOR_SCHEMA
        ):
            raise M03RV15ResidualOperatorError("v15 residual operator drifted")
        ...
        if not torch.allclose(identity, expected, rtol=0.0, atol=2.0e-10):
            raise M03RV15ResidualOperatorError(
                "v15 QR coefficient map does not reproduce the design identity"
            )
        if self.receipt_sha256 != _operator_receipt_sha256(
            ...
            raise M03RV15ResidualOperatorError("v15 residual receipt drifted")
    ...
    def require_fast_identity(self) -> None:
        if (
            ...
            raise M03RV15ResidualOperatorError("v15 residual tensor identity changed")
...
def _operator_receipt_sha256(
    *,
    ...
    return _sha256(
        {
            "schema": M03R_V15_RESIDUAL_OPERATOR_SCHEMA,
            "protocol_sha256": M03R_V15_PROTOCOL_SHA256,
            "operator_rule": M03R_V15_EXECUTABLE_SCORE_RULE,
...
def build_m03r_v15_residual_operator(
    *,
    ...
    result = M03RV15ResidualOperator(
        base=base,
...
@dataclass(frozen=True, slots=True)
class M03RV15ResidualResult:
    residual: torch.Tensor
    qualified_asset_mask: torch.Tensor
    operator_receipt_sha256: str
    weighted_exposure_error: float
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_RESIDUAL_RESULT_SCHEMA

    def validate(self, operator: M03RV15ResidualOperator) -> None:
        ...
        if (
            ...
            or self.protocol_sha256 != M03R_V15_PROTOCOL_SHA256
            or self.schema != M03R_V15_RESIDUAL_RESULT_SCHEMA
        ):
            raise M03RV15ResidualOperatorError("v15 residual result drifted")


def apply_m03r_v15_residual_operator(
    value: torch.Tensor,
    ...
    if (
        ...
        raise M03RV15ResidualOperatorError("v15 residual value is malformed")
