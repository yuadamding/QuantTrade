"""Attach-only Seadragon lifecycle for the M03R-v14 predictive panel.
...
from rl_quant.training.top2000_m03r_v14_kubernetes import (
    M03RV14TwoH100CapacityQualification,
)
...
ATTACH_CONFIG_SCHEMA: Final = "rl-quant.top2000-dev.m03r-v14-seadragon-attach-config-v1"
COMPLETION_COVERAGE_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v14-predictive-coverage-v1"
)
_SHA256_RE = re.compile(r"^[0-9a-f]{64}$")
CAPACITY_CONFIG_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v14-capacity-attach-config-v1"
)
M03R_V14_STARTUP_SCHEMA: Final = "rl-quant.top2000-dev.m03r-v14-two-h100-startup-v1"
M03R_V14_FOLD_TERMINAL_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v14-predictive-fold-terminal-v1"
)
M03R_V14_WORKER_TERMINAL_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v14-predictive-worker-terminal-v1"
)
M03R_V14_CAPACITY_TERMINAL_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v14-two-h100-capacity-terminal-v1"
)
M03R_V14_BOOTSTRAP_PLAN_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v14-bootstrap-plan-v1"
)
M03R_V14_QUALIFICATION_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v14-qualification-v1"
)
M03R_V14_EXECUTION_HORIZONS: Final = (M03R_V14_SELECTED_HORIZON_SESSIONS,)
M03R_V14_EXECUTION_HORIZON_KEYS: Final = frozenset(
    str(value) for value in M03R_V14_EXECUTION_HORIZONS
)


class M03RV14SeadragonLifecycleError(RuntimeError):
    """The exact v14 attachment or its evidence failed closed."""


class M03RV14ActivationAttachRequired(M03RV14SeadragonLifecycleError):
    """Activation outcome is ambiguous and exact state must be retained."""


@dataclass(frozen=True, slots=True)
class M03RV14CapacityAttachConfig:
    job_name: str
    run_id: str
    job_uid: str
    binding_path: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise M03RV14SeadragonLifecycleError(
                "v14 capacity config drifted from the one-worker 2-H100 contract"
            )
        for name in (
            "binding_path",
        ...
        if not Path(self.host_python_path).is_absolute():
            raise M03RV14SeadragonLifecycleError("host_python_path must be absolute")
...
def _canonical_bytes(value: Any) -> bytes:
    try:
        ...
        raise M03RV14SeadragonLifecycleError(
            "v14 lifecycle evidence is not canonical-JSON safe"
        ) from exc
...
def _require_sha256(name: str, value: str) -> None:
    if _SHA256_RE.fullmatch(value) is None:
        raise M03RV14SeadragonLifecycleError(f"{name} must be a lowercase SHA-256")


def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise M03RV14SeadragonLifecycleError(f"{label} must be an object")
...
def _receipt_payload(value: Mapping[str, Any], label: str) -> Mapping[str, Any]:
    ...
    if not isinstance(receipt, str):
        raise M03RV14SeadragonLifecycleError(f"{label} receipt hash is absent")
    _require_sha256(f"{label} receipt", receipt)
    unsigned = dict(value)
    del unsigned["receipt_sha256"]
    if receipt != _content_sha256(unsigned):
        raise M03RV14SeadragonLifecycleError(f"{label} receipt hash drifted")
    return unsigned
...
def _project_path(path: str, label: str) -> Path:
    ...
    if not value.is_absolute():
        raise M03RV14SeadragonLifecycleError(f"{label} must be absolute")
    try:
...
@dataclass(frozen=True, slots=True)
class M03RV14ExpectedCompletion:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise M03RV14SeadragonLifecycleError(
                "v14 completion identity must be the frozen direct 0..1 map"
            )


@dataclass(frozen=True, slots=True)
class M03RV14AttachSupervisorConfig:
    job_name: str
    run_id: str
    job_uid: str
    binding_path: str
    ...
    lifecycle_source_sha256: str
    expected_completions: tuple[M03RV14ExpectedCompletion, ...]
    host_python_path: str
    pythonpath: str
    kubectl_path: str = SEADRAGON_KUBECTL
    kubeconfig_path: str = SEADRAGON_KUBECONFIG
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise M03RV14SeadragonLifecycleError(
                "v14 attach config drifted from the two-setting 2-H100 contract"
            )
        for name in (
            "binding_path",
            "activation_request_path",
...
def _load_config(path: Path, expected_sha256: str) -> M03RV14AttachSupervisorConfig:
    ...
    values["expected_completions"] = tuple(
        M03RV14ExpectedCompletion(**dict(_mapping(row, "expected completion")))
        for row in cast(Sequence[Any], values.get("expected_completions", ()))
    )
    try:
        return M03RV14AttachSupervisorConfig(**values)
    except (TypeError, ValueError) as exc:
        raise M03RV14SeadragonLifecycleError("v14 attach config is invalid") from exc


def _load_capacity_config(
    path: Path, expected_sha256: str
) -> M03RV14CapacityAttachConfig:
    _require_sha256("capacity config file", expected_sha256)
    payload = _mapping(
        common._read_json_file(path, expected_sha256=expected_sha256),
        "v14 capacity attach config",
    )
    try:
        return M03RV14CapacityAttachConfig(**dict(payload))
    except (TypeError, ValueError) as exc:
        raise M03RV14SeadragonLifecycleError(
            "v14 capacity attach config is invalid"
        ) from exc


def _load_package_authorization(
    config: M03RV14AttachSupervisorConfig | M03RV14CapacityAttachConfig,
) -> tuple[Any, Any]:
    ...
    if package.package_plan_sha256 != config.package_plan_sha256:
        raise M03RV14SeadragonLifecycleError("package-plan identity drifted")
    ...
    if authorization.receipt_sha256 != config.execution_authorization_receipt_sha256:
        raise M03RV14SeadragonLifecycleError("execution-authorization identity drifted")
...
def _host_output_path(config: M03RV14AttachSupervisorConfig, value: Any) -> Path:
    if not isinstance(value, str):
        raise M03RV14SeadragonLifecycleError("worker artifact path is absent")
    ...
    if not relative.parts or any(part in {"", ".", ".."} for part in relative.parts):
        raise M03RV14SeadragonLifecycleError("worker artifact path is unsafe")
    return Path(config.output_root).joinpath(*relative.parts)


def _validate_rank_runtime(value: Any) -> None:
    if not isinstance(value, list) or len(value) != 2:
        raise M03RV14SeadragonLifecycleError("startup must contain two rank rows")
    rows = sorted(
...
def _validate_capacity_output(
    config: M03RV14CapacityAttachConfig,
    ...
    if (
        startup.get("schema") != M03R_V14_STARTUP_SCHEMA
        ...
        or startup.get("setting_id") != M03R_V14_SETTING_IDS[0]
        ...
        or terminal.get("schema") != M03R_V14_CAPACITY_TERMINAL_SCHEMA
        ...
        or terminal.get("setting_id") != M03R_V14_SETTING_IDS[0]
...
def _read_bound_file(path: Path, expected_sha256: str, label: str) -> bytes:
    ...
    if hashlib.sha256(content).hexdigest() != expected_sha256:
        raise M03RV14SeadragonLifecycleError(f"{label} file hash drifted")
    return content
...
def _read_bound_json(path: Path, expected_sha256: str, label: str) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise M03RV14SeadragonLifecycleError(f"{label} is invalid JSON") from exc
...
def _validate_static_gate_lineage(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "rl-quant.top2000-dev.m03r-v14-static-gate-v1"
        ...
        raise M03RV14SeadragonLifecycleError("static gate lineage drifted")
    for name in digest_fields:
        digest = value.get(name)
        if not isinstance(digest, str):
            raise M03RV14SeadragonLifecycleError(f"static gate {name} is not a digest")
...
def _validate_qualification(
    ...
    *,
    ...
    if (
        set(value) != expected_keys
        or value.get("schema") != M03R_V14_QUALIFICATION_SCHEMA
        or value.get("protocol_sha256") != M03R_V14_PROTOCOL_SHA256
        or value.get("setting_index") != setting_index
        or value.get("setting_id") != M03R_V14_SETTING_IDS[setting_index]
...
def _validate_host_bootstrap_plan(value: Mapping[str, Any]) -> str:
    ...
    if (
        ...
        or value.get("bootstrap_seed") != M03R_V14_PREDICTIVE_SPEC.bootstrap_seed
        or value.get("replicates") != M03R_V14_PREDICTIVE_SPEC.bootstrap_replicates
        or value.get("protocol_sha256") != M03R_V14_PROTOCOL_SHA256
        or value.get("schema") != M03R_V14_BOOTSTRAP_PLAN_SCHEMA
    ):
        raise M03RV14SeadragonLifecycleError("bootstrap plan is invalid")
    chronology = value.get("chronology_sha256")
    if not isinstance(chronology, str):
        raise M03RV14SeadragonLifecycleError("bootstrap chronology hash is absent")
...
def _validate_one_worker(
    config: M03RV14AttachSupervisorConfig,
    row: M03RV14ExpectedCompletion,
    ...
    if (
        ...
        or startup.get("reportable") is not False
        or startup.get("promotion_eligible") is not False
        or terminal.get("schema") != M03R_V14_WORKER_TERMINAL_SCHEMA
        ...
        raise M03RV14SeadragonLifecycleError("worker terminal identity drifted")
    fold_hashes = terminal.get("fold_terminal_file_sha256")
    if not isinstance(fold_hashes, list) or len(fold_hashes) != 6:
        raise M03RV14SeadragonLifecycleError("worker must bind six fold receipts")
    ...
    for fold_index, expected_sha in enumerate(fold_hashes):
        if not isinstance(expected_sha, str):
            raise M03RV14SeadragonLifecycleError("fold receipt hash is absent")
        ...
        if (
            ...
            raise M03RV14SeadragonLifecycleError("fold artifact identity is absent")
        ...
        for name in (
            ...
            value = fold.get(name)
            if not isinstance(value, str):
                raise M03RV14SeadragonLifecycleError(f"fold {name} is absent")
            _require_sha256(name, value)
        if (
            fold.get("schema") != M03R_V14_FOLD_TERMINAL_SCHEMA
            ...
            != M03R_V14_PREDICTIVE_SPEC.training_epochs
            ...
            raise M03RV14SeadragonLifecycleError("fold receipt semantics drifted")
    ...
    if terminal.get("bootstrap_plan_sha256") != bootstrap_sha256:
        raise M03RV14SeadragonLifecycleError("bootstrap plan identity drifted")
    ...
    if gate is True:
        if selected != M03R_V14_SELECTED_HORIZON_SESSIONS:
            raise M03RV14SeadragonLifecycleError("passed terminal omitted selection")
        if (
            qualification.get("passed") is not True
            or terminal.get("economic_generation_may_be_minted") is not True
        ):
            raise M03RV14SeadragonLifecycleError("selected qualification drifted")
    elif gate is False:
        if (
            ...
            raise M03RV14SeadragonLifecycleError("failed gate selection drifted")
    else:
        raise M03RV14SeadragonLifecycleError("predictive gate flag is invalid")
    return terminal_sha, {
        "setting_index": row.setting_index,
...
def validate_m03r_v14_predictive_coverage(
    config: M03RV14AttachSupervisorConfig,
    *,
    ...
    if len(owned_pods) != 2:
        raise M03RV14SeadragonLifecycleError(
            "complete v14 Job must retain exactly two owned Pods"
        )
    ...
    for pod in owned_pods:
        ...
        if (
            ...
            raise M03RV14SeadragonLifecycleError(
                "terminal Pods are not one success per v14 completion"
            )
        pod_inventory[index] = {"pod_name": name, "pod_uid": uid}
    if set(pod_inventory) != {0, 1}:
        raise M03RV14SeadragonLifecycleError("terminal Pod coverage drifted")
...
def _cleanup_preactivation_exact(
    *,
    ...
    common._exclusive_json(
        ...
        {
            "schema": ("rl-quant.top2000-dev.m03r-v14-preactivation-cleanup-reads-v1"),
            "binding_receipt_sha256": binding.receipt_sha256,
    ...
    if absence is None:
        common._exclusive_json(
            ...
            {
                "schema": (
                    "rl-quant.top2000-dev.m03r-v14-preactivation-cleanup-"
                    "attach-required-v1"
                ),
        ...
        raise M03RV14ActivationAttachRequired(
    ...
    verification_sha = common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v14-cleanup-verification-v1",
            "first": first,
            "second": second,
            "delete_attempted": True,
            "delete_retried": False,
    ...
    *,
    ...
    if absence is None:
        common._exclusive_json(
            ...
            {
                "schema": "rl-quant.top2000-dev.m03r-v14-cleanup-attach-required-v1",
                "delete_attempted": True,
                "delete_retried": False,
                "error_type": (
                    None if delete_error is None else type(delete_error).__name__
...
def _prepare_activation(
    *,
    ...
    try:
        fresh = live.get_job()
        if fresh is None:
            raise M03RV14SeadragonLifecycleError("bound suspended Job is absent")
        common._job_identity(
            fresh,
            job_name=config.job_name,
            run_id=config.run_id,
        ...
        if live.get_owned_pods():
            raise M03RV14SeadragonLifecycleError("attach target already owns Pods")
        ...
        readiness_sha = common._exclusive_json(
            root / "readiness.json",
            {
                "schema": "rl-quant.top2000-dev.m03r-v14-supervisor-readiness-v1",
        ...
        arm_sha = common._exclusive_json(
            root / "arm.json",
            {
                "schema": "rl-quant.top2000-dev.m03r-v14-supervisor-arm-v1",
...
def _prepare_capacity_activation(
    *,
    ...
    try:
        fresh = live.get_job()
        if fresh is None:
            raise M03RV14SeadragonLifecycleError("bound capacity Job is absent")
        common._job_identity(
            fresh,
            job_name=config.job_name,
            run_id=config.run_id,
        ...
        common._exclusive_json(
            root / "readiness.json",
            {
                "schema": "rl-quant.top2000-dev.m03r-v14-capacity-readiness-v1",
                "config_sha256": config_sha256,
                "binding_receipt_sha256": binding.receipt_sha256,
                "configured_activation_file_sha256": (
                    config.activation_request_file_sha256
...
def _run_supervisor_inner(
    ...
    *,
    ...
    if _file_sha256(source) != config.lifecycle_source_sha256:
        raise M03RV14SeadragonLifecycleError("lifecycle source hash drifted")
    ...
    if (
        ...
        raise M03RV14SeadragonLifecycleError(
            "v14 config, binding, or activation identity drifted"
        )
    ...
    activation_sha = common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v14-supervisor-activation-v1",
    ...
    common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v14-supervisor-launch-success-v1",
    ...
    if terminal_reason == "complete":
        coverage = validate_m03r_v14_predictive_coverage(
    ...
    if terminal_reason != "complete":
        raise M03RV14SeadragonLifecycleError(
            f"v14 predictive Job terminated without completion: {terminal_reason}"
        )
...
def _run_capacity_supervisor_inner(
    ...
    *,
    ...
    source = common._regular_no_symlink(Path(__file__), label="lifecycle source")
    if _file_sha256(source) != config.lifecycle_source_sha256:
        raise M03RV14SeadragonLifecycleError("capacity lifecycle source hash drifted")
    package, authorization = _load_package_authorization(config)
    ...
    arm_sha = common._exclusive_json(
        root / "arm.json",
        {
            "schema": "rl-quant.top2000-dev.m03r-v14-capacity-arm-v1",
            "config_sha256": config_sha256,
            "spawn_process_file_sha256": _file_sha256(process_path),
            "activation_request_sha256": activation.request_sha256,
        },
    ...
    common._exclusive_json(
        root / "activation.json",
        {
            "schema": "rl-quant.top2000-dev.m03r-v14-capacity-activation-v1",
            "arm_file_sha256": arm_sha,
            "activation_request_sha256": activation.request_sha256,
            "activated_job_sha256": common._content_sha256(activated),
            "activation_reconciled_after_transport_error": False,
    ...
    common._exclusive_json(
        root / "launch-success.json",
        {
            "schema": "rl-quant.top2000-dev.m03r-v14-capacity-launch-success-v1",
            "job_name": config.job_name,
            "job_uid": config.job_uid,
            "run_id": config.run_id,
            "parallelism": 1,
    ...
    if not isinstance(terminal_receipt, str):
        raise M03RV14SeadragonLifecycleError("capacity terminal receipt is absent")
    package, authorization = _load_package_authorization(config)
    qualification = M03RV14TwoH100CapacityQualification(
...
def _recover_predictive_supervisor_failure(
    *,
    ...
    """Capture and issue at most one cleanup delete after a v14 child error."""
    ...
    try:
    ...
    except M03RV14ActivationAttachRequired:
        raise
        ...
        if not attach_path.exists():
            common._exclusive_json(
                ...
                {
                    "schema": (
                        "rl-quant.top2000-dev.m03r-v14-supervisor-recovery-"
                        "attach-required-v1"
                    ),
    ...
    try:
        ...
        if not (root / "capacity-supervisor-error.json").exists():
            common._exclusive_json(
                ...
                {
                    "schema": "rl-quant.top2000-dev.m03r-v14-capacity-error-v1",
        ...
        try:
            ...
            common._exclusive_json(
                ...
                {
                    "schema": (
                        "rl-quant.top2000-dev.m03r-v14-capacity-recovery-"
                        "attach-required-v1"
                    ),
    ...
    try:
        ...
        if not error_path.exists():
            common._exclusive_json(
                error_path,
                {
                    "schema": "rl-quant.top2000-dev.m03r-v14-supervisor-error-v1",
                    "error_type": type(exc).__name__,
                    "error": str(exc),
                    "attach_required": False,
                },
...
def _validated_host_tool(path: Path, label: str) -> Path:
    ...
    if not os.access(path, os.X_OK):
        raise M03RV14SeadragonLifecycleError(f"{label} is not executable")
    return path
...
def _validated_launch_success(
    ...
    try:
        ...
        if (
            launch.get("schema")
            != "rl-quant.top2000-dev.m03r-v14-supervisor-launch-success-v1"
            ...
            != "rl-quant.top2000-dev.m03r-v14-supervisor-activation-v1"
            ...
            return None
        return launch
    except (OSError, ValueError, M03RV14SeadragonLifecycleError):
        return None
...
def _validated_capacity_launch_success(
    ...
    try:
        ...
        if (
            launch.get("schema")
            != "rl-quant.top2000-dev.m03r-v14-capacity-launch-success-v1"
            or launch.get("job_name") != config.job_name
            or launch.get("job_uid") != config.job_uid
            or launch.get("run_id") != config.run_id
            ...
            or launch.get("economic_panel_authorized") is not False
            or activation.get("schema")
            != "rl-quant.top2000-dev.m03r-v14-capacity-activation-v1"
            or activation.get("activation_reconciled_after_transport_error")
            is not False
            or activation.get("activation_retried") is not False
        ):
...
def spawn_capacity_supervisor(
    ...
    intent_sha = common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v14-capacity-spawn-intent-v1",
            "config_sha256": expected_config_sha256,
    ...
    process_receipt = {
        "schema": "rl-quant.top2000-dev.m03r-v14-capacity-process-v1",
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            common._exclusive_json(
                ...
                {
                    "schema": (
                        "rl-quant.top2000-dev.m03r-v14-capacity-spawn-attach-required-v1"
                    ),
                    "pid": process.pid,
...
def spawn_attach_supervisor(
    ...
    intent_sha = common._exclusive_json(
        root / "spawn-intent.json",
        {
            "schema": "rl-quant.top2000-dev.m03r-v14-supervisor-spawn-intent-v1",
            "config_sha256": expected_config_sha256,
            "command_sha256": command_sha,
            "python_sha256": _file_sha256(python),
            "kubectl_sha256": _file_sha256(kubectl),
    ...
    identity = common._process_identity(process.pid)
    process_receipt = {
        "schema": "rl-quant.top2000-dev.m03r-v14-supervisor-process-v1",
        "spawn_intent_file_sha256": intent_sha,
        "config_sha256": expected_config_sha256,
        "command_sha256": command_sha,
        **identity,
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            common._exclusive_json(
                ...
                {
                    "schema": (
                        "rl-quant.top2000-dev.m03r-v14-spawn-attach-required-v1"
                    ),
                    "pid": process.pid,
                    "process_receipt_file_sha256": _file_sha256(
                        root / "spawn-process.json"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except M03RV14ActivationAttachRequired as exc:
        print(
            json.dumps(
...
__all__ = [
    ...
    "M03RV14ActivationAttachRequired",
    "M03RV14AttachSupervisorConfig",
    "M03RV14CapacityAttachConfig",
    "M03RV14ExpectedCompletion",
    "M03RV14SeadragonLifecycleError",
    "run_attach_supervisor",
    "run_capacity_supervisor",
    "spawn_attach_supervisor",
    "spawn_capacity_supervisor",
    "validate_m03r_v14_predictive_coverage",
]
