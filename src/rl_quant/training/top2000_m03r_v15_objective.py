"""Direct h3 rank/economic objective for M03R-v15."""
...
M03R_V15_RANK_DIMENSIONLESS_RMS_FLOOR = 0.05


class M03RV15ObjectiveError(ValueError):
    """The v15 direct h3 objective inputs drifted."""


@dataclass(frozen=True, slots=True)
class M03RV15PredictiveBatch:
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise M03RV15ObjectiveError("v15 predictive batch drifted")


@dataclass(frozen=True, slots=True)
class M03RV15PredictiveLoss:
    total: torch.Tensor
...
def _rank_loss(
    ...
    for date_index in range(prediction.shape[0]):
        ...
        if setting.ranking_objective == "none" or int(mask.sum()) < 2:
            ...
            continue
        horizon_scale = 0.02 * math.sqrt(M03R_V15_SELECTED_HORIZON_SESSIONS)
        selected = prediction[date_index, mask] / horizon_scale
        centered = selected - selected.mean()
        prediction_scale = torch.sqrt(centered.square().mean()).clamp_min(
            M03R_V15_RANK_DIMENSIONLESS_RMS_FLOOR
        )
...
def m03r_v15_predictive_loss(
    batch: M03RV15PredictiveBatch,
) -> M03RV15PredictiveLoss:
    batch.validate()
    ...
    if not bool(torch.isfinite(total)):
        raise M03RV15ObjectiveError("v15 predictive loss is non-finite")
    return M03RV15PredictiveLoss(
        total=total,
        ranking=ranking,
