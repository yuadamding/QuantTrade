"""Suspended Seadragon Jobs for M03R-v15 predictive qualification only."""
...
M03R_V15_KUBERNETES_LIVE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-live-kubernetes-evidence-v1"
)
M03R_V15_CAPACITY_QUALIFICATION_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-two-h100-capacity-qualification-v1"
)
M03R_V15_RENDERED_JOB_SCHEMA = "rl-quant.top2000-dev.m03r-v15-rendered-job-v1"
M03R_V15_WORKER_ARGV_PREFIX = (
    ...
    "--nproc-per-node=2",
    "-m",
    "rl_quant.workflows.top2000_m03r_v15_predictive",
)
M03R_V15_STATIC_ARGV_PREFIX = (
    "/opt/conda/envs/quanttrade/bin/python",
    "-m",
    "rl_quant.workflows.top2000_m03r_v15_static_validate",
)


class M03RV15KubernetesError(ValueError):
    """The v15 manifest, live evidence, or capacity receipt drifted."""
...
def _digest(name: str, value: str) -> None:
    if len(value) != 64 or any(
        ...
        raise M03RV15KubernetesError(f"{name} must be a lowercase SHA-256")


@dataclass(frozen=True, slots=True)
class M03RV15LiveEvidence:
    observed_at_utc: str
    ...
    schema: str = M03R_V15_KUBERNETES_LIVE_SCHEMA
    ...
    def __post_init__(self) -> None:
        try:
            ...
            raise M03RV15KubernetesError("live timestamp must be ISO-8601") from exc
        if observed.tzinfo is None or observed.utcoffset() != UTC.utcoffset(observed):
            raise M03RV15KubernetesError("live timestamp must carry UTC timezone")
        if (
            self.schema != M03R_V15_KUBERNETES_LIVE_SCHEMA
            ...
            raise M03RV15KubernetesError("v15 live Kubernetes evidence is invalid")
    ...
    def allowed_parallelism(self) -> int:
        ...
        return min(2, remaining // 2)

    def require_fresh(self, *, now_utc: datetime, max_age_seconds: int = 300) -> None:
        if now_utc.tzinfo is None:
            raise M03RV15KubernetesError("now_utc must be timezone-aware")
        observed = datetime.fromisoformat(self.observed_at_utc)
        age = (now_utc.astimezone(UTC) - observed.astimezone(UTC)).total_seconds()
        if age < 0 or age > max_age_seconds:
            raise M03RV15KubernetesError("live Kubernetes evidence is stale")


def build_m03r_v15_live_evidence(
    *,
    observed_at_utc: str,
    rbac: M03RV7KubernetesRBACEvidence,
    protected_or_other_committed_h100_count: int,
    ...
    payload = {
        "schema": M03R_V15_KUBERNETES_LIVE_SCHEMA,
...
@dataclass(frozen=True, slots=True)
class M03RV15TwoH100CapacityQualification:
    ...
    disposable_update_step_receipt_sha256: str
    qualification_risk_state_sha256: str
    world_size: int = 2
    ...
    disposable_rank_states_equal_after_update: bool = True
    disposable_qualification_projection_performed: bool = True
    peak_cuda_memory_recorded: bool = True
    scientific_checkpoint_published: bool = False
    passed: bool = True
    research_only: bool = True
    development_only: bool = True
    promotion_eligible: bool = False
    schema: str = M03R_V15_CAPACITY_QUALIFICATION_SCHEMA
    ...
    def validate_for(
        ...
        if (
            ...
            or self.initial_parameter_architecture_sha256
            ...
            or not self.disposable_rank_states_equal_after_update
            or not self.disposable_qualification_projection_performed
            or not self.peak_cuda_memory_recorded
            or self.scientific_checkpoint_published
            or not self.passed
            or not self.research_only
            or not self.development_only
            or self.promotion_eligible
            or self.schema != M03R_V15_CAPACITY_QUALIFICATION_SCHEMA
        ):
            raise M03RV15KubernetesError(
                "capacity receipt does not bind this v15 package"
            )


@dataclass(frozen=True, slots=True)
class M03RV15RenderedJob:
    ...
    schema: str = M03R_V15_RENDERED_JOB_SCHEMA
    ...
    def validate(self) -> None:
        if (
            ...
            or self.package_plan_sha256
            != self.manifest["metadata"]["annotations"][
                "rl-quant/package-plan-sha256"
            ]
            or self.execution_authorization_receipt_sha256
            != self.manifest["metadata"]["annotations"][
                "rl-quant/execution-authorization-sha256"
            ]
            or self.mode not in {"static", "capacity", "predictive"}
            or self.completions != (2 if self.mode == "predictive" else 1)
            or self.parallelism not in ({1, 2} if self.mode == "predictive" else {1})
            or self.gpus_per_completion != (0 if self.mode == "static" else 2)
            or self.request_ceiling_h100 > 4
            or self.economic_panel_authorized
            or self.schema != M03R_V15_RENDERED_JOB_SCHEMA
        ):
            raise M03RV15KubernetesError("rendered v15 Job drifted")


def bind_m03r_v15_admitted_suspended_job(
    *,
    ...
    """Bind one stable suspended admission through the shared strict contract."""

    rendered.validate()
    if rendered.mode == "static":
        from rl_quant.training.top2000_m03r_v15_static_gate import (
            bind_m03r_v15_static_admitted_suspended_job,
        )

        try:
            return bind_m03r_v15_static_admitted_suspended_job(
                rendered=rendered,
            ...
            raise M03RV15KubernetesError(
                "v15 static admitted suspended Job failed the strict contract"
            ) from exc
    try:
        ...
        raise M03RV15KubernetesError(
            "v15 admitted suspended Job failed the shared strict contract"
        ) from exc
...
def _render(
    *,
    ...
    live: M03RV15LiveEvidence,
    template: M03RV7KubernetesTemplateConfig,
    now_utc: datetime,
    mode: Literal["static", "capacity", "predictive"],
    capacity: M03RV15TwoH100CapacityQualification | None,
) -> M03RV15RenderedJob:
    package.validate()
    authorization.validate(package)
    ...
    if (
        ...
        raise M03RV15KubernetesError("package and PVC mount identities drifted")
    completions = 2 if mode == "predictive" else 1
    if mode != "static" and live.allowed_parallelism <= 0:
        raise M03RV15KubernetesError("live H100 cap does not allow one worker")
    parallelism = 1 if mode != "predictive" else min(2, live.allowed_parallelism)
    gpus = 0 if mode == "static" else 2
    if parallelism * gpus > authorization.maximum_h100_requests:
        raise M03RV15KubernetesError("predictive H100 request ceiling drifted")
    labels = {
        "app.kubernetes.io/name": (
            "quanttrade-m03r-v15-static"
            if mode == "static"
            else "quanttrade-m03r-v15-predictive"
        ),
    ...
    annotations = {
        ...
        "rl-quant/projector-binding-sha256": (
            package.artifacts.projector_binding_sha256
        ),
    ...
    argv = [
        *(M03R_V15_STATIC_ARGV_PREFIX if mode == "static" else M03R_V15_WORKER_ARGV_PREFIX),
        "--package-plan",
...
def render_m03r_v15_suspended_static_job(
    *,
    package: M03RV15PackagePlan,
    authorization: M03RV15ExecutionAuthorization,
    package_plan_file_sha256: str,
...
def render_m03r_v15_suspended_capacity_job(
    *,
    package: M03RV15PackagePlan,
    authorization: M03RV15ExecutionAuthorization,
    package_plan_file_sha256: str,
...
def render_m03r_v15_suspended_predictive_job(
    *,
    package: M03RV15PackagePlan,
    authorization: M03RV15ExecutionAuthorization,
    package_plan_file_sha256: str,
    authorization_file_sha256: str,
    capacity: M03RV15TwoH100CapacityQualification,
    live: M03RV15LiveEvidence,
    template: M03RV7KubernetesTemplateConfig,
    now_utc: datetime,
) -> M03RV15RenderedJob:
...
__all__ = [
    "M03R_V15_CAPACITY_QUALIFICATION_SCHEMA",
    "M03R_V15_KUBERNETES_LIVE_SCHEMA",
    "M03R_V15_RENDERED_JOB_SCHEMA",
    "M03R_V15_STATIC_ARGV_PREFIX",
    "M03R_V15_WORKER_ARGV_PREFIX",
    "M03RV15KubernetesError",
    "M03RV15LiveEvidence",
    "M03RV15RenderedJob",
    "M03RV15TwoH100CapacityQualification",
    "bind_m03r_v15_admitted_suspended_job",
    "build_m03r_v15_live_evidence",
    "render_m03r_v15_suspended_capacity_job",
    "render_m03r_v15_suspended_predictive_job",
    "render_m03r_v15_suspended_static_job",
]
