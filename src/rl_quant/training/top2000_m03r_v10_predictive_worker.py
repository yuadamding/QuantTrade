"""Immutable three-setting worker plan for M03R-v10 rank research."""
...
M03R_V10_WORKER_PLAN_SCHEMA = "rl-quant.top2000-dev.m03r-v10-worker-plan-v1"
M03R_V10_PANEL_PLAN_SCHEMA = "rl-quant.top2000-dev.m03r-v10-panel-plan-v1"


class M03RV10PredictiveWorkerError(ValueError):
    """The v10 worker identity or resource geometry drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV10PredictiveWorkerError(f"{name} must be a lowercase SHA-256")
    return value


@dataclass(frozen=True, slots=True)
class M03RV10PredictiveWorkerPlan:
    setting_index: int
    setting_id: str
    cache_path: str
    cache_sha256: str
    ...
    protocol_generation: str = M03R_V10_PROTOCOL_GENERATION
    design_id: str = M03R_V10_DESIGN_ID
    protocol_sha256: str = M03R_V10_PROTOCOL_SHA256
    development_only: bool = True
    reportable: bool = False
    promotion_eligible: bool = False
    schema: str = M03R_V10_WORKER_PLAN_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            != (21, 30)
            or self.seed != M03R_V10_PREDICTIVE_SPEC.seed
            or self.fold_count != M03R_V10_PREDICTIVE_SPEC.chronological_fold_count
            or self.expected_world_size != 2
            or self.h100s_per_worker != M03R_V10_PREDICTIVE_SPEC.h100s_per_worker
            ...
            or self.protocol_generation != M03R_V10_PROTOCOL_GENERATION
            or self.design_id != M03R_V10_DESIGN_ID
            or self.protocol_sha256 != M03R_V10_PROTOCOL_SHA256
            or not self.development_only
            or self.reportable
            or self.promotion_eligible
            or self.schema != M03R_V10_WORKER_PLAN_SCHEMA
        ):
            raise M03RV10PredictiveWorkerError("v10 worker plan drifted")
...
@dataclass(frozen=True, slots=True)
class M03RV10PredictivePanelPlan:
    workers: tuple[M03RV10PredictiveWorkerPlan, ...]
    ...
    protocol_sha256: str = M03R_V10_PROTOCOL_SHA256
    schema: str = M03R_V10_PANEL_PLAN_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.protocol_sha256 != M03R_V10_PROTOCOL_SHA256
            or self.schema != M03R_V10_PANEL_PLAN_SCHEMA
        ):
            raise M03RV10PredictiveWorkerError("v10 panel plan drifted")
...
def resolve_m03r_v10_predictive_setting_index(completion_index: int) -> int:
    if (
        isinstance(completion_index, bool)
        or not isinstance(completion_index, int)
        or completion_index not in range(3)
    ):
        raise M03RV10PredictiveWorkerError(
...
__all__ = [
    "M03R_V10_PANEL_PLAN_SCHEMA",
    "M03R_V10_WORKER_PLAN_SCHEMA",
