"""Causal h3 target and direct-score batch construction for M03R-v15."""
...
from rl_quant.training.top2000_m03r_v15_fold import M03R_V15_MINIMUM_LOCAL_ORIGIN
from rl_quant.training.top2000_m03r_v15_objective import M03RV15PredictiveBatch
...
M03R_V15_BUILT_BATCH_SCHEMA = "rl-quant.top2000-dev.m03r-v15-built-batch-v1"


class M03RV15PretrainingRuntimeError(ValueError):
    """V15 failed its full-context direct-h3 batch boundary."""
...
class M03RV15BuiltPredictiveBatch:
    objective: M03RV15PredictiveBatch
    raw_predicted_mean: torch.Tensor
    ...
    target_residual_operators: tuple[M03RV15ResidualOperator, ...]
    action_residual_operators: tuple[M03RV15ResidualOperator, ...]
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_BUILT_BATCH_SCHEMA
    ...
    def validate(self) -> None:
        ...
        operator_counts_valid = (
            len(self.target_residual_operators) == rows
            and len(self.action_residual_operators) == rows
        )
        target_masks = (
            torch.stack(
                tuple(
                    target.qualified_asset_mask
                    for target in self.target_residual_operators
                )
            )
            if operator_counts_valid
            else None
        )
        mask_relationship_valid = (
            target_masks is not None
            and _tensor_sha256(self.objective.valid) == _tensor_sha256(target_masks)
            and all(
                not bool(
                    (
                        target.qualified_asset_mask
                        & ~action.qualified_asset_mask
                    ).any()
                )
        ...
        executable_score_valid = operator_counts_valid and all(
            torch.equal(
                apply_m03r_v15_residual_operator(
                    self.raw_predicted_mean[index], operator
                ).residual,
                self.objective.predicted_mean[index],
            )
            for index, operator in enumerate(self.action_residual_operators)
        )
        if (
            not isinstance(self.origin_indices, torch.Tensor)
            or not isinstance(self.raw_predicted_mean, torch.Tensor)
            or tuple(self.raw_predicted_mean.shape)
            != tuple(self.objective.predicted_mean.shape)
            or self.raw_predicted_mean.dtype != self.objective.predicted_mean.dtype
            or self.raw_predicted_mean.device != self.objective.predicted_mean.device
            or not bool(torch.isfinite(self.raw_predicted_mean).all())
            ...
            or not mask_relationship_valid
            or not executable_score_valid
            or any(
            ...
            or self.schema != M03R_V15_BUILT_BATCH_SCHEMA
        ):
            raise M03RV15PretrainingRuntimeError("v15 built batch drifted")
            ...
            json.dumps(
                {
                    ...
                    "raw_predicted_mean_sha256": _tensor_sha256(
                        self.raw_predicted_mean
                    ),
...
def build_m03r_v15_batch_from_origin_states(
    policy: Top2000M03RV15PredictivePolicy,
    setting: M03RV15PredictiveSetting,
    ...
    *,
    ...
    if (
        not isinstance(policy, Top2000M03RV15PredictivePolicy)
        or policy.v15_setting != setting
        ...
        or int(local_origin_indices[0]) < M03R_V15_MINIMUM_LOCAL_ORIGIN
    ):
        raise M03RV15PretrainingRuntimeError(
            "v15 policy, state, origin, or full-context geometry drifted"
        )
    ...
    if (
        ...
        raise M03RV15PretrainingRuntimeError("v15 origins leave the declared split")
    ...
    if (
        ...
        raise M03RV15PretrainingRuntimeError(
            "v15 exposure and sequence asset axes differ"
        )

    raw_means: list[torch.Tensor] = []
    executable_means: list[torch.Tensor] = []
    ...
    horizon = M03R_V15_SELECTED_HORIZON_SESSIONS
    for row_index, local_origin in enumerate(local_origin_indices.tolist()):
        ...
        residual = apply_m03r_v15_residual_operator(
            (stock - benchmark).detach(), target_operator
        )
        executable = apply_m03r_v15_residual_operator(raw_mean, action_operator)
        executable_means.append(executable.residual)
    ...
    objective = M03RV15PredictiveBatch(
        predicted_mean=torch.stack(executable_means),
        predicted_log_scale=torch.stack(log_scales),
        target_log_return=torch.stack(targets),
        valid=torch.stack(valid),
        setting=setting,
    )
    result = M03RV15BuiltPredictiveBatch(
        objective=objective,
        raw_predicted_mean=torch.stack(raw_means),
...
__all__ = [
    "M03R_V15_BUILT_BATCH_SCHEMA",
    "M03RV15BuiltPredictiveBatch",
    "M03RV15PretrainingRuntimeError",
