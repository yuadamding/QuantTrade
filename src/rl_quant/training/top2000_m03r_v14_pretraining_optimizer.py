"""Two-way optimizer partition for the direct M03R-v14 predictor."""
...
M03R_V14_OPTIMIZER_SCHEMA = "rl-quant.top2000-dev.m03r-v14-optimizer-v1"
M03R_V14_ENCODER_LEARNING_RATE = 2.0e-5
M03R_V14_HEAD_LEARNING_RATE = 1.0e-4
M03R_V14_WEIGHT_DECAY = 1.0e-4
M03R_V14_ENCODER_GRADIENT_CLIP_NORM = 1.0
M03R_V14_HEAD_GRADIENT_CLIP_NORM = 1.0


class M03RV14OptimizerError(ValueError):
    """The v14 optimizer partition drifted."""
...
class M03RV14OptimizerPartition:
    setting_id: str
    encoder_parameter_names: tuple[str, ...]
    head_parameter_names: tuple[str, ...]
    encoder_learning_rate: float = M03R_V14_ENCODER_LEARNING_RATE
    head_learning_rate: float = M03R_V14_HEAD_LEARNING_RATE
    weight_decay: float = M03R_V14_WEIGHT_DECAY
    encoder_gradient_clip_norm: float = M03R_V14_ENCODER_GRADIENT_CLIP_NORM
    head_gradient_clip_norm: float = M03R_V14_HEAD_GRADIENT_CLIP_NORM
    protocol_sha256: str = M03R_V14_PROTOCOL_SHA256
    schema: str = M03R_V14_OPTIMIZER_SCHEMA

    def validate(self) -> None:
        if (
            self.setting_id not in M03R_V14_SETTING_IDS
            ...
            or self.encoder_learning_rate != M03R_V14_ENCODER_LEARNING_RATE
            or self.head_learning_rate != M03R_V14_HEAD_LEARNING_RATE
            or self.weight_decay != M03R_V14_WEIGHT_DECAY
            or self.encoder_gradient_clip_norm
            != M03R_V14_ENCODER_GRADIENT_CLIP_NORM
            or self.head_gradient_clip_norm != M03R_V14_HEAD_GRADIENT_CLIP_NORM
            or self.protocol_sha256 != M03R_V14_PROTOCOL_SHA256
            or self.schema != M03R_V14_OPTIMIZER_SCHEMA
            or not all(
                name == "source_policy.core.cash_bias"
                or name.startswith(M03R_V14_ENCODER_PARAMETER_PREFIXES)
            ...
            raise M03RV14OptimizerError("v14 optimizer partition drifted")
...
def _parameter_names(
    ...
    if unassigned:
        raise M03RV14OptimizerError(
            f"v14 has unassigned trainable parameters: {tuple(sorted(unassigned))!r}"
        )
    return tuple(sorted(encoder)), tuple(sorted(heads))


def build_m03r_v14_optimizer(
    policy: Top2000M03RV14PredictivePolicy,
) -> tuple[torch.optim.AdamW, M03RV14OptimizerPartition]:
    if not isinstance(policy, Top2000M03RV14PredictivePolicy):
        raise M03RV14OptimizerError("v14 optimizer requires its exact policy")
...
def validate_m03r_v14_optimizer(
    policy: Top2000M03RV14PredictivePolicy,
    ...
    if (
        partition.setting_id != policy.v14_setting.setting_id
        or names != expected_names
        or len(optimizer.param_groups) != 2
    ):
        raise M03RV14OptimizerError("v14 optimizer inventory drifted")
    named = dict(policy.named_parameters())
    expected = (
    ...
    for group, (group_name, rate, parameter_names) in zip(
        ...
        if (
            ...
            raise M03RV14OptimizerError("v14 optimizer group drifted")


__all__ = [
    "M03R_V14_ENCODER_LEARNING_RATE",
    "M03R_V14_HEAD_LEARNING_RATE",
    "M03R_V14_OPTIMIZER_SCHEMA",
    "M03R_V14_WEIGHT_DECAY",
    "M03RV14OptimizerError",
