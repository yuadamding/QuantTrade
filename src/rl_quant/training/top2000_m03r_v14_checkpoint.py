"""Immutable direct-h3 checkpoint round trip for M03R-v14."""
...
from rl_quant.training.top2000_m03r_v14_fold import render_m03r_v14_fold_geometries
from rl_quant.training.top2000_m03r_v14_policy import (
    M03RV14HeadIdentity,
    Top2000M03RV14PredictivePolicy,
)

M03R_V14_ALPHA_CHECKPOINT_SCHEMA = "rl-quant.top2000-dev.m03r-v14-alpha-checkpoint-v1"
_MAX_CHECKPOINT_BYTES = 8 * 1024**3
EvaluationResult = TypeVar("EvaluationResult")


class M03RV14CheckpointError(ValueError):
    """The v14 checkpoint artifact or exact reload drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV14CheckpointError(f"{name} must be a lowercase SHA-256")
    return value
...
def _expected_updates(fold_index: int) -> int:
    if isinstance(fold_index, bool) or fold_index not in range(6):
        raise M03RV14CheckpointError("v14 checkpoint fold index drifted")
    try:
        return render_m03r_v14_fold_geometries(1001)[fold_index].optimizer_updates
    except (IndexError, TypeError) as exc:
        raise M03RV14CheckpointError("v14 checkpoint fold index drifted") from exc


@dataclass(frozen=True, slots=True)
class M03RV14LoadedCheckpoint:
    setting_index: int
    setting_id: str
    fold_index: int
    ...
    head_identity: M03RV14HeadIdentity
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise M03RV14CheckpointError("v14 loaded checkpoint drifted")
        for name, value in (
...
def write_immutable_m03r_v14_alpha_checkpoint(
    path: str | Path,
    ...
    *,
    ...
    if target.exists() or target.is_symlink():
        raise M03RV14CheckpointError("v14 checkpoint target already exists")
    if (
        ...
        raise M03RV14CheckpointError("v14 checkpoint cursor drifted")
    for name, value in (
    ...
    identity = policy.v14_head_identity()
    payload = {
        "schema": M03R_V14_ALPHA_CHECKPOINT_SCHEMA,
        "protocol_sha256": M03R_V14_PROTOCOL_SHA256,
        "setting_index": policy.v14_setting.setting_index,
        "setting_id": policy.v14_setting.setting_id,
...
def load_m03r_v14_alpha_checkpoint_for_evaluation(
    path: str | Path,
    *,
    ...
    if (
        ...
        raise M03RV14CheckpointError("v14 checkpoint horizon or update drifted")
    expected_file_sha256 = _digest("expected_file_sha256", expected_file_sha256)
    try:
        descriptor = os.open(source, os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0))
    except OSError as exc:
        raise M03RV14CheckpointError(
            "v14 checkpoint must be a readable regular non-symlink"
        ) from exc
    try:
        ...
        if (
            ...
            raise M03RV14CheckpointError("v14 checkpoint size or type is invalid")
        digest = hashlib.sha256()
        while block := os.read(descriptor, 1024 * 1024):
            digest.update(block)
        if digest.hexdigest() != expected_file_sha256:
            raise M03RV14CheckpointError("v14 checkpoint file hash drifted")
        ...
        if (
            ...
            raise M03RV14CheckpointError("v14 checkpoint changed while read")
    finally:
        os.close(descriptor)
    if not isinstance(payload, Mapping):
        raise M03RV14CheckpointError("v14 checkpoint payload is not a mapping")
    if expected_setting_index not in range(len(M03R_V14_SETTING_IDS)):
        raise M03RV14CheckpointError("v14 expected setting index drifted")
    expected = {
        "schema": M03R_V14_ALPHA_CHECKPOINT_SCHEMA,
        "protocol_sha256": M03R_V14_PROTOCOL_SHA256,
        "setting_index": expected_setting_index,
        "setting_id": M03R_V14_SETTING_IDS[expected_setting_index],
    ...
    if any(payload.get(name) != value for name, value in expected.items()):
        raise M03RV14CheckpointError("v14 checkpoint immutable identity drifted")
    ...
    if (
        ...
        raise M03RV14CheckpointError("v14 checkpoint model state drifted")
    try:
        policy.load_state_dict(dict(state), strict=True)
        stored_identity = M03RV14HeadIdentity(**dict(head_row))
    except (RuntimeError, TypeError, ValueError) as exc:
        raise M03RV14CheckpointError("v14 checkpoint strict load failed") from exc
    observed_identity = policy.v14_head_identity()
    if (
        state_dict_sha256(policy.state_dict()) != payload["model_state_sha256"]
        or observed_identity != stored_identity
    ):
        raise M03RV14CheckpointError("loaded v14 semantic state drifted")
    result = M03RV14LoadedCheckpoint(
        setting_index=expected_setting_index,
        setting_id=M03R_V14_SETTING_IDS[expected_setting_index],
        fold_index=expected_fold_index,
        completed_updates=expected_completed_updates,
...
def write_reload_evaluate_m03r_v14_checkpoint(
    path: str | Path,
    policy: Top2000M03RV14PredictivePolicy,
    fresh_policy_factory: Callable[[], Top2000M03RV14PredictivePolicy],
    evaluator: Callable[
        [Top2000M03RV14PredictivePolicy, M03RV14LoadedCheckpoint], EvaluationResult
    ],
    *,
    fold_index: int,
    completed_updates: int,
    ...
    source_array_sha256: str,
    asset_axis_sha256: str,
) -> tuple[M03RV14LoadedCheckpoint, EvaluationResult]:
    file_sha256 = write_immutable_m03r_v14_alpha_checkpoint(
        path,
        policy,
        fold_index=fold_index,
        completed_updates=completed_updates,
    ...
    loaded = load_m03r_v14_alpha_checkpoint_for_evaluation(
        ...
        expected_setting_index=loaded_policy.v14_setting.setting_index,
...
__all__ = [
    "M03R_V14_ALPHA_CHECKPOINT_SCHEMA",
    "M03RV14CheckpointError",
    "M03RV14LoadedCheckpoint",
    "load_m03r_v14_alpha_checkpoint_for_evaluation",
    "write_immutable_m03r_v14_alpha_checkpoint",
    "write_reload_evaluate_m03r_v14_checkpoint",
]
