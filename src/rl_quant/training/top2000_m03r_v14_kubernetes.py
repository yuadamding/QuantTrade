"""Suspended Seadragon Jobs for M03R-v14 predictive qualification only."""
...
M03R_V14_KUBERNETES_LIVE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v14-live-kubernetes-evidence-v1"
)
M03R_V14_CAPACITY_QUALIFICATION_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v14-two-h100-capacity-qualification-v1"
)
M03R_V14_RENDERED_JOB_SCHEMA = "rl-quant.top2000-dev.m03r-v14-rendered-job-v1"
M03R_V14_WORKER_ARGV_PREFIX = (
    ...
    "--nproc-per-node=2",
    "-m",
    "rl_quant.workflows.top2000_m03r_v14_predictive",
)
M03R_V14_STATIC_ARGV_PREFIX = (
    "/opt/conda/envs/quanttrade/bin/python",
    "-m",
    "rl_quant.workflows.top2000_m03r_v14_static_validate",
)


class M03RV14KubernetesError(ValueError):
    """The v14 manifest, live evidence, or capacity receipt drifted."""
...
def _digest(name: str, value: str) -> None:
    if len(value) != 64 or any(
        ...
        raise M03RV14KubernetesError(f"{name} must be a lowercase SHA-256")


@dataclass(frozen=True, slots=True)
class M03RV14LiveEvidence:
    observed_at_utc: str
    ...
    schema: str = M03R_V14_KUBERNETES_LIVE_SCHEMA
    ...
    def __post_init__(self) -> None:
        try:
            ...
            raise M03RV14KubernetesError("live timestamp must be ISO-8601") from exc
        if observed.tzinfo is None or observed.utcoffset() != UTC.utcoffset(observed):
            raise M03RV14KubernetesError("live timestamp must carry UTC timezone")
        if (
            self.schema != M03R_V14_KUBERNETES_LIVE_SCHEMA
            ...
            raise M03RV14KubernetesError("v14 live Kubernetes evidence is invalid")
    ...
    def require_fresh(self, *, now_utc: datetime, max_age_seconds: int = 300) -> None:
        if now_utc.tzinfo is None:
            raise M03RV14KubernetesError("now_utc must be timezone-aware")
        observed = datetime.fromisoformat(self.observed_at_utc)
        age = (now_utc.astimezone(UTC) - observed.astimezone(UTC)).total_seconds()
        if age < 0 or age > max_age_seconds:
            raise M03RV14KubernetesError("live Kubernetes evidence is stale")


def build_m03r_v14_live_evidence(
    *,
    observed_at_utc: str,
    rbac: M03RV7KubernetesRBACEvidence,
    protected_or_other_committed_h100_count: int,
    ...
    payload = {
        "schema": M03R_V14_KUBERNETES_LIVE_SCHEMA,
...
@dataclass(frozen=True, slots=True)
class M03RV14TwoH100CapacityQualification:
    ...
    schema: str = M03R_V14_CAPACITY_QUALIFICATION_SCHEMA
    ...
    def validate_for(
        ...
        if (
            ...
            or self.schema != M03R_V14_CAPACITY_QUALIFICATION_SCHEMA
        ):
            raise M03RV14KubernetesError(
                "capacity receipt does not bind this v14 package"
            )


@dataclass(frozen=True, slots=True)
class M03RV14RenderedJob:
    ...
    schema: str = M03R_V14_RENDERED_JOB_SCHEMA
    ...
    def validate(self) -> None:
        if (
            ...
            or self.schema != M03R_V14_RENDERED_JOB_SCHEMA
        ):
            raise M03RV14KubernetesError("rendered v14 Job drifted")


def bind_m03r_v14_admitted_suspended_job(
    *,
    ...
    if rendered.mode == "static":
        from rl_quant.training.top2000_m03r_v14_static_gate import (
            bind_m03r_v14_static_admitted_suspended_job,
        )

        try:
            return bind_m03r_v14_static_admitted_suspended_job(
                rendered=rendered,
            ...
            raise M03RV14KubernetesError(
                "v14 static admitted suspended Job failed the strict contract"
            ) from exc
    try:
        ...
        raise M03RV14KubernetesError(
            "v14 admitted suspended Job failed the shared strict contract"
        ) from exc
...
def _render(
    *,
    ...
    live: M03RV14LiveEvidence,
    template: M03RV7KubernetesTemplateConfig,
    now_utc: datetime,
    mode: Literal["static", "capacity", "predictive"],
    capacity: M03RV14TwoH100CapacityQualification | None,
) -> M03RV14RenderedJob:
    package.validate()
    authorization.validate(package)
    ...
    if (
        ...
        raise M03RV14KubernetesError("package and PVC mount identities drifted")
    completions = 2 if mode == "predictive" else 1
    if mode != "static" and live.allowed_parallelism <= 0:
        raise M03RV14KubernetesError("live H100 cap does not allow one worker")
    ...
    if parallelism * gpus > authorization.maximum_h100_requests:
        raise M03RV14KubernetesError("predictive H100 request ceiling drifted")
    labels = {
        "app.kubernetes.io/name": (
            "quanttrade-m03r-v14-static"
            if mode == "static"
            else "quanttrade-m03r-v14-predictive"
        ),
    ...
    argv = [
        *(M03R_V14_STATIC_ARGV_PREFIX if mode == "static" else M03R_V14_WORKER_ARGV_PREFIX),
        "--package-plan",
...
def render_m03r_v14_suspended_static_job(
    *,
    package: M03RV14PackagePlan,
    authorization: M03RV14ExecutionAuthorization,
    package_plan_file_sha256: str,
...
def render_m03r_v14_suspended_capacity_job(
    *,
    package: M03RV14PackagePlan,
    authorization: M03RV14ExecutionAuthorization,
    package_plan_file_sha256: str,
...
def render_m03r_v14_suspended_predictive_job(
    *,
    package: M03RV14PackagePlan,
    authorization: M03RV14ExecutionAuthorization,
    package_plan_file_sha256: str,
    authorization_file_sha256: str,
    capacity: M03RV14TwoH100CapacityQualification,
    live: M03RV14LiveEvidence,
    template: M03RV7KubernetesTemplateConfig,
    now_utc: datetime,
) -> M03RV14RenderedJob:
...
__all__ = [
    "M03R_V14_CAPACITY_QUALIFICATION_SCHEMA",
    "M03R_V14_KUBERNETES_LIVE_SCHEMA",
    "M03R_V14_RENDERED_JOB_SCHEMA",
    "M03R_V14_STATIC_ARGV_PREFIX",
    "M03R_V14_WORKER_ARGV_PREFIX",
    "M03RV14KubernetesError",
    "M03RV14LiveEvidence",
    "M03RV14RenderedJob",
    "M03RV14TwoH100CapacityQualification",
    "bind_m03r_v14_admitted_suspended_job",
    "build_m03r_v14_live_evidence",
    "render_m03r_v14_suspended_capacity_job",
    "render_m03r_v14_suspended_predictive_job",
    "render_m03r_v14_suspended_static_job",
]
