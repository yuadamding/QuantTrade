"""Single direct-to-execution three-session policy output for M03R-v13."""
...
M03R_V13_POLICY_SCHEMA = "rl-quant.top2000-dev.m03r-v13-predictive-policy-v1"
M03R_V13_OUTPUT_SCHEMA = "rl-quant.top2000-dev.m03r-v13-direct-h3-output-v1"
M03R_V13_ALPHA_OUTPUT_CONTRACT_SHA256 = hashlib.sha256(
    (
        "one-three-session-economic-mean-is-rank-and-execution-score;"
        "one-three-session-log-scale;no-separate-rank-head;cash-zero-v1"
    ).encode("ascii")
).hexdigest()
...
class M03RV13PolicyError(ValueError):
    """The v13 direct predictive output or identity drifted."""
...
class M03RV13AlphaDistribution:
    ...
    schema: str = M03R_V13_OUTPUT_SCHEMA
    ...
    def validate(self) -> None:
        if (
            ...
            or self.schema != M03R_V13_OUTPUT_SCHEMA
        ):
            raise M03RV13PolicyError("v13 direct alpha distribution drifted")
...
@dataclass(frozen=True, slots=True)
class M03RV13HeadIdentity:
    setting_id: str
    selected_alpha_horizon: int
    economic_mean_head_state_sha256: str
    economic_scale_head_state_sha256: str
    output_contract_sha256: str = M03R_V13_ALPHA_OUTPUT_CONTRACT_SHA256

    def validate(self) -> None:
        resolve_m03r_v13_setting(self.setting_id)
        for value in (
            ...
            if len(value) != 64 or any(
                ...
                raise M03RV13PolicyError("v13 head identity has a malformed digest")
        if (
            self.selected_alpha_horizon != M03R_V13_SELECTED_HORIZON_SESSIONS
            or self.output_contract_sha256
            != M03R_V13_ALPHA_OUTPUT_CONTRACT_SHA256
        ):
            raise M03RV13PolicyError("v13 head identity drifted")
...
class Top2000M03RV13PredictivePolicy(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.v13_setting: M03RV13PredictiveSetting = resolve_m03r_v13_setting(setting)
        if (
            isinstance(selected_horizon_sessions, bool)
            or selected_horizon_sessions != M03R_V13_SELECTED_HORIZON_SESSIONS
        ):
            raise M03RV13PolicyError("v13 model selected horizon drifted")
        ...
        if source_alpha is None:
            raise M03RV13PolicyError("reviewed source policy omitted its alpha head")
        ...
        nn.init.constant_(
            ...
            math.log(0.02 * math.sqrt(M03R_V13_SELECTED_HORIZON_SESSIONS)),
        )
    ...
    def predictive_output(
        ...
        if (
            ...
            raise M03RV13PolicyError("v13 state and availability axes are invalid")
        with torch.autocast(
            ...
            log_scale = (
                self.economic_scale_head(hidden).squeeze(-1).float().clamp(-8.0, 2.0)
            )
        ...
        result = M03RV13AlphaDistribution(mean, log_scale)
        result.validate()
        return result

    def v13_head_identity(self) -> M03RV13HeadIdentity:
        result = M03RV13HeadIdentity(
            setting_id=self.v13_setting.setting_id,
...
__all__ = [
    "M03R_V13_ALPHA_OUTPUT_CONTRACT_SHA256",
    "M03R_V13_ENCODER_PARAMETER_PREFIXES",
    "M03R_V13_OUTPUT_SCHEMA",
    "M03R_V13_POLICY_SCHEMA",
    "M03RV13AlphaDistribution",
    "M03RV13HeadIdentity",
    "M03RV13PolicyError",
