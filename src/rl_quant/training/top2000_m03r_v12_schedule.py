"""Setting-neutral paired episode schedule for M03R-v12."""
...
M03R_V12_PANEL_SCHEDULE_SCHEMA = "rl-quant.top2000-dev.m03r-v12-panel-schedule-v1"
M03R_V12_PAIRED_INPUT_SCHEMA = "rl-quant.top2000-dev.m03r-v12-paired-input-v1"


class M03RV12ScheduleError(ValueError):
    """The v12 paired episode or rank-shard schedule drifted."""


class M03RV12TrainingShardLike(Protocol):
    @property
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV12ScheduleError(f"{name} must be a lowercase SHA-256")
    return value


@dataclass(frozen=True, slots=True)
class M03RV12PanelEpisodeSchedule:
    protocol_common_data_sha256: str
    cache_sha256: str
    fold_geometry_sha256: tuple[str, ...]
    seed: int = 17
    optimizer_updates: int = 64
    schedule_rule: str = M03R_V12_EPISODE_SCHEDULE_RULE
    protocol_sha256: str = M03R_V12_PROTOCOL_SHA256
    schema: str = M03R_V12_PANEL_SCHEDULE_SCHEMA

    def validate(self) -> None:
        if (
            len(self.fold_geometry_sha256)
            != M03R_V12_PREDICTIVE_SPEC.chronological_fold_count
            or len(set(self.fold_geometry_sha256)) != len(self.fold_geometry_sha256)
            or self.seed != M03R_V12_PREDICTIVE_SPEC.seed
            or self.optimizer_updates != M03R_V12_PREDICTIVE_SPEC.optimizer_updates
            or self.schedule_rule != M03R_V12_EPISODE_SCHEDULE_RULE
            or self.protocol_sha256 != M03R_V12_PROTOCOL_SHA256
            or self.schema != M03R_V12_PANEL_SCHEDULE_SCHEMA
        ):
            raise M03RV12ScheduleError("v12 panel episode schedule drifted")
        for name, value in (
...
def deterministic_m03r_v12_episode_start(
    ...
    *,
    ...
    if (
        ...
        raise M03RV12ScheduleError("v12 episode cursor or geometry is invalid")
...
def m03r_v12_complementary_rank_shards(
    ...
    *,
    ...
    if (
        ...
        or world_size != M03R_V12_PREDICTIVE_SPEC.expected_world_size
    ):
        raise M03RV12ScheduleError("v12 global origins or world size drifted")
    usable = global_origins[: len(global_origins) - len(global_origins) % world_size]
    if len(usable) < world_size:
        raise M03RV12ScheduleError("v12 paired origin set is too small")
    ...
    if (
        ...
        raise M03RV12ScheduleError("v12 rank shards are not complementary")
    return shards


@dataclass(frozen=True, slots=True)
class M03RV12PairedInputReceipt:
    schedule_sha256: str
    fold_index: int
    ...
    protocol_sha256: str = M03R_V12_PROTOCOL_SHA256
    schema: str = M03R_V12_PAIRED_INPUT_SCHEMA

    def validate(self) -> None:
        shards = m03r_v12_complementary_rank_shards(self.global_origins)
        ...
        if (
            ...
            or self.protocol_sha256 != M03R_V12_PROTOCOL_SHA256
            or self.schema != M03R_V12_PAIRED_INPUT_SCHEMA
        ):
            raise M03RV12ScheduleError("v12 paired input receipt drifted")
        for name, value in (
...
def _torch_module() -> Any:
    try:
        ...
        raise M03RV12ScheduleError(
            "PyTorch is required for v12 tensor schedule operations"
        ) from exc
    return torch
...
def build_m03r_v12_paired_input_receipt(
    training_shard: M03RV12TrainingShardLike,
    ...
    *,
    ...
    if (
        ...
        or rank_origins != m03r_v12_complementary_rank_shards(global_origins)
    ):
        raise M03RV12ScheduleError("v12 paired input tensor inventory drifted")
    result = M03RV12PairedInputReceipt(
...
__all__ = [
    "M03R_V12_PAIRED_INPUT_SCHEMA",
    "M03R_V12_PANEL_SCHEDULE_SCHEMA",
    "M03RV12PairedInputReceipt",
    "M03RV12PanelEpisodeSchedule",
    "M03RV12ScheduleError",
    "M03RV12TrainingShardLike",
