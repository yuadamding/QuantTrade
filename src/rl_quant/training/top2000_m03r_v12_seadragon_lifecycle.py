"""Attach-only Seadragon lifecycle for the M03R-v12 predictive panel.
...
ATTACH_CONFIG_SCHEMA: Final = "rl-quant.top2000-dev.m03r-v12-seadragon-attach-config-v1"
COMPLETION_COVERAGE_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v12-predictive-coverage-v1"
)
_SHA256_RE = re.compile(r"^[0-9a-f]{64}$")
CAPACITY_CONFIG_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v12-capacity-attach-config-v1"
)
M03R_V12_STARTUP_SCHEMA: Final = "rl-quant.top2000-dev.m03r-v12-two-h100-startup-v1"
M03R_V12_FOLD_TERMINAL_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v12-predictive-fold-terminal-v1"
)
M03R_V12_WORKER_TERMINAL_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v12-predictive-worker-terminal-v1"
)
M03R_V12_CAPACITY_TERMINAL_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v12-two-h100-capacity-terminal-v1"
)
M03R_V12_BOOTSTRAP_PLAN_SCHEMA: Final = (
    "rl-quant.top2000-dev.m03r-v12-bootstrap-plan-v1"
)
M03R_V12_EXECUTION_HORIZONS: Final = tuple(M03R_V12_ELIGIBLE_EXECUTION_HORIZONS)
M03R_V12_EXECUTION_HORIZON_KEYS: Final = frozenset(
    str(value) for value in M03R_V12_EXECUTION_HORIZONS
)


class M03RV12SeadragonLifecycleError(RuntimeError):
    """The exact v12 attachment or its evidence failed closed."""


class M03RV12ActivationAttachRequired(M03RV12SeadragonLifecycleError):
    """Activation outcome is ambiguous and exact state must be retained."""


@dataclass(frozen=True, slots=True)
class M03RV12CapacityAttachConfig:
    job_name: str
    run_id: str
    job_uid: str
    binding_path: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise M03RV12SeadragonLifecycleError(
                "v12 capacity config drifted from the one-worker 2-H100 contract"
            )
        for name in (
            "binding_path",
        ...
        if not Path(self.host_python_path).is_absolute():
            raise M03RV12SeadragonLifecycleError("host_python_path must be absolute")
...
def _canonical_bytes(value: Any) -> bytes:
    try:
        ...
        raise M03RV12SeadragonLifecycleError(
            "v12 lifecycle evidence is not canonical-JSON safe"
        ) from exc
...
def _require_sha256(name: str, value: str) -> None:
    if _SHA256_RE.fullmatch(value) is None:
        raise M03RV12SeadragonLifecycleError(f"{name} must be a lowercase SHA-256")


def _mapping(value: Any, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise M03RV12SeadragonLifecycleError(f"{label} must be an object")
...
def _receipt_payload(value: Mapping[str, Any], label: str) -> Mapping[str, Any]:
    ...
    if not isinstance(receipt, str):
        raise M03RV12SeadragonLifecycleError(f"{label} receipt hash is absent")
    _require_sha256(f"{label} receipt", receipt)
    unsigned = dict(value)
    del unsigned["receipt_sha256"]
    if receipt != _content_sha256(unsigned):
        raise M03RV12SeadragonLifecycleError(f"{label} receipt hash drifted")
    return unsigned
...
def _project_path(path: str, label: str) -> Path:
    ...
    if not value.is_absolute():
        raise M03RV12SeadragonLifecycleError(f"{label} must be absolute")
    try:
...
@dataclass(frozen=True, slots=True)
class M03RV12ExpectedCompletion:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise M03RV12SeadragonLifecycleError(
                "v12 completion identity must be the frozen direct 0..2 map"
            )


@dataclass(frozen=True, slots=True)
class M03RV12AttachSupervisorConfig:
    job_name: str
    run_id: str
    job_uid: str
    binding_path: str
    ...
    lifecycle_source_sha256: str
    expected_completions: tuple[M03RV12ExpectedCompletion, ...]
    host_python_path: str
    pythonpath: str
    kubectl_path: str = SEADRAGON_KUBECTL
    kubeconfig_path: str = SEADRAGON_KUBECONFIG
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise M03RV12SeadragonLifecycleError(
                "v12 attach config drifted from the three-setting 2-H100 contract"
            )
        for name in (
            "binding_path",
            "activation_request_path",
...
def _load_config(path: Path, expected_sha256: str) -> M03RV12AttachSupervisorConfig:
    ...
    values["expected_completions"] = tuple(
        M03RV12ExpectedCompletion(**dict(_mapping(row, "expected completion")))
        for row in cast(Sequence[Any], values.get("expected_completions", ()))
    )
    try:
        return M03RV12AttachSupervisorConfig(**values)
    except (TypeError, ValueError) as exc:
        raise M03RV12SeadragonLifecycleError("v12 attach config is invalid") from exc


def _load_capacity_config(
    path: Path, expected_sha256: str
) -> M03RV12CapacityAttachConfig:
    _require_sha256("capacity config file", expected_sha256)
    payload = _mapping(
        common._read_json_file(path, expected_sha256=expected_sha256),
        "v12 capacity attach config",
    )
    try:
        return M03RV12CapacityAttachConfig(**dict(payload))
    except (TypeError, ValueError) as exc:
        raise M03RV12SeadragonLifecycleError(
            "v12 capacity attach config is invalid"
        ) from exc


def _load_package_authorization(
    config: M03RV12AttachSupervisorConfig | M03RV12CapacityAttachConfig,
) -> tuple[Any, Any]:
    ...
    if package.package_plan_sha256 != config.package_plan_sha256:
        raise M03RV12SeadragonLifecycleError("package-plan identity drifted")
    ...
    if authorization.receipt_sha256 != config.execution_authorization_receipt_sha256:
        raise M03RV12SeadragonLifecycleError("execution-authorization identity drifted")
...
def _host_output_path(config: M03RV12AttachSupervisorConfig, value: Any) -> Path:
    if not isinstance(value, str):
        raise M03RV12SeadragonLifecycleError("worker artifact path is absent")
    ...
    if not relative.parts or any(part in {"", ".", ".."} for part in relative.parts):
        raise M03RV12SeadragonLifecycleError("worker artifact path is unsafe")
    return Path(config.output_root).joinpath(*relative.parts)


def _validate_rank_runtime(value: Any) -> None:
    if not isinstance(value, list) or len(value) != 2:
        raise M03RV12SeadragonLifecycleError("startup must contain two rank rows")
    rows = sorted(
...
def _validate_capacity_output(
    config: M03RV12CapacityAttachConfig,
    ...
    if (
        startup.get("schema") != M03R_V12_STARTUP_SCHEMA
        ...
        or startup.get("setting_id") != M03R_V12_SETTING_IDS[0]
        ...
        or terminal.get("schema") != M03R_V12_CAPACITY_TERMINAL_SCHEMA
        ...
        or terminal.get("setting_id") != M03R_V12_SETTING_IDS[0]
...
def _read_bound_file(path: Path, expected_sha256: str, label: str) -> bytes:
    ...
    if hashlib.sha256(content).hexdigest() != expected_sha256:
        raise M03RV12SeadragonLifecycleError(f"{label} file hash drifted")
    return content
...
def _read_bound_json(path: Path, expected_sha256: str, label: str) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise M03RV12SeadragonLifecycleError(f"{label} is invalid JSON") from exc
...
def _validate_static_gate_lineage(
    ...
    *,
    ...
    if (
        ...
        or value.get("schema") != "rl-quant.top2000-dev.m03r-v12-static-gate-v1"
        ...
        raise M03RV12SeadragonLifecycleError("static gate lineage drifted")
    for name in digest_fields:
        digest = value.get(name)
        if not isinstance(digest, str):
            raise M03RV12SeadragonLifecycleError(f"static gate {name} is not a digest")
...
def _validate_qualification(
    ...
    *,
    ...
    expected_fold_receipts: list[str],
    expected_bootstrap_plan_sha256: str,
) -> None:
    unsigned = dict(_semantic_receipt_payload(value, "horizon qualification"))
    try:
        qualification = M03RV12PredictiveQualification(
            **{
                **unsigned,
                "fold_receipt_sha256": tuple(unsigned["fold_receipt_sha256"]),
            }
        )
        qualification.validate()
    except (KeyError, TypeError, ValueError) as exc:
        raise M03RV12SeadragonLifecycleError(
            "horizon qualification is not a valid v12 typed receipt"
        ) from exc
    if (
        qualification.schema != M03R_V12_QUALIFICATION_SCHEMA
        or qualification.setting_index != setting_index
        or qualification.setting_id != M03R_V12_SETTING_IDS[setting_index]
        or qualification.horizon_sessions != expected_horizon
        or list(qualification.fold_receipt_sha256) != expected_fold_receipts
        or qualification.bootstrap_plan_sha256 != expected_bootstrap_plan_sha256
        or value.get("receipt_sha256") != qualification.receipt_sha256
    ):
        raise M03RV12SeadragonLifecycleError(
...
def _validate_host_bootstrap_plan(value: Mapping[str, Any]) -> str:
    ...
    if (
        ...
        or value.get("bootstrap_seed") != 17
        or value.get("replicates") != M03R_V12_PREDICTIVE_SPEC.bootstrap_replicates
        or value.get("rule") != M03R_V12_BOOTSTRAP_RULE
        or value.get("protocol_sha256") != M03R_V12_PROTOCOL_SHA256
        or value.get("schema") != M03R_V12_BOOTSTRAP_PLAN_SCHEMA
    ):
        raise M03RV12SeadragonLifecycleError("bootstrap plan is invalid")
    chronology = value.get("chronology_sha256")
    if not isinstance(chronology, str):
        raise M03RV12SeadragonLifecycleError("bootstrap chronology hash is absent")
...
def _validate_one_worker(
    config: M03RV12AttachSupervisorConfig,
    row: M03RV12ExpectedCompletion,
    ...
    if (
        ...
        or startup.get("reportable") is not False
        or startup.get("promotion_eligible") is not False
        or terminal.get("schema") != M03R_V12_WORKER_TERMINAL_SCHEMA
        ...
        raise M03RV12SeadragonLifecycleError("worker terminal identity drifted")
    fold_hashes = terminal.get("fold_terminal_file_sha256")
    if not isinstance(fold_hashes, list) or len(fold_hashes) != 6:
        raise M03RV12SeadragonLifecycleError("worker must bind six fold receipts")
    fold_receipts: list[str] = []
    fold_evidence_receipts: dict[int, list[str]] = {
        horizon: [] for horizon in M03R_V12_EXECUTION_HORIZONS
    }
    for fold_index, expected_sha in enumerate(fold_hashes):
        if not isinstance(expected_sha, str):
            raise M03RV12SeadragonLifecycleError("fold receipt hash is absent")
        ...
        if set(candidates) != M03R_V12_EXECUTION_HORIZON_KEYS:
            raise M03RV12SeadragonLifecycleError("fold horizon inventory drifted")
        for candidate_horizon in M03R_V12_EXECUTION_HORIZONS:
            candidate = _mapping(
            ...
            for name in (
                ...
                if not isinstance(value, str):
                    raise M03RV12SeadragonLifecycleError(f"candidate {name} is absent")
                _require_sha256(name, value)
            fold_evidence_receipts[candidate_horizon].append(
                cast(str, candidate["fold_evidence_sha256"])
            )
        if (
            fold.get("schema") != M03R_V12_FOLD_TERMINAL_SCHEMA
            ...
            or fold.get("qualification_evaluated_only_after_checkpoint_publication")
            is not True
            or not isinstance(fold.get("model_state_sha256"), str)
            or not isinstance(fold.get("optimizer_state_sha256"), str)
            or not isinstance(fold.get("paired_input_receipt_sha256"), str)
            or not isinstance(fold.get("training_source_array_sha256"), str)
            or not isinstance(fold.get("training_residual_operator_root_sha256"), str)
            ...
            raise M03RV12SeadragonLifecycleError("fold receipt semantics drifted")
        steps = fold.get("rank_step_receipt_sha256")
        if not isinstance(steps, list) or len(steps) != 2 or len(set(steps)) != 2:
            raise M03RV12SeadragonLifecycleError(
                "fold must bind two distinct final rank-step receipts"
            )
        for value in steps:
            if not isinstance(value, str):
                raise M03RV12SeadragonLifecycleError("update receipt hash is absent")
    ...
    if terminal.get("bootstrap_plan_sha256") != bootstrap_sha256:
        raise M03RV12SeadragonLifecycleError("bootstrap plan identity drifted")
    qualifications = terminal.get("horizon_qualification")
    if not isinstance(qualifications, list) or len(qualifications) != len(
        M03R_V12_EXECUTION_HORIZONS
    ):
        raise M03RV12SeadragonLifecycleError(
            "terminal qualification count drifted from the execution horizons"
        )
    ...
    for value in qualifications:
        ...
        qualified_horizon = qualification.get("horizon_sessions")
        if (
            isinstance(qualified_horizon, bool)
            or not isinstance(qualified_horizon, int)
            or qualified_horizon not in M03R_V12_EXECUTION_HORIZONS
        ...
        _validate_qualification(
            ...
            expected_fold_receipts=fold_evidence_receipts[qualified_horizon],
    ...
    if gate is True:
        if (
            selected != M03R_V12_SELECTED_HORIZON_SESSIONS
            or not isinstance(selected_sha, str)
        ):
            raise M03RV12SeadragonLifecycleError("passed terminal omitted selection")
        ...
        if (
            ...
            raise M03RV12SeadragonLifecycleError("selected qualification drifted")
    elif gate is False:
        if (
            ...
            raise M03RV12SeadragonLifecycleError("failed gate selection drifted")
    else:
        raise M03RV12SeadragonLifecycleError("predictive gate flag is invalid")
    return terminal_sha, {
        "setting_index": row.setting_index,
...
def validate_m03r_v12_predictive_coverage(
    config: M03RV12AttachSupervisorConfig,
    *,
    ...
    if len(owned_pods) != 3:
        raise M03RV12SeadragonLifecycleError(
            "complete v12 Job must retain exactly three owned Pods"
        )
    ...
    for pod in owned_pods:
        ...
        if (
            ...
            raise M03RV12SeadragonLifecycleError(
                "terminal Pods are not one success per v12 completion"
            )
        pod_inventory[index] = {"pod_name": name, "pod_uid": uid}
    if set(pod_inventory) != {0, 1, 2}:
        raise M03RV12SeadragonLifecycleError("terminal Pod coverage drifted")
...
def _cleanup_preactivation_exact(
    *,
    ...
    common._exclusive_json(
        ...
        {
            "schema": ("rl-quant.top2000-dev.m03r-v12-preactivation-cleanup-reads-v1"),
            "binding_receipt_sha256": binding.receipt_sha256,
    ...
    if absence is None:
        common._exclusive_json(
            ...
            {
                "schema": (
                    "rl-quant.top2000-dev.m03r-v12-preactivation-cleanup-"
                    "attach-required-v1"
                ),
        ...
        raise M03RV12ActivationAttachRequired(
    ...
    verification_sha = common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-cleanup-verification-v1",
            "first": first,
            "second": second,
            "delete_attempted": True,
            "delete_retried": False,
    ...
    *,
    ...
    if absence is None:
        common._exclusive_json(
            ...
            {
                "schema": "rl-quant.top2000-dev.m03r-v12-cleanup-attach-required-v1",
                "delete_attempted": True,
                "delete_retried": False,
                "error_type": (
                    None if delete_error is None else type(delete_error).__name__
...
def _prepare_activation(
    *,
    ...
    try:
        fresh = live.get_job()
        if fresh is None:
            raise M03RV12SeadragonLifecycleError("bound suspended Job is absent")
        common._job_identity(
            fresh,
            job_name=config.job_name,
            run_id=config.run_id,
        ...
        if live.get_owned_pods():
            raise M03RV12SeadragonLifecycleError("attach target already owns Pods")
        ...
        readiness_sha = common._exclusive_json(
            root / "readiness.json",
            {
                "schema": "rl-quant.top2000-dev.m03r-v12-supervisor-readiness-v1",
        ...
        arm_sha = common._exclusive_json(
            root / "arm.json",
            {
                "schema": "rl-quant.top2000-dev.m03r-v12-supervisor-arm-v1",
...
def _prepare_capacity_activation(
    *,
    ...
    try:
        fresh = live.get_job()
        if fresh is None:
            raise M03RV12SeadragonLifecycleError("bound capacity Job is absent")
        common._job_identity(
            fresh,
            job_name=config.job_name,
            run_id=config.run_id,
        ...
        common._exclusive_json(
            root / "readiness.json",
            {
                "schema": "rl-quant.top2000-dev.m03r-v12-capacity-readiness-v1",
                "config_sha256": config_sha256,
                "binding_receipt_sha256": binding.receipt_sha256,
                "configured_activation_file_sha256": (
                    config.activation_request_file_sha256
...
def _run_supervisor_inner(
    ...
    *,
    ...
    if _file_sha256(source) != config.lifecycle_source_sha256:
        raise M03RV12SeadragonLifecycleError("lifecycle source hash drifted")
    ...
    if (
        ...
        raise M03RV12SeadragonLifecycleError(
            "v12 config, binding, or activation identity drifted"
        )
    ...
    activation_sha = common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-supervisor-activation-v1",
    ...
    common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-supervisor-launch-success-v1",
    ...
    if terminal_reason == "complete":
        coverage = validate_m03r_v12_predictive_coverage(
    ...
    if terminal_reason != "complete":
        raise M03RV12SeadragonLifecycleError(
            f"v12 predictive Job terminated without completion: {terminal_reason}"
        )
...
def _run_capacity_supervisor_inner(
    ...
    *,
    ...
    source = common._regular_no_symlink(Path(__file__), label="lifecycle source")
    if _file_sha256(source) != config.lifecycle_source_sha256:
        raise M03RV12SeadragonLifecycleError("capacity lifecycle source hash drifted")
    package, authorization = _load_package_authorization(config)
    ...
    arm_sha = common._exclusive_json(
        root / "arm.json",
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-capacity-arm-v1",
            "config_sha256": config_sha256,
            "spawn_process_file_sha256": _file_sha256(process_path),
            "activation_request_sha256": activation.request_sha256,
        },
    ...
    common._exclusive_json(
        root / "activation.json",
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-capacity-activation-v1",
            "arm_file_sha256": arm_sha,
            "activation_request_sha256": activation.request_sha256,
            "activated_job_sha256": common._content_sha256(activated),
            "activation_reconciled_after_transport_error": False,
    ...
    common._exclusive_json(
        root / "launch-success.json",
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-capacity-launch-success-v1",
            "job_name": config.job_name,
            "job_uid": config.job_uid,
            "run_id": config.run_id,
            "parallelism": 1,
    ...
    if not isinstance(terminal_receipt, str):
        raise M03RV12SeadragonLifecycleError("capacity terminal receipt is absent")
    package, authorization = _load_package_authorization(config)
    qualification = M03RV12TwoH100CapacityQualification(
...
def _recover_predictive_supervisor_failure(
    *,
    ...
    """Capture and issue at most one cleanup delete after a v12 child error."""
    ...
    try:
    ...
    except M03RV12ActivationAttachRequired:
        raise
        ...
        if not attach_path.exists():
            common._exclusive_json(
                ...
                {
                    "schema": (
                        "rl-quant.top2000-dev.m03r-v12-supervisor-recovery-"
                        "attach-required-v1"
                    ),
    ...
    try:
        ...
        if not (root / "capacity-supervisor-error.json").exists():
            common._exclusive_json(
                ...
                {
                    "schema": "rl-quant.top2000-dev.m03r-v12-capacity-error-v1",
        ...
        try:
            ...
            common._exclusive_json(
                ...
                {
                    "schema": (
                        "rl-quant.top2000-dev.m03r-v12-capacity-recovery-"
                        "attach-required-v1"
                    ),
    ...
    try:
        ...
        if not error_path.exists():
            common._exclusive_json(
                error_path,
                {
                    "schema": "rl-quant.top2000-dev.m03r-v12-supervisor-error-v1",
                    "error_type": type(exc).__name__,
                    "error": str(exc),
                    "attach_required": False,
                },
...
def _validated_host_tool(path: Path, label: str) -> Path:
    ...
    if not os.access(path, os.X_OK):
        raise M03RV12SeadragonLifecycleError(f"{label} is not executable")
    return path
...
def _validated_launch_success(
    ...
    try:
        ...
        if (
            launch.get("schema")
            != "rl-quant.top2000-dev.m03r-v12-supervisor-launch-success-v1"
            ...
            != "rl-quant.top2000-dev.m03r-v12-supervisor-activation-v1"
            ...
            return None
        return launch
    except (OSError, ValueError, M03RV12SeadragonLifecycleError):
        return None
...
def _validated_capacity_launch_success(
    ...
    try:
        ...
        if (
            launch.get("schema")
            != "rl-quant.top2000-dev.m03r-v12-capacity-launch-success-v1"
            or launch.get("job_name") != config.job_name
            or launch.get("job_uid") != config.job_uid
            or launch.get("run_id") != config.run_id
            ...
            or launch.get("economic_panel_authorized") is not False
            or activation.get("schema")
            != "rl-quant.top2000-dev.m03r-v12-capacity-activation-v1"
            or activation.get("activation_reconciled_after_transport_error")
            is not False
            or activation.get("activation_retried") is not False
        ):
...
def spawn_capacity_supervisor(
    ...
    intent_sha = common._exclusive_json(
        ...
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-capacity-spawn-intent-v1",
            "config_sha256": expected_config_sha256,
    ...
    process_receipt = {
        "schema": "rl-quant.top2000-dev.m03r-v12-capacity-process-v1",
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            common._exclusive_json(
                ...
                {
                    "schema": (
                        "rl-quant.top2000-dev.m03r-v12-capacity-spawn-attach-required-v1"
                    ),
                    "pid": process.pid,
...
def spawn_attach_supervisor(
    ...
    intent_sha = common._exclusive_json(
        root / "spawn-intent.json",
        {
            "schema": "rl-quant.top2000-dev.m03r-v12-supervisor-spawn-intent-v1",
            "config_sha256": expected_config_sha256,
            "command_sha256": command_sha,
            "python_sha256": _file_sha256(python),
            "kubectl_sha256": _file_sha256(kubectl),
    ...
    identity = common._process_identity(process.pid)
    process_receipt = {
        "schema": "rl-quant.top2000-dev.m03r-v12-supervisor-process-v1",
        "spawn_intent_file_sha256": intent_sha,
        "config_sha256": expected_config_sha256,
        "command_sha256": command_sha,
        **identity,
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            common._exclusive_json(
                ...
                {
                    "schema": (
                        "rl-quant.top2000-dev.m03r-v12-spawn-attach-required-v1"
                    ),
                    "pid": process.pid,
                    "process_receipt_file_sha256": _file_sha256(
                        root / "spawn-process.json"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except M03RV12ActivationAttachRequired as exc:
        print(
            json.dumps(
...
__all__ = [
    ...
    "M03RV12ActivationAttachRequired",
    "M03RV12AttachSupervisorConfig",
    "M03RV12CapacityAttachConfig",
    "M03RV12ExpectedCompletion",
    ...
    "validate_m03r_v12_predictive_coverage",
]
