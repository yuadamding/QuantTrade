"""Immutable paired three-setting worker plan for M03R-v11."""
...
M03R_V11_WORKER_PLAN_SCHEMA = "rl-quant.top2000-dev.m03r-v11-worker-plan-v1"
M03R_V11_PANEL_PLAN_SCHEMA = "rl-quant.top2000-dev.m03r-v11-panel-plan-v1"


class M03RV11PredictiveWorkerError(ValueError):
    """The v11 worker, panel, or paired-input identity drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV11PredictiveWorkerError(f"{name} must be a lowercase SHA-256")
    return value


@dataclass(frozen=True, slots=True)
class M03RV11PredictiveWorkerPlan:
    setting_index: int
    setting_id: str
    output_root: str
    cache_path: str
    ...
    schema: str = M03R_V11_WORKER_PLAN_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.fold_count != M03R_V11_PREDICTIVE_SPEC.chronological_fold_count
            or self.expected_world_size != M03R_V11_PREDICTIVE_SPEC.expected_world_size
            ...
            or self.schema != M03R_V11_WORKER_PLAN_SCHEMA
        ):
            raise M03RV11PredictiveWorkerError("v11 worker plan drifted")
...
@dataclass(frozen=True, slots=True)
class M03RV11PredictivePanelPlan:
    workers: tuple[M03RV11PredictiveWorkerPlan, ...]
    ...
    kubernetes_launch_authorized: bool = False
    protocol_sha256: str = M03R_V11_PROTOCOL_SHA256
    schema: str = M03R_V11_PANEL_PLAN_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.kubernetes_launch_authorized
            or self.protocol_sha256 != M03R_V11_PROTOCOL_SHA256
            or self.schema != M03R_V11_PANEL_PLAN_SCHEMA
        ):
            raise M03RV11PredictiveWorkerError("v11 panel plan drifted")
...
__all__ = [
    "M03R_V11_PANEL_PLAN_SCHEMA",
    "M03R_V11_WORKER_PLAN_SCHEMA",
