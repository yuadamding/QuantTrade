"""Causal fixed-rank qualification sleeve for M03R-v15.
...
from rl_quant.training.top2000_m03r_v15_checkpoint import M03RV15LoadedCheckpoint
from rl_quant.training.top2000_m03r_v15_pretraining_runtime import (
    M03RV15BuiltPredictiveBatch,
)

M03R_V15_SIMPLE_SLEEVE_TRACE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-causal-fixed-rank-sleeve-v1"
)


class M03RV15RuntimeError(ValueError):
    """The v15 causal qualification chronology or evidence drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV15RuntimeError(f"{name} must be a lowercase SHA-256")
    return value
...
def _fixed_rank_target(
    ...
    maximum = M03R_V15_PREDICTIVE_SPEC.simple_sleeve_maximum_active_one_way_mass
    mass = min(
        maximum,
    ...
    if (
        ...
        raise M03RV15RuntimeError("v15 rank target violated its weight constraints")
    return requested, common


@dataclass(frozen=True, slots=True)
class M03RV15SimpleSleeveTrace:
    setting_index: int
    setting_id: str
    fold_index: int
    ...
    trace_sha256: str
    simple_sleeve_rule: str = M03R_V15_SIMPLE_SLEEVE_RULE
    ...
    schema: str = M03R_V15_SIMPLE_SLEEVE_TRACE_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or dates != M03R_V15_PREDICTIVE_SPEC.qualification_origins_per_fold
            ...
            != len(M03R_V15_PREDICTIVE_SPEC.evaluation_cost_basis_points)
            ...
            or self.simple_sleeve_rule != M03R_V15_SIMPLE_SLEEVE_RULE
            ...
            or self.schema != M03R_V15_SIMPLE_SLEEVE_TRACE_SCHEMA
            or self.trace_sha256 != _sha256(self.unsigned_payload())
        ):
            raise M03RV15RuntimeError("v15 simple-sleeve trace drifted")
        for name, value in (
            ("checkpoint_file_sha256", self.checkpoint_file_sha256),
            ("checkpoint_model_state_sha256", self.checkpoint_model_state_sha256),
...
def run_m03r_v15_simple_sleeve(
    ...
    *,
    ...
    if (
        ...
        raise M03RV15RuntimeError(
            "v15 checkpoint, batch, sequence, or risk chronology drifted"
        )
    ...
    for row_index, local_origin_tensor in enumerate(local_origins):
        ...
        if observed_mean.numel() < 2:
            raise M03RV15RuntimeError("v15 qualification date has no diagnostic support")
        ...
        if float(policy_return) <= -1.0 or float(benchmark_return) <= -1.0:
            raise M03RV15RuntimeError("v15 qualification chronology lost all value")
        ...
        action_mask = operator.qualified_asset_mask.to(device=raw_mean.device)
        action_weights = operator.base.qualified_weights.to(
            device=raw_mean.device,
            dtype=torch.float64,
        )
        raw_action = raw_mean[action_mask].to(torch.float64)
        residual_action = executable_score[action_mask].to(torch.float64)
        raw_norm = torch.sqrt((action_weights * raw_action.square()).sum())
        residual_norm = torch.sqrt(
            (action_weights * residual_action.square()).sum()
        )
        ...
        if target_operator.receipt_sha256 == operator.receipt_sha256 and not torch.equal(
            ...
            raise M03RV15RuntimeError("v15 residual operator identity collision")
    ...
    net_by_cost = tuple(
        ...
        for basis_points in M03R_V15_PREDICTIVE_SPEC.evaluation_cost_basis_points
    )
    ...
    if scale.numel() == 0:
        raise M03RV15RuntimeError("v15 qualification has no selected scale values")
    ...
    cap = M03R_V15_PREDICTIVE_SPEC.simple_sleeve_maximum_active_one_way_mass
    active_mass_tensor = arrays[7]
    provisional = M03RV15SimpleSleeveTrace(
