"""Untouched-tail fold qualification for an exact reloaded M03R-v12 checkpoint."""
...
from rl_quant.training.top2000_m03r_v12_runtime import run_m03r_v12_simple_sleeve


class M03RV12FoldQualificationError(ValueError):
    """The v12 untouched-tail fold evaluation lineage drifted."""
...
def _slice_sleeve_sequence(
    ...
    *,
    ...
    if local_stop_exclusive - local_start != M03R_V9_QUALIFICATION_ORIGINS + 1:
        raise M03RV12FoldQualificationError(
            "v12 sleeve chronology must contain exactly 64 states"
        )
...
def evaluate_m03r_v12_loaded_qualification_fold(
    ...
    *,
    ...
    if (
        ...
        raise M03RV12FoldQualificationError(
            "v12 worker, checkpoint, policy, cache, risk, or horizon drifted"
        )
    ...
    if batch.residual_operators is None:
        raise M03RV12FoldQualificationError(
            "v12 qualification did not retain executable residual operators"
        )
    ...
    distributions = tuple(
        M03RV12AlphaDistribution(
            ...
            selected_mean=batch.predicted_mean[index, :, horizon_index].unsqueeze(0),
            selected_scale=torch.exp(
                batch.predicted_log_scale[index, :, horizon_index]
            ).unsqueeze(0),
        )
        for index in range(M03R_V9_QUALIFICATION_ORIGINS)
    )
    selected_operators = tuple(
        batch.residual_operators[index * len(M03R_V12_HORIZONS) + horizon_index]
        for index in range(M03R_V9_QUALIFICATION_ORIGINS)
    )
    ...
    return build_m03r_v12_fold_qualification_lineage(loaded, batch, trace)
