"""Joint six-fold block inference and fail-closed stop gate for M03R-v15."""
...
from rl_quant.training.top2000_m03r_v15_runtime import M03RV15SimpleSleeveTrace

M03R_V15_BOOTSTRAP_PLAN_SCHEMA = "rl-quant.top2000-dev.m03r-v15-bootstrap-plan-v1"
M03R_V15_QUALIFICATION_SCHEMA = "rl-quant.top2000-dev.m03r-v15-qualification-v1"


class M03RV15SelectionError(ValueError):
    """The v15 chronology, bootstrap, or predictive stop gate drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV15SelectionError(f"{name} must be a lowercase SHA-256")
    return value
...
def _draw_indices(
    ...
    *,
    ...
    generator.manual_seed(
        M03R_V15_PREDICTIVE_SPEC.bootstrap_seed + block_sessions * 1_000_003
    )
    ...
    for fold_length in fold_lengths:
        ...
        starts = torch.randint(
            fold_length,
            (M03R_V15_PREDICTIVE_SPEC.bootstrap_replicates, blocks),
            generator=generator,
            dtype=torch.int64,
        )
        local = (starts.unsqueeze(-1) + block_offset) % fold_length
        rows.append(
            local.reshape(M03R_V15_PREDICTIVE_SPEC.bootstrap_replicates, -1)[
                :, :fold_length
            ]
            + offset
        )
...
class M03RV15BootstrapPlan:
    chronology_sha256: str
    fold_lengths: tuple[int, ...]
    draw_sha256_by_block: tuple[str, str, str]
    block_sessions: tuple[int, int, int] = (10, 21, 30)
    replicates: int = M03R_V15_PREDICTIVE_SPEC.bootstrap_replicates
    bootstrap_seed: int = M03R_V15_PREDICTIVE_SPEC.bootstrap_seed
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_BOOTSTRAP_PLAN_SCHEMA

    def validate(self) -> None:
        if (
            self.fold_lengths != (63, 63, 63, 63, 63, 63)
            or self.block_sessions != (10, 21, 30)
            or self.replicates != M03R_V15_PREDICTIVE_SPEC.bootstrap_replicates
            or self.bootstrap_seed != M03R_V15_PREDICTIVE_SPEC.bootstrap_seed
            ...
            or self.schema != M03R_V15_BOOTSTRAP_PLAN_SCHEMA
        ):
            raise M03RV15SelectionError("v15 bootstrap plan drifted")
...
def build_m03r_v15_bootstrap_plan(
    origin_indices_by_fold: tuple[torch.Tensor, ...],
) -> M03RV15BootstrapPlan:
    if len(origin_indices_by_fold) != 6:
        raise M03RV15SelectionError("v15 bootstrap requires six folds")
    chronology: list[int] = []
    ...
    for row in origin_indices_by_fold:
        if (
            ...
            raise M03RV15SelectionError("v15 fold chronology is not ordered/disjoint")
...
@dataclass(frozen=True, slots=True)
class M03RV15PredictiveQualification:
    setting_index: int
    setting_id: str
    ...
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_QUALIFICATION_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.protocol_sha256 != M03R_V15_PROTOCOL_SHA256
            or self.schema != M03R_V15_QUALIFICATION_SCHEMA
        ):
            raise M03RV15SelectionError("v15 predictive qualification drifted")
...
def qualify_m03r_v15_predictive_candidate(
    traces: tuple[M03RV15SimpleSleeveTrace, ...],
    bootstrap: M03RV15BootstrapPlan,
) -> M03RV15PredictiveQualification:
    if len(traces) != 6:
        raise M03RV15SelectionError("v15 qualification requires six fold traces")
    ...
    if (
        ...
        raise M03RV15SelectionError("v15 fold identities or chronology drifted")
    gross = torch.cat(
        tuple(
    ...
    provisional = M03RV15PredictiveQualification(
        setting_index=setting,
        setting_id=M03R_V15_SETTING_IDS[setting],
        ...
        positive_date_fraction_fold_count=sum(
            value > M03R_V15_PREDICTIVE_SPEC.minimum_positive_ic_date_fraction
    ...
    result = M03RV15PredictiveQualification(
        **{
            **asdict(provisional),
            "passed": passed,
...
__all__ = [
    "M03R_V15_BOOTSTRAP_PLAN_SCHEMA",
    "M03R_V15_QUALIFICATION_SCHEMA",
    "M03RV15BootstrapPlan",
    "M03RV15PredictiveQualification",
    "M03RV15SelectionError",
