"""Causal h3 target and direct-score batch construction for M03R-v14."""
...
from rl_quant.training.top2000_m03r_v14_fold import M03R_V14_MINIMUM_LOCAL_ORIGIN
from rl_quant.training.top2000_m03r_v14_objective import M03RV14PredictiveBatch
...
M03R_V14_BUILT_BATCH_SCHEMA = "rl-quant.top2000-dev.m03r-v14-built-batch-v1"


class M03RV14PretrainingRuntimeError(ValueError):
    """V14 failed its full-context direct-h3 batch boundary."""
...
class M03RV14BuiltPredictiveBatch:
    objective: M03RV14PredictiveBatch
    ...
    target_residual_operators: tuple[M03RV14ResidualOperator, ...]
    action_residual_operators: tuple[M03RV14ResidualOperator, ...]
    protocol_sha256: str = M03R_V14_PROTOCOL_SHA256
    schema: str = M03R_V14_BUILT_BATCH_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.schema != M03R_V14_BUILT_BATCH_SCHEMA
        ):
            raise M03RV14PretrainingRuntimeError("v14 built batch drifted")
...
def build_m03r_v14_batch_from_origin_states(
    ...
    *,
    ...
    if (
        not isinstance(policy, Top2000M03RV14PredictivePolicy)
        or policy.v14_setting != setting
        ...
        or int(local_origin_indices[0]) < M03R_V14_MINIMUM_LOCAL_ORIGIN
    ):
        raise M03RV14PretrainingRuntimeError(
            "v14 policy, state, origin, or full-context geometry drifted"
        )
    ...
    if (
        ...
        raise M03RV14PretrainingRuntimeError("v14 origins leave the declared split")
    ...
    if (
        ...
        raise M03RV14PretrainingRuntimeError(
            "v14 exposure and sequence asset axes differ"
        )
    ...
    target_operators: list[M03RV14ResidualOperator] = []
    action_operators: list[M03RV14ResidualOperator] = []
    cash_index = sequence.initial_ledger.cash_index
    horizon = M03R_V14_SELECTED_HORIZON_SESSIONS
    for row_index, local_origin in enumerate(local_origin_indices.tolist()):
        ...
        if local_stop > sequence.asset_returns.shape[0]:
            raise M03RV14PretrainingRuntimeError("v14 h3 target path is unavailable")
        stock = torch.log1p(
        ...
        target_operator = build_m03r_v14_residual_operator(
            available_mask=label_available,
            **operator_kwargs,
        )
        action_operator = build_m03r_v14_residual_operator(
            available_mask=action_available,
            **operator_kwargs,
        )
        residual = apply_m03r_v14_residual_operator(
            (stock - benchmark).detach(), target_operator
        )
        executable = apply_m03r_v14_residual_operator(raw_mean, action_operator)
    ...
    result = M03RV14BuiltPredictiveBatch(
        objective=objective,
