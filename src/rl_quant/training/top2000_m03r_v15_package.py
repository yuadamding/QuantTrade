"""Content-bound local package plan for the M03R-v15 predictive panel."""
...
from rl_quant.training.top2000_m03r_v15_fold import M03RV15PanelEpisodeSchedule
from rl_quant.training.top2000_m03r_v15_predictive_worker import (
    M03RV15PredictivePanelPlan,
    M03RV15PredictiveWorkerPlan,
)

M03R_V15_PACKAGE_SCHEMA = "rl-quant.top2000-dev.m03r-v15-predictive-package-v1"
M03R_V15_PACKAGE_FILE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-predictive-package-file-v1"
)
M03R_V15_EXECUTION_AUTHORIZATION_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-predictive-execution-authorization-v1"
)
M03R_V15_PACKAGE_SOURCE_PYTHONPATH = "/mnt/package/source/src"
M03R_V15_RUNTIME_ENTRYPOINT = "rl_quant.workflows.top2000_m03r_v15_predictive"
_MAX_PACKAGE_PLAN_BYTES = 4 * 1024**2
_DIGEST = re.compile(r"^[0-9a-f]{64}$")
_IMAGE = re.compile(r"^[^@\s]+@sha256:([0-9a-f]{64})$")


class M03RV15PackageError(ValueError):
    """The v15 source, data, risk, image, or panel package drifted."""
...
def _digest(name: str, value: str) -> str:
    if _DIGEST.fullmatch(value) is None:
        raise M03RV15PackageError(f"{name} must be one lowercase SHA-256")
    return value


@dataclass(frozen=True, slots=True)
class M03RV15PackageArtifacts:
    ...
    def validate(self) -> None:
        ...
        if match is None or match.group(1) != self.image_digest_sha256:
            raise M03RV15PackageError("v15 image reference must be digest pinned")


@dataclass(frozen=True, slots=True)
class M03RV15PackagePlan:
    artifacts: M03RV15PackageArtifacts
    schedule: M03RV15PanelEpisodeSchedule
    panel: M03RV15PredictivePanelPlan
    plan_directory: str
    package_plan_sha256: str
    source_pythonpath: str = M03R_V15_PACKAGE_SOURCE_PYTHONPATH
    ...
    schema: str = M03R_V15_PACKAGE_SCHEMA
    ...
    def validate(self) -> None:
        ...
        for completion, worker in enumerate(self.panel.workers):
            if (
                ...
                raise M03RV15PackageError(
                    "v15 worker does not bind exact package artifacts"
                )
        if (
            ...
            or self.source_pythonpath != M03R_V15_PACKAGE_SOURCE_PYTHONPATH
            ...
            or self.schema != M03R_V15_PACKAGE_SCHEMA
            or self.package_plan_sha256 != _sha256(self.unsigned_payload())
        ):
            raise M03RV15PackageError("v15 package plan identity drifted")


@dataclass(frozen=True, slots=True)
class M03RV15ExecutionAuthorization:
    package_plan_sha256: str
    ...
    worker_source_sha256: str
    image_reference: str
    runtime_entrypoint: str = M03R_V15_RUNTIME_ENTRYPOINT
    ...
    promotion_eligible: bool = False
    protocol_sha256: str = M03R_V15_PROTOCOL_SHA256
    schema: str = M03R_V15_EXECUTION_AUTHORIZATION_SCHEMA

    def validate(self, package: M03RV15PackagePlan) -> None:
        package.validate()
        for name in (
            "package_plan_sha256",
        ...
        if (
            ...
            or self.runtime_entrypoint != M03R_V15_RUNTIME_ENTRYPOINT
            ...
            or self.promotion_eligible
            or self.protocol_sha256 != M03R_V15_PROTOCOL_SHA256
            or self.schema != M03R_V15_EXECUTION_AUTHORIZATION_SCHEMA
        ):
            raise M03RV15PackageError("v15 execution authorization drifted")
...
def build_m03r_v15_package_plan(
    artifacts: M03RV15PackageArtifacts,
    schedule: M03RV15PanelEpisodeSchedule,
) -> M03RV15PackagePlan:
    artifacts.validate()
    ...
    workers = tuple(
        ...
        for index, setting_id in enumerate(M03R_V15_SETTING_IDS)
    )
    panel = M03RV15PredictivePanelPlan(workers)
    provisional = M03RV15PackagePlan(
        artifacts=artifacts,
        schedule=schedule,
        panel=panel,
...
def write_m03r_v15_package_plan(path: str | Path, plan: M03RV15PackagePlan) -> str:
    plan.validate()
    target = Path(path)
    if target.exists() or target.is_symlink():
        raise M03RV15PackageError("v15 package-plan target already exists")
    encoded = _canonical(
        {
            "schema": M03R_V15_PACKAGE_FILE_SCHEMA,
            "plan": asdict(plan),
...
def load_m03r_v15_package_plan(
    path: str | Path,
    *,
    ...
    try:
        ...
        raise M03RV15PackageError("v15 package plan is unavailable") from exc
    try:
        ...
        if (
            ...
            raise M03RV15PackageError("v15 package plan type or size drifted")
        ...
        if (
            ...
            raise M03RV15PackageError("v15 package plan changed while read")
    finally:
        os.close(descriptor)
    if (
        ...
        raise M03RV15PackageError("v15 package plan file hash drifted")
    try:
        payload = json.loads(raw)
        row = dict(payload["plan"])
        row["artifacts"] = M03RV15PackageArtifacts(**row["artifacts"])
        schedule_row = dict(row["schedule"])
        schedule_row["fold_geometry_sha256"] = tuple(
            schedule_row["fold_geometry_sha256"]
        )
        row["schedule"] = M03RV15PanelEpisodeSchedule(**schedule_row)
        ...
        for worker_row in panel_row["workers"]:
            ...
            workers.append(M03RV15PredictiveWorkerPlan(**worker_payload))
        panel_row["workers"] = tuple(workers)
        row["panel"] = M03RV15PredictivePanelPlan(**panel_row)
        plan = M03RV15PackagePlan(**row)
    except (KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
        raise M03RV15PackageError("v15 package plan is malformed") from exc
    if (
        payload.get("schema") != M03R_V15_PACKAGE_FILE_SCHEMA
        or payload.get("package_plan_sha256") != plan.package_plan_sha256
    ):
        raise M03RV15PackageError("v15 package plan receipt drifted")
    plan.validate()
    return plan


def write_m03r_v15_execution_authorization(
    path: str | Path,
    ...
    target = Path(path)
    if target.exists() or target.is_symlink():
        raise M03RV15PackageError("v15 execution authorization already exists")
    encoded = _canonical(
        {
...
def load_m03r_v15_execution_authorization(
    path: str | Path,
    *,
    expected_file_sha256: str,
    package: M03RV15PackagePlan,
    ...
    try:
        ...
        raise M03RV15PackageError(
            "v15 execution authorization is unavailable"
        ) from exc
    try:
        before = os.fstat(descriptor)
        if (
            ...
            raise M03RV15PackageError("v15 execution authorization is invalid")
        raw = os.read(descriptor, before.st_size + 1)
        after = os.fstat(descriptor)
        if (
            before.st_dev,
            ...
            raise M03RV15PackageError(
                "v15 execution authorization changed while read"
            )
    finally:
        os.close(descriptor)
    if len(raw) != before.st_size or hashlib.sha256(raw).hexdigest() != expected_file_sha256:
        raise M03RV15PackageError("v15 execution authorization hash drifted")
    try:
        payload = json.loads(raw)
        authorization = M03RV15ExecutionAuthorization(
        ...
        raise M03RV15PackageError(
            "v15 execution authorization is malformed"
        ) from exc
    authorization.validate(package)
    if payload.get("receipt_sha256") != authorization.receipt_sha256:
        raise M03RV15PackageError("v15 execution authorization receipt drifted")
    return authorization


__all__ = [
    "M03R_V15_PACKAGE_FILE_SCHEMA",
    "M03R_V15_PACKAGE_SCHEMA",
    "M03R_V15_EXECUTION_AUTHORIZATION_SCHEMA",
    "M03R_V15_PACKAGE_SOURCE_PYTHONPATH",
