"""Exact checkpoint-to-trace fold qualification runtime for M03R-v15."""
...
from rl_quant.training.top2000_m03r_v15_runtime import (
...
class M03RV15QualificationRuntimeError(ValueError):
...
def _daily_log_returns(
    ...
    receipt = hashlib.sha256(
        json.dumps(
            {
                "schema": "rl-quant.top2000-dev.m03r-v15-past-log-returns-v1",
...
def build_m03r_v15_qualification_risk_state(
    cache: Top2000VerifiedDevelopmentCache,
    ...
    *,
    ...
    """Qualify one V15 fold's exact origin set and transfer it once."""
...
class M03RV15FoldQualificationResult:
    loaded_checkpoint: M03RV15LoadedCheckpoint
    batch: M03RV15BuiltPredictiveBatch
    trace: M03RV15SimpleSleeveTrace
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise M03RV15QualificationRuntimeError(
                "v15 fold qualification receipt chain drifted"
            )


def run_m03r_v15_fold_qualification(
    cache: Top2000VerifiedDevelopmentCache,
    geometry: M03RV15FoldGeometry,
    risk_source: M03RV9MaterializedRiskSource,
    ...
    *,
    ...
    if (
        ...
        or loaded.setting_index != policy.v15_setting.setting_index
        ...
        raise M03RV15QualificationRuntimeError(
            "v15 cache, fold, checkpoint, model, or risk identity drifted"
        )
    ...
    trace = run_m03r_v15_simple_sleeve(
        bound_sequence,
        batch,
        risk_state,
        loaded,
    ...
    result = M03RV15FoldQualificationResult(loaded, batch, trace)
    result.validate()
    return result
...
__all__ = [
    "M03RV15FoldQualificationResult",
    "M03RV15QualificationRuntimeError",
