"""Real-cache structural qualification for every M03R-v15 scheduled origin."""
...
M03R_V15_STRUCTURAL_PREFLIGHT_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-real-cache-structural-preflight-v1"
)
M03R_V15_STRUCTURAL_PREFLIGHT_FILE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v15-real-cache-structural-preflight-file-v1"
)
_MAX_PREFLIGHT_BYTES = 4 * 1024**2


class M03RV15PreflightError(ValueError):
    """The exact data-dependent v15 structural gate failed or drifted."""
...
def _digest(name: str, value: str) -> str:
    if len(value) != 64 or any(
        ...
        raise M03RV15PreflightError(f"{name} must be a lowercase SHA-256")
    return value
...
class M03RV15StructuralPreflightReceipt:
    cache_sha256: str
    ...
    schema: str = M03R_V15_STRUCTURAL_PREFLIGHT_SCHEMA
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            or self.schema != M03R_V15_STRUCTURAL_PREFLIGHT_SCHEMA
        ):
            raise M03RV15PreflightError("v15 structural preflight receipt drifted")
        for name, value in (
            ...
            ("risk_artifact_file_sha256", self.risk_artifact_file_sha256),
            (
            ...
            ("projector_binding_sha256", self.projector_binding_sha256),
    ...
    def validate_for_package(
        ...
        *,
        ...
        if any(getattr(self, name) != value for name, value in expected.items()):
            raise M03RV15PreflightError(
                "v15 structural preflight does not bind the exact package"
            )
...
def run_m03r_v15_structural_preflight(
    ...
    *,
    ...
    if (
        ...
        raise M03RV15PreflightError(
            "v15 real cache, risk source, or asset axis drifted"
        )
    ...
    for origin in _scheduled_origins():
        ...
        target = build_m03r_v15_residual_operator(
        ...
        action = build_m03r_v15_residual_operator(
    ...
    geometries = render_m03r_v15_fold_geometries(1001)
    origins = _scheduled_origins()
    result = M03RV15StructuralPreflightReceipt(
        cache_sha256=cache.cache_sha256,
        cache_manifest_sha256=_digest(
            "cache_manifest_sha256", cache_manifest_sha256
        ),
        asset_axis_sha256=cache.action_hash,
...
def write_m03r_v15_structural_preflight(
    path: str | Path,
    receipt: M03RV15StructuralPreflightReceipt,
) -> str:
    receipt.validate()
    target = Path(path)
    if target.exists() or target.is_symlink():
        raise M03RV15PreflightError("v15 preflight target already exists")
    encoded = (
        json.dumps(
            {
                "schema": M03R_V15_STRUCTURAL_PREFLIGHT_FILE_SCHEMA,
...
def load_m03r_v15_structural_preflight(
    path: str | Path,
    *,
    ...
    try:
        ...
        raise M03RV15PreflightError("v15 preflight file is unavailable") from exc
    try:
        ...
        if (
            ...
            raise M03RV15PreflightError("v15 preflight file type or size drifted")
        ...
        if (
            ...
            raise M03RV15PreflightError("v15 preflight file changed while read")
    finally:
        os.close(descriptor)
    if len(raw) != before.st_size or hashlib.sha256(raw).hexdigest() != expected_file_sha256:
        raise M03RV15PreflightError("v15 preflight file hash drifted")
    try:
        payload = json.loads(raw)
        row = dict(payload["receipt"])
        row["fold_geometry_sha256"] = tuple(row["fold_geometry_sha256"])
        receipt = M03RV15StructuralPreflightReceipt(**row)
    except (KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
        raise M03RV15PreflightError("v15 preflight file is malformed") from exc
    if (
        payload.get("schema") != M03R_V15_STRUCTURAL_PREFLIGHT_FILE_SCHEMA
        or payload.get("receipt_sha256") != expected_receipt_sha256
        or receipt.receipt_sha256 != expected_receipt_sha256
    ):
        raise M03RV15PreflightError("v15 preflight receipt drifted")
    return receipt
