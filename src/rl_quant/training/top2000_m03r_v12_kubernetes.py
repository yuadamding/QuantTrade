"""Suspended Seadragon Jobs for M03R-v12 predictive qualification only."""
...
M03R_V12_KUBERNETES_LIVE_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v12-live-kubernetes-evidence-v1"
)
M03R_V12_CAPACITY_QUALIFICATION_SCHEMA = (
    "rl-quant.top2000-dev.m03r-v12-two-h100-capacity-qualification-v1"
)
M03R_V12_RENDERED_JOB_SCHEMA = "rl-quant.top2000-dev.m03r-v12-rendered-job-v1"
M03R_V12_WORKER_ARGV_PREFIX = (
    ...
    "--nproc-per-node=2",
    "-m",
    "rl_quant.workflows.top2000_m03r_v12_predictive",
)
M03R_V12_STATIC_ARGV_PREFIX = (
    "/opt/conda/envs/quanttrade/bin/python",
    "-m",
    "rl_quant.workflows.top2000_m03r_v12_static_validate",
)


class M03RV12KubernetesError(ValueError):
    """The v12 manifest, live evidence, or capacity receipt drifted."""
...
def _digest(name: str, value: str) -> None:
    if len(value) != 64 or any(
        ...
        raise M03RV12KubernetesError(f"{name} must be a lowercase SHA-256")


@dataclass(frozen=True, slots=True)
class M03RV12LiveEvidence:
    observed_at_utc: str
    ...
    schema: str = M03R_V12_KUBERNETES_LIVE_SCHEMA
    ...
    def __post_init__(self) -> None:
        try:
            ...
            raise M03RV12KubernetesError("live timestamp must be ISO-8601") from exc
        if observed.tzinfo is None or observed.utcoffset() != UTC.utcoffset(observed):
            raise M03RV12KubernetesError("live timestamp must carry UTC timezone")
        if (
            self.schema != M03R_V12_KUBERNETES_LIVE_SCHEMA
            ...
            raise M03RV12KubernetesError("v12 live Kubernetes evidence is invalid")
    ...
    def require_fresh(self, *, now_utc: datetime, max_age_seconds: int = 300) -> None:
        if now_utc.tzinfo is None:
            raise M03RV12KubernetesError("now_utc must be timezone-aware")
        observed = datetime.fromisoformat(self.observed_at_utc)
        age = (now_utc.astimezone(UTC) - observed.astimezone(UTC)).total_seconds()
        if age < 0 or age > max_age_seconds:
            raise M03RV12KubernetesError("live Kubernetes evidence is stale")


def build_m03r_v12_live_evidence(
    *,
    observed_at_utc: str,
    rbac: M03RV7KubernetesRBACEvidence,
    protected_or_other_committed_h100_count: int,
    ...
    payload = {
        "schema": M03R_V12_KUBERNETES_LIVE_SCHEMA,
...
@dataclass(frozen=True, slots=True)
class M03RV12TwoH100CapacityQualification:
    ...
    schema: str = M03R_V12_CAPACITY_QUALIFICATION_SCHEMA
    ...
    def validate_for(
        ...
        if (
            ...
            or self.schema != M03R_V12_CAPACITY_QUALIFICATION_SCHEMA
        ):
            raise M03RV12KubernetesError("capacity receipt does not bind this package")


@dataclass(frozen=True, slots=True)
class M03RV12RenderedJob:
    ...
    schema: str = M03R_V12_RENDERED_JOB_SCHEMA
...
def bind_m03r_v12_admitted_suspended_job(
    *,
    ...
    """Bind a v12 rendered Job through the reviewed exact v7 admission contract.
    ...
    if (
        rendered.schema != M03R_V12_RENDERED_JOB_SCHEMA
        ...
        raise M03RV12KubernetesError("rendered v12 Job identity drifted before binding")
    if rendered.mode == "static":
        ...
        try:
            return bind_m03r_v12_static_admitted_suspended_job(
                rendered=rendered,
            ...
            raise M03RV12KubernetesError(
                "v12 static admitted suspended Job failed the strict contract"
            ) from exc
    ...
    try:
        ...
        raise M03RV12KubernetesError(
            "v12 admitted suspended Job failed the shared strict contract"
        ) from exc
...
def _render(
    *,
    ...
    live: M03RV12LiveEvidence,
    template: M03RV7KubernetesTemplateConfig,
    now_utc: datetime,
    mode: Literal["static", "capacity", "predictive"],
    capacity: M03RV12TwoH100CapacityQualification | None,
) -> M03RV12RenderedJob:
    package.validate()
    authorization.validate(package)
    ...
    if authorization.package_plan_file_sha256 != package_plan_file_sha256:
        raise M03RV12KubernetesError("v12 authorization and package-plan file disagree")
    ...
    if (
        ...
        raise M03RV12KubernetesError("package and PVC mount identities drifted")
    completions = 3 if mode == "predictive" else 1
    if mode != "static" and live.allowed_parallelism <= 0:
        raise M03RV12KubernetesError("live H100 cap does not allow one worker")
    ...
    if parallelism * gpus_per_completion > 6:
        raise M03RV12KubernetesError("predictive H100 request ceiling drifted")
    labels = {
        "app.kubernetes.io/name": (
            "quanttrade-m03r-v12-static"
            if mode == "static"
            else "quanttrade-m03r-v12-predictive"
        ),
    ...
    argv = [
        *(
            M03R_V12_STATIC_ARGV_PREFIX
            if mode == "static"
            else M03R_V12_WORKER_ARGV_PREFIX
        ),
        "--package-plan",
    ...
    phase = {
        "static": "v12-static",
        "capacity": "v12-capacity",
        "predictive": "v12-predictive",
    }[mode]
...
def render_m03r_v12_suspended_static_job(
    *,
    package: M03RV12PackagePlan,
    authorization: M03RV12ExecutionAuthorization,
    package_plan_file_sha256: str,
...
def render_m03r_v12_suspended_capacity_job(
    *,
    package: M03RV12PackagePlan,
    authorization: M03RV12ExecutionAuthorization,
    package_plan_file_sha256: str,
...
def render_m03r_v12_suspended_predictive_job(
    *,
    package: M03RV12PackagePlan,
    authorization: M03RV12ExecutionAuthorization,
    package_plan_file_sha256: str,
    authorization_file_sha256: str,
    capacity: M03RV12TwoH100CapacityQualification,
    live: M03RV12LiveEvidence,
    template: M03RV7KubernetesTemplateConfig,
    now_utc: datetime,
) -> M03RV12RenderedJob:
...
__all__ = [
    ...
    "bind_m03r_v12_admitted_suspended_job",
