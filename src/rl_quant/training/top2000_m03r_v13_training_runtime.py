"""Executable paired fold-update runtime for M03R-v13 predictive training."""
...
from rl_quant.training.top2000_m03r_v13_pretraining_step import (
    M03RV13AlphaStepReceipt,
    train_m03r_v13_predictive_batch_update,
)


class M03RV13TrainingRuntimeError(ValueError):
    """The v13 executable fold update drifted before mutation."""


@dataclass(frozen=True, slots=True)
class M03RV13FoldUpdateResult:
    update_plan: M03RV13TrainingUpdatePlan
    paired_input: M03RV13PairedInputBinding
    step: M03RV13AlphaStepReceipt
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise M03RV13TrainingRuntimeError("v13 fold-update receipt chain drifted")


def move_and_bind_m03r_v13_sequence(
...
def run_m03r_v13_pretraining_fold_update(
    ...
    *,
    ...
    device: torch.device,
) -> M03RV13FoldUpdateResult:
    """Build and execute one setting-neutral, full-context v13 update."""
    ...
    if (
        policy.v13_setting.setting_index != worker.setting_index
        ...
        raise M03RV13TrainingRuntimeError(
            "v13 worker, schedule, cache, risk, rank, fold, or initial state drifted"
        )
    update_plan = render_m03r_v13_training_update_plan(
        schedule,
        geometry,
    ...
    paired_input = build_m03r_v13_paired_input_binding(
        update_plan,
    ...
    step = train_m03r_v13_predictive_batch_update(
        policy,
        batch,
        optimizer,
        partition,
    ...
    result = M03RV13FoldUpdateResult(update_plan, paired_input, step)
    result.validate()
    return result
