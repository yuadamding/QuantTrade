"""Exact checkpoint-to-trace fold qualification runtime for M03R-v14."""
...
from rl_quant.training.top2000_m03r_v14_fold import M03RV14FoldGeometry
...
from rl_quant.training.top2000_m03r_v14_runtime import (
...
class M03RV14QualificationRuntimeError(ValueError):
...
def _daily_log_returns(
    ...
    receipt = hashlib.sha256(
        json.dumps(
            {
                "schema": "rl-quant.top2000-dev.m03r-v14-past-log-returns-v1",
...
def build_m03r_v14_qualification_risk_state(
    cache: Top2000VerifiedDevelopmentCache,
    ...
    *,
    ...
    """Qualify one V14 fold's exact origin set and transfer it once."""
...
class M03RV14FoldQualificationResult:
    loaded_checkpoint: M03RV14LoadedCheckpoint
    batch: M03RV14BuiltPredictiveBatch
    trace: M03RV14SimpleSleeveTrace
    ...
    def validate(self) -> None:
        ...
        if (
            ...
            raise M03RV14QualificationRuntimeError(
                "v14 fold qualification receipt chain drifted"
            )


def run_m03r_v14_fold_qualification(
    cache: Top2000VerifiedDevelopmentCache,
    geometry: M03RV14FoldGeometry,
    risk_source: M03RV9MaterializedRiskSource,
    ...
    *,
    ...
    if (
        ...
        or loaded.setting_index != policy.v14_setting.setting_index
        ...
        raise M03RV14QualificationRuntimeError(
            "v14 cache, fold, checkpoint, model, or risk identity drifted"
        )
    ...
    trace = run_m03r_v14_simple_sleeve(
        bound_sequence,
        batch,
        risk_state,
        loaded,
    ...
    result = M03RV14FoldQualificationResult(loaded, batch, trace)
    result.validate()
    return result
...
__all__ = [
    "M03RV14FoldQualificationResult",
    "M03RV14QualificationRuntimeError",
