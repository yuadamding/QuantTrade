...
class RawSecondPortfolioEnv:
    ...
    def _execute(self, requested: torch.Tensor, decision: int, end: int) -> tuple[list[SecondFill], Decimal]:
        ...
        for asset, rows in market.items():
            ...
            for stamp, values, _ in rows:
                if decision < stamp and stamp + 1000 <= end:
        ...
        for stamp, bars in sorted(events.items()):
            if _session(stamp) != self.book.action_date:
                ...
                for event in self.splits:
                    if event.effective_date == self.book.action_date:
    ...
    def step(self, action: ActionBatch) -> TransitionBatch:
        ...
        with localcontext() as context:
            context.prec = 34
            fills, fees = self._execute(requested[0], decision, end)
            self.index += 1
