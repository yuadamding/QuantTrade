...
| v16 holding-aligned selection | Current local v5 successor comparing h21, h30, and reference-Hold-30 value on common 30-session support; selection-only with causal action masks, self-financing signal cohorts, freshly derived risk repair, balanced blocks, package-owned structural materialization, terminal-checkpoint qualification, fold-preserving aggregate, exact two-H100 worker, and suspended receipt-gated Job renderers | Build and validate the real-data package, then obtain same-image static, server-dry-run, admission, and exact-shape capacity evidence before scientific activation |
