...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an action-conditioned Q model on second-context gold datasets.")
...
def main() -> int:
    ...
    from rl_quant.second_context_transformer import (
        ...
        evaluate_second_context_policy,
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % max(1, args.epochs // 5) == 0:
            val_metrics = evaluate_second_context_policy(val, model, device=device, reward_scale=args.reward_scale)
            avg_loss = sum(losses) / max(len(losses), 1)
            print(f"epoch={epoch + 1} loss={avg_loss:.4f} val_total={val_metrics['total_return']:.6f}")
    train_metrics = evaluate_second_context_policy(train, model, device=device, reward_scale=args.reward_scale)
    val_metrics = evaluate_second_context_policy(val, model, device=device, reward_scale=args.reward_scale)
    test_metrics = evaluate_second_context_policy(test, model, device=device, reward_scale=args.reward_scale)
    run_name = args.run_name or f"second_context_{datetime.now():%Y%m%d_%H%M%S}"
