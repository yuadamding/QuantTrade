...
def test_qualification_batch_binds_explicit_downward_api_default() -> None:
    ...
    environment = batch.manifest["spec"]["template"]["spec"]["containers"][0][
        "env"
    ]
    completion_index = next(
        row for row in environment if row["name"] == "JOB_COMPLETION_INDEX"
    )
    assert completion_index == {
    ...
    first, second = _admitted_reads(batch)
    for admitted in (first, second):
        admitted_environment = admitted["spec"]["template"]["spec"]["containers"][
            0
        ]["env"]
        admitted_completion_index = next(
            row
            for row in admitted_environment
            if row["name"] == "JOB_COMPLETION_INDEX"
        )
        admitted_completion_index["valueFrom"]["fieldRef"]["apiVersion"] = "v1"
    binding = bind_m03r_v7_top2000_admitted_suspended_job(
        rendered=batch,
        first_read=first,
        second_read=second,
...
def test_suspended_indexed_manifest_uses_proven_two_h100_pool_profile() -> None:
    ...
    completion_index = next(
        item for item in container["env"] if item["name"] == "JOB_COMPLETION_INDEX"
    )
    assert completion_index["valueFrom"]["fieldRef"] == {
        "apiVersion": "v1",
        "fieldPath": (
