...
from rl_quant.protocol.hold30_alpha_m03r_v14_top2000_dev import M03R_V14_SETTING_IDS
...
def _worker(index: int) -> M03RV14PredictiveWorkerPlan:
    return M03RV14PredictiveWorkerPlan(
        setting_index=index,
        setting_id=M03R_V14_SETTING_IDS[index],
        output_root=f"/approved/v14/setting-{index}",
...
def test_v14_worker_binds_variable_equal_epoch_fold_updates() -> None:
    worker = _worker(0)
    worker.validate()
    assert M03R_V14_FOLD_UPDATE_COUNTS == (24, 40, 48, 64, 72, 88)
    assert worker.fold_optimizer_updates == M03R_V14_FOLD_UPDATE_COUNTS
...
def test_v14_panel_is_two_paired_workers_and_four_h100_requests() -> None:
    panel = M03RV14PredictivePanelPlan((_worker(0), _worker(1)))
    panel.validate()
...
def test_v14_worker_rejects_uniform_64_update_reversion() -> None:
    with pytest.raises(M03RV14PredictiveWorkerError, match="worker plan drifted"):
