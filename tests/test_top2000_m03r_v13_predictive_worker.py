...
def _worker(index: int) -> M03RV13PredictiveWorkerPlan:
    return M03RV13PredictiveWorkerPlan(
        setting_index=index,
        setting_id=M03R_V13_SETTING_IDS[index],
        output_root=f"/approved/v13/setting-{index}",
...
def test_v13_worker_binds_variable_equal_epoch_fold_updates() -> None:
    worker = _worker(0)
    worker.validate()
    assert M03R_V13_FOLD_UPDATE_COUNTS == (24, 40, 48, 64, 72, 88)
    assert worker.fold_optimizer_updates == M03R_V13_FOLD_UPDATE_COUNTS
...
def test_v13_panel_is_two_paired_workers_and_four_h100_requests() -> None:
    panel = M03RV13PredictivePanelPlan((_worker(0), _worker(1)))
    panel.validate()
...
def test_v13_worker_rejects_uniform_64_update_reversion() -> None:
    with pytest.raises(M03RV13PredictiveWorkerError, match="worker plan drifted"):
