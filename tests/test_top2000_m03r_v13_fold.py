...
def test_v13_geometry_matches_context_and_consumes_pre2026_boundary() -> None:
    geometries = render_m03r_v13_fold_geometries(1001)
...
def test_v13_every_origin_appears_once_per_epoch_with_full_context() -> None:
    schedule = _schedule()
    for geometry in render_m03r_v13_fold_geometries(1001):
        for setting_index in range(2):
            ...
            for plan in plans:
                ...
                assert min(local) >= M03R_V13_MINIMUM_LOCAL_ORIGIN
                assert max(local) <= M03R_V13_MAXIMUM_LOCAL_TRAINING_ORIGIN
...
def test_v13_loss_only_setting_change_preserves_all_scheduled_inputs() -> None:
    schedule = _schedule()
    for geometry in render_m03r_v13_fold_geometries(1001):
        for update in range(geometry.optimizer_updates):
            ranked = render_m03r_v13_training_update_plan(
                schedule,
                geometry,
                setting_index=0,
                completed_update=update,
            )
            control = render_m03r_v13_training_update_plan(
                schedule,
                geometry,
                setting_index=1,
                completed_update=update,
