...
def test_v13_batch_is_h3_only_full_context_and_separates_action_mask() -> None:
...
def test_v13_batch_rejects_less_than_full_local_context() -> None:
    sequence, exposure = _surfaces()
    policy = Top2000M03RV13PredictivePolicy(
        1,
        selected_horizon_sessions=3,
    ...
    with pytest.raises(M03RV13PretrainingRuntimeError, match="full-context"):
        build_m03r_v13_batch_from_origin_states(
            policy,
            M03R_V13_SETTINGS[1],
            torch.randn((1, 1, sequence.num_assets, 16)),
...
def test_v13_optimizer_has_no_disconnected_rank_group_and_mutates_once() -> None:
    sequence, exposure = _surfaces()
    policy = Top2000M03RV13PredictivePolicy(
        0,
        selected_horizon_sessions=3,
    ...
    optimizer, partition = build_m03r_v13_optimizer(policy)
    ...
    receipt = train_m03r_v13_predictive_batch_update(
        policy,
        batch,
        optimizer,
        partition,
