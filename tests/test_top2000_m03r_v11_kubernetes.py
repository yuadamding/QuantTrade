...
from rl_quant.training.top2000_m03r_v11_seadragon_lifecycle import (
...
def test_v11_capacity_lineage_requires_exact_static_gate_receipt() -> None:
    ...
    value = {
        ...
        "reportable": False,
        "schema": "rl-quant.top2000-dev.m03r-v11-static-gate-v1",
    ...
    with pytest.raises(M03RV11SeadragonLifecycleError):
        _validate_static_gate_lineage(
            tampered,
...
def _package_and_authorization():
    ...
    package = build_m03r_v11_package_plan(
        M03RV11PackageArtifacts(
...
def _live(*, committed: int = 0):
    return build_m03r_v11_live_evidence(
...
def test_v11_capacity_and_predictive_manifests_bind_exact_authorization() -> None:
    package, authorization = _package_and_authorization()
    common = {
        "package": package,
    ...
    capacity = render_m03r_v11_suspended_capacity_job(
        **common,
        template=_template("qt-m03r-v11-cap-a02"),
    )
    predictive = render_m03r_v11_suspended_predictive_job(
        **common,
        capacity=_capacity(package, authorization),
        template=_template("qt-m03r-v11-pred-a02"),
    )
...
def test_v11_static_manifest_binds_zero_gpu_and_actual_pod_surface() -> None:
    package, authorization = _package_and_authorization()
    rendered = render_m03r_v11_suspended_static_job(
        ...
        authorization_file_sha256=AUTH_FILE_SHA,
        live=_live(),
        template=_template("qt-m03r-v11-static-a03"),
        now_utc=NOW,
    )
    ...
    binding = bind_m03r_v11_static_admitted_suspended_job(
        rendered=rendered,
        first_read=admitted,
    ...
    with pytest.raises(M03RV11StaticGateError):
        bind_m03r_v11_static_admitted_suspended_job(
            rendered=rendered,
...
def test_v11_renderer_rejects_authorization_or_capacity_drift() -> None:
    package, authorization = _package_and_authorization()
    with pytest.raises(M03RV11KubernetesError, match="package-plan file"):
        render_m03r_v11_suspended_capacity_job(
            package=package,
            authorization=authorization,
    ...
    with pytest.raises(M03RV11KubernetesError, match="does not bind"):
        render_m03r_v11_suspended_predictive_job(
            package=package,
            authorization=authorization,
            package_plan_file_sha256=PLAN_FILE_SHA,
