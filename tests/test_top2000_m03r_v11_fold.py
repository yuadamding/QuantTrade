...
def test_every_v11_setting_gets_identical_episode_and_rank_shards() -> None:
    ...
    for setting in range(3):
        worker = M03RV11PredictiveWorkerPlan(
            ...
            output_root=f"/approved/v11/{setting}",
...
def test_every_fold_update_origin_has_all_four_horizons_inside_episode() -> None:
    ...
    for setting_index, setting_id in enumerate(M03R_V11_SETTING_IDS):
        worker = M03RV11PredictiveWorkerPlan(
            setting_index=setting_index,
            setting_id=setting_id,
            output_root=f"/approved/v11/{setting_index}",
            cache_path="/approved/cache.pt",
            initial_parameter_state_path="/approved/common-initial-state.pt",
            panel_episode_schedule_sha256=schedule.receipt_sha256,
            initial_parameter_state_file_sha256="9" * 64,
        ...
        for fold in folds:
            ...
            for completed_update in range(64):
                plan = render_m03r_v11_training_shard_plan(
                    worker,
                    schedule,
                    fold,
