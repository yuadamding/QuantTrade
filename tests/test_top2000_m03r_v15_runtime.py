...
def test_v15_scale_quantiles_keep_probability_tensor_on_cuda_device() -> None:
...
def test_v15_fixed_rank_target_compares_simplex_mass_in_one_dtype() -> None:
    ...
    # This is the exact failure mode from the first v15 qualification attempt:
...
def test_v15_sleeve_uses_origin_action_mask_and_post_fill_return(
    ...
    action_operators = tuple(
        _Validated(
            ...
            base=SimpleNamespace(qualified_weights=torch.ones(3, dtype=torch.float64)),
    ...
    batch = _Validated(
        objective=objective,
        raw_predicted_mean=score.clone(),
    ...
    loaded = _Validated(
        setting_index=0,
        setting_id=M03R_V15_SETTINGS[0].setting_id,
        fold_index=0,
        asset_axis_sha256=axis,
    ...
    monkeypatch.setattr(
        "rl_quant.training.top2000_m03r_v15_runtime.project_m03r_v9_active_book",
    ...
    assert torch.allclose(
        trace.date_spearman_ic,
        torch.ones(dates, dtype=torch.float64),
        rtol=0.0,
        atol=1.0e-12,
    )
