...
from rl_quant.protocol.hold30_alpha_m03r_v15_top2000_dev import M03R_V15_SETTINGS
...
def test_v15_batch_is_h3_only_full_context_and_separates_action_mask() -> None:
    ...
    forged_valid = batch.objective.valid.clone()
    forged_valid[0, 11] = True
    with pytest.raises(M03RV15PretrainingRuntimeError, match="built batch"):
        replace(
            batch,
            objective=replace(batch.objective, valid=forged_valid),
        ).validate()
    forged_score = batch.objective.predicted_mean.clone()
    forged_score[0, 1] += 1.0e-4
    with pytest.raises(M03RV15PretrainingRuntimeError, match="built batch"):
        replace(
            batch,
            objective=replace(batch.objective, predicted_mean=forged_score),
        ).validate()


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is unavailable")
def test_v15_mask_identity_is_device_independent() -> None:
    mask = torch.tensor([[True, False, True], [False, True, False]])
    assert _tensor_sha256(mask) == _tensor_sha256(mask.to(device="cuda"))


def test_v15_batch_rejects_less_than_full_local_context() -> None:
    sequence, exposure = _surfaces()
    policy = Top2000M03RV15PredictivePolicy(
        1,
        selected_horizon_sessions=3,
    ...
    with pytest.raises(M03RV15PretrainingRuntimeError, match="full-context"):
        build_m03r_v15_batch_from_origin_states(
            policy,
            M03R_V15_SETTINGS[1],
            torch.randn((1, 1, sequence.num_assets, 16)),
...
def test_v15_optimizer_has_no_disconnected_rank_group_and_mutates_once() -> None:
    sequence, exposure = _surfaces()
    policy = Top2000M03RV15PredictivePolicy(
        0,
        selected_horizon_sessions=3,
    ...
    optimizer, partition = build_m03r_v15_optimizer(policy)
    ...
    assert all("rank" not in name for name in partition.mean_head_parameter_names)
    assert all("rank" not in name for name in partition.scale_head_parameter_names)
    assert all(
        group["weight_decay"] == 0.0
        for group in optimizer.param_groups
        if group["group_name"].endswith("no-decay")
    )
    geometry = render_m03r_v15_fold_geometries(1001)[0]
    ...
    receipt = train_m03r_v15_predictive_batch_update(
        policy,
        batch,
        optimizer,
        partition,
    ...
    assert receipt.learning_rate_multiplier == pytest.approx(
        m03r_v15_learning_rate_multiplier(plan), abs=1.0e-15
    )
    assert receipt.encoder_learning_rate <= partition.encoder_learning_rate
    assert receipt.mean_head_learning_rate <= partition.mean_head_learning_rate
    assert receipt.scale_head_learning_rate <= partition.scale_head_learning_rate


def test_v15_post_step_finite_guard_covers_parameters_and_adam_state() -> None:
