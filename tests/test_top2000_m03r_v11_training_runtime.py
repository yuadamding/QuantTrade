...
def test_v11_training_runtime_binds_one_exact_paired_rank_update(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rl_quant.training.top2000_m03r_v11_training_runtime as runtime
    ...
    monkeypatch.setattr(runtime, "train_m03r_v11_alpha_pretraining_update", _step)
    policy = SimpleNamespace(source_policy=object())
    result = run_m03r_v11_pretraining_fold_update(
        cache,
        worker,
        schedule,
        fold,
...
def test_v11_training_runtime_rejects_wrong_external_risk_manifest_before_build(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rl_quant.training.top2000_m03r_v11_training_runtime as runtime
    ...
    with pytest.raises(M03RV11TrainingRuntimeError, match="risk"):
        run_m03r_v11_pretraining_fold_update(
            cache,
            worker,
            schedule,
