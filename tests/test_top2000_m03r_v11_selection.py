...
def _fold(fold: int, *, incremental_turnover: float = 0.002) -> object:
    length = 40
    start = fold * 100
    return build_m03r_v11_fold_evidence(
        setting_index=1,
        fold_index=fold,
        horizon_sessions=30,
...
def test_joint_block_gate_and_aggregate_break_even_pass_known_process() -> None:
    ...
    plan = build_m03r_v11_bootstrap_plan(
        tuple(row.score_session_index for row in folds),  # type: ignore[attr-defined]
        bootstrap_seed=17,
    )
    result = qualify_m03r_v11_predictive_candidate(folds, plan)  # type: ignore[arg-type]
...
def test_break_even_uses_aggregate_sign_cases_not_fold_ratio_average() -> None:
    ...
    del weak
    failed = qualify_m03r_v11_predictive_candidate(negative, plan)
