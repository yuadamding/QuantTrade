...
def _package_and_authorization() -> tuple[
    ...
    image = "f" * 64
    package = build_m03r_v14_package_plan(
        M03RV14PackageArtifacts(
...
def _live(*, committed: int = 0) -> M03RV14LiveEvidence:
    return build_m03r_v14_live_evidence(
...
def _template(name: str) -> M03RV7KubernetesTemplateConfig:
    return M03RV7KubernetesTemplateConfig(
        job_name=name,
        run_id="qt-m03r-v14-context-h3-s17-20260814-a03",
...
def test_v14_static_capacity_and_predictive_shapes_are_bounded() -> None:
    package, authorization = _package_and_authorization()
    live = _live()
    static = render_m03r_v14_suspended_static_job(
        package=package,
        authorization=authorization,
        package_plan_file_sha256=PLAN_FILE_SHA,
        authorization_file_sha256=AUTH_FILE_SHA,
        live=live,
        template=_template("qt-m03r-v14-static-a03"),
        now_utc=NOW,
    )
    capacity = render_m03r_v14_suspended_capacity_job(
        package=package,
        authorization=authorization,
        package_plan_file_sha256=PLAN_FILE_SHA,
        authorization_file_sha256=AUTH_FILE_SHA,
        live=live,
        template=_template("qt-m03r-v14-cap-a03"),
        now_utc=NOW,
    )
    predictive = render_m03r_v14_suspended_predictive_job(
        package=package,
        authorization=authorization,
        package_plan_file_sha256=PLAN_FILE_SHA,
        authorization_file_sha256=AUTH_FILE_SHA,
        ...
        template=_template("qt-m03r-v14-pred-a03"),
        now_utc=NOW,
    )
...
def test_v14_live_cap_reduces_predictive_parallelism_without_changing_coverage() -> None:
    package, authorization = _package_and_authorization()
    rendered = render_m03r_v14_suspended_predictive_job(
        ...
        template=_template("qt-m03r-v14-pred-a04"),
        now_utc=NOW,
    )
