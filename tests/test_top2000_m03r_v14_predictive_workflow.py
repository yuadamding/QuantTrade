...
from rl_quant.workflows.top2000_m03r_v14_predictive import (
...
def test_v14_completion_index_is_exactly_two_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("JOB_COMPLETION_INDEX", "1")
    assert resolve_m03r_v14_completion_index(None) == 1
    assert resolve_m03r_v14_completion_index(0) == 0
    for invalid in (-1, 2, True):
        with pytest.raises(M03RV14PredictiveWorkflowError, match="0 or 1"):
            resolve_m03r_v14_completion_index(invalid)


def test_v14_rank_update_requires_equal_state_and_complete_origin_shards() -> None:
    rows = [_rank_row(0), _rank_row(1)]
    _validate_gathered_update(rows, 2)

    drifted = [rows[0], {**rows[1], "model_state_after_sha256": "f" * 64}]
    with pytest.raises(M03RV14PredictiveWorkflowError, match="diverged"):
...
def test_v14_workflow_source_is_predictive_only_and_v14_homogeneous() -> None:
    source = Path(
    ...
    assert "run_m03r_v14_fold_qualification" in source
    assert "load_m03r_v14_alpha_checkpoint_for_evaluation" in source
