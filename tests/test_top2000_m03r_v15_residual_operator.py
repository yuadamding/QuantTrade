...
def _operator() -> object:
    ...
    weights[0] = 0.0
    return build_m03r_v15_residual_operator(
...
def test_v15_precomputed_map_matches_qualified_qr_and_preserves_gradients() -> None:
    operator = _operator()
    value = torch.tensor(
    ...
    observed = apply_m03r_v15_residual_operator(value, operator)  # type: ignore[arg-type]
...
def test_v15_apply_does_not_repeat_qr_factorization(
    ...
    first = apply_m03r_v15_residual_operator(value, operator)  # type: ignore[arg-type]
    second = apply_m03r_v15_residual_operator(2.0 * value, operator)  # type: ignore[arg-type]
    assert torch.allclose(second.residual, 2.0 * first.residual)


def test_v15_projection_is_contractive_in_its_weighted_action_space() -> None:
    operator = _operator()
    value = torch.randn(9, dtype=torch.float64)
    value[0] = 0.0
    observed = apply_m03r_v15_residual_operator(value, operator)  # type: ignore[arg-type]
    mask = operator.qualified_asset_mask  # type: ignore[attr-defined]
    weights = operator.base.qualified_weights  # type: ignore[attr-defined]
    before = torch.sqrt((weights * value[mask].square()).sum())
    after = torch.sqrt((weights * observed.residual[mask].square()).sum())
    assert float(after) <= float(before) + 1.0e-12
