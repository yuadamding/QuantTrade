...
from rl_quant.features.massive_economic_history_v3 import (
...
def _identity(
    ...
    rule = PITUniverseRuleSpec.build(
        rule_id="economic-history-v3-test",
        target_size=2,
...
def _record(source_kind: str, **values: object) -> dict[str, object]:
    row = dict(values)
    row["source_row_receipt_sha256"] = economic_event_source_row_receipt_v3(
...
def _corporate(
    ...
    *,
    ...
    return _record(
        ...
        effective_timestamp_contract=expected_effective_timestamp_contract_v3(
...
def _publish_source(
    ...
    *,
    ...
    publish_massive_source_object(
        ...
        root=root,
        relative_payload_path=relative,
        dataset_id=MASSIVE_ECONOMIC_EVENT_SOURCE_V3_DATASETS[source_kind],
        source_object_key=relative,
        requested_at_ms=1,
        downloaded_at_ms=1,
        schema_sha256=MASSIVE_ECONOMIC_EVENT_SOURCE_V3_SOURCE_SCHEMA_SHA256,
        entitlement_receipt_sha256=DIGEST,
        committed_at_ms=1,
    )
...
def _authority(
    ...
    *,
    ...
    return build_massive_economic_event_authority_v3(
        root=tmp_path,
...
def _run(
    ...
    *,
    ...
    origin = build_massive_economic_history_origin_v3(
...
def test_explicit_sequence_controls_same_time_successor_event(tmp_path: Path) -> None:
    sessions = _sessions()
...
def test_event_id_closes_same_time_same_sequence_successor_order(
    tmp_path: Path,
) -> None:
    sessions = _sessions()
    effective = sessions.sessions[4].regular_close_ns // 1_000_000 - 1
    ...
    assert b_lot.acquisition_order == (effective, 0, "A-SPIN-FIRST")


def test_cash_return_and_distribution_use_complete_global_order(
    tmp_path: Path,
) -> None:
    sessions = _sessions()
    effective = sessions.sessions[4].regular_close_ns // 1_000_000 - 1
    cases = (
        ("dividend-first", "A-DIVIDEND", "Z-CASH-RETURN", 111.0),
        ("cash-first", "Z-DIVIDEND", "A-CASH-RETURN", 110.0),
    )
    for case_name, dividend_id, cash_id, expected_value in cases:
        ...
        source_a = [row for row in rows if row.security_id == "SEC-A"][-1]
        assert source_a.economic_value == pytest.approx(expected_value)
...
def test_event_sequence_cannot_reach_base_sentinel(tmp_path: Path) -> None:
    sessions = _sessions()
    identity = _identity(sessions)
    row = _corporate(
        "INVALID-SEQUENCE",
        ...
        CorporateActionKind.CASH_DIVIDEND,
        sessions.sessions[3].regular_close_ns // 1_000_000 - 1,
        event_sequence=MASSIVE_ECONOMIC_BASE_SEQUENCE_V3,
        cash_per_share=1.0,
    )
    ...
    with pytest.raises(MassiveEconomicEventSourceV3Error, match="base sentinel"):
        parse_massive_economic_event_source_v3(root=tmp_path, loaded_source=loaded)
...
def test_materialized_history_round_trips_and_detects_byte_tampering(
    ...
    artifact = materialize_massive_economic_history_v3(
    ...
    validate_massive_economic_history_v3(root=output_root, artifact=artifact)
