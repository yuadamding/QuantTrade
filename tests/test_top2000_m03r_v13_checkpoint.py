...
def _policy() -> Top2000M03RV13PredictivePolicy:
...
def _identity() -> dict[str, object]:
    return {
        "fold_index": 2,
        "completed_updates": render_m03r_v13_fold_geometries(1001)[
            2
        ].optimizer_updates,
...
def test_v13_checkpoint_round_trip_evaluates_exact_loaded_bytes(tmp_path: Path) -> None:
    path = tmp_path / "checkpoint.pt"
    loaded, observed = write_reload_evaluate_m03r_v13_checkpoint(
        path,
        _policy(),
        _policy,
        lambda policy, receipt: (
            policy.v13_head_identity(),
...
def test_v13_checkpoint_rejects_wrong_fold_update_count(tmp_path: Path) -> None:
    identity = _identity()
    identity["completed_updates"] = int(identity["completed_updates"]) - 1
    with pytest.raises(M03RV13CheckpointError, match="cursor"):
        write_immutable_m03r_v13_alpha_checkpoint(
...
def test_v13_checkpoint_rejects_missing_direct_mean_state(tmp_path: Path) -> None:
    clean = tmp_path / "clean.pt"
    file_sha = write_immutable_m03r_v13_alpha_checkpoint(
        clean,
        _policy(),
        **_identity(),  # type: ignore[arg-type]
    )
    ...
    identity = _identity()
    with pytest.raises(M03RV13CheckpointError, match="strict load"):
        load_m03r_v13_alpha_checkpoint_for_evaluation(
            tampered,
