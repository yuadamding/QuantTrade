...
def test_v12_panel_requires_shared_schedule_inputs_and_initial_state() -> None:
    panel = M03RV12PredictivePanelPlan(tuple(_worker(index) for index in range(3)))
    panel.validate()
...
def test_v12_panel_rejects_setting_specific_schedule_or_launch_authority() -> None:
    ...
    with pytest.raises(M03RV12PredictiveWorkerError, match="panel plan"):
        M03RV12PredictivePanelPlan(drifted).validate()
    with pytest.raises(M03RV12PredictiveWorkerError, match="worker plan"):
