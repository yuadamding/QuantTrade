...
def test_v13_real_preflight_covers_every_origin_and_round_trips(
    monkeypatch: Any,
    tmp_path: Path,
) -> None:
    import rl_quant.training.top2000_m03r_v13_preflight as preflight
    ...
    monkeypatch.setattr(preflight, "build_m03r_v11_residual_operator", _operator)
    receipt = run_m03r_v13_structural_preflight(cache, risk)  # type: ignore[arg-type]
    ...
    path = tmp_path / "v13-structural-preflight.json"
    file_sha = write_m03r_v13_structural_preflight(path, receipt)
    loaded = load_m03r_v13_structural_preflight(
        path,
