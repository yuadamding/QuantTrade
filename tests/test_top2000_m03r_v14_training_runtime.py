...
def _schedule_and_worker() -> tuple[
    ...
    worker = M03RV14PredictiveWorkerPlan(
        setting_index=0,
        setting_id=M03R_V14_SETTING_IDS[0],
        output_root="/approved/v14/setting-0",
...
def test_v14_runtime_uses_exact_full_context_rank_shard(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rl_quant.training.top2000_m03r_v14_training_runtime as runtime

    schedule, worker = _schedule_and_worker()
    geometry = render_m03r_v14_fold_geometries(1001)[2]
    ...
    monkeypatch.setattr(runtime, "train_m03r_v14_predictive_batch_update", _step)
    policy = SimpleNamespace(
        v14_setting=SimpleNamespace(setting_index=0), source_policy=object()
    )
...
def test_v14_runtime_rejects_risk_manifest_before_episode_build(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rl_quant.training.top2000_m03r_v14_training_runtime as runtime
    ...
    with pytest.raises(M03RV14TrainingRuntimeError, match="risk"):
        run_m03r_v14_pretraining_fold_update(
            cache,
            worker,
            schedule,
