...
def _policy(setting: int) -> Top2000M03RV14PredictivePolicy:
...
def test_v14_common_initial_bytes_pair_both_settings(tmp_path: Path) -> None:
    torch.manual_seed(17)
    canonical = _policy(0)
    path = tmp_path / "common-v14-initial-state.pt"
    state_sha, file_sha, architecture_sha = write_m03r_v14_initial_parameter_state(
        path, canonical
    )
    ...
    assert model_state_sha256(control) == state_sha
    assert control.v14_setting.setting_index == 1


def test_v14_common_initial_state_rejects_tampered_file(tmp_path: Path) -> None:
    torch.manual_seed(17)
    path = tmp_path / "common-v14-initial-state.pt"
    state_sha, file_sha, architecture_sha = write_m03r_v14_initial_parameter_state(
        path, _policy(0)
    ...
    with pytest.raises(M03RV14InitialStateError, match="file hash"):
        load_m03r_v14_initial_parameter_state(
            path,
            _policy(1),
            expected_file_sha256=file_sha,
