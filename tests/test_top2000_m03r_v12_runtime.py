...
from rl_quant.training.top2000_m03r_v12_runtime import (
    M03RV12RuntimeError,
...
def _distributions() -> tuple[M03RV12AlphaDistribution, ...]:
    rows = []
    for index in range(3):
        means = torch.zeros((1, 4, 5), dtype=torch.float64)
        means[0, 1:, 0] = torch.tensor([0.01, -0.01, 0.02]) + index * 0.001
        ...
        rows.append(
            M03RV12AlphaDistribution(
                ...
                selected_scale=torch.exp(log_scale[..., 0]),
            )
        )
    return tuple(rows)


def test_v12_sleeve_routes_exact_shared_operators_and_v4_allocator(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rl_quant.training.top2000_m03r_v12_runtime as runtime
    ...
    def _proposal(anchor: torch.Tensor, *args: object, **kwargs: object) -> object:
        assert kwargs["selected_horizon_sessions"] == 3
        return SimpleNamespace(
            proposal=SimpleNamespace(requested_weights=anchor.clone())
        )
    ...
    monkeypatch.setattr(runtime, "build_cost_aware_active_proposal_v4", _proposal)
    trace = run_m03r_v12_simple_sleeve(
        sequence,
        _distributions(),
    ...
    assert trace.v12_shared_residual_operator_used
    assert trace.v12_cost_aware_allocator_used
    assert not trace.imported_v9_signal_projector_used


def test_v12_sleeve_rejects_operator_from_wrong_origin_before_trade(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rl_quant.training.top2000_m03r_v12_runtime as runtime
    ...
    with pytest.raises(M03RV12RuntimeError, match="operator"):
        run_m03r_v12_simple_sleeve(
            _sequence(),
            _distributions(),
            operators,  # type: ignore[arg-type]
