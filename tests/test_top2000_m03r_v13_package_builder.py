...
from rl_quant.workflows.top2000_m03r_v13_package_builder import (
...
def _inputs(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    worker = source / "src/rl_quant/workflows/top2000_m03r_v13_predictive.py"
...
def _preflight(tmp_path: Path, cache: Path) -> Path:
    ...
    write_m03r_v13_structural_preflight(path, receipt)
    return path


def test_v13_builder_mints_predictive_only_package_and_deterministic_archive(
    tmp_path: Path,
) -> None:
    source, cache, cache_manifest, risk = _inputs(tmp_path)
    output = tmp_path / "stage"
    receipt = build_m03r_v13_local_package(
        source_root=source,
        cache_path=cache,
    ...
    assert validate_m03r_v13_local_package(output)["receipt_sha256"] == receipt[
        "receipt_sha256"
    ]
    ...
    first = build_m03r_v13_transfer_archive(output, tmp_path / "first.tar")
    second = build_m03r_v13_transfer_archive(output, tmp_path / "second.tar")
...
def test_v13_builder_rejects_member_tamper(tmp_path: Path) -> None:
    source, cache, cache_manifest, risk = _inputs(tmp_path)
    output = tmp_path / "stage"
    build_m03r_v13_local_package(
        source_root=source,
    ...
    with pytest.raises(M03RV13PackageBuildError, match="inventory"):
