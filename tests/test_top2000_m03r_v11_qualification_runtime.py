...
import rl_quant.training.top2000_m03r_v11_runtime as sleeve_runtime
...
def _lineage(fold_index: int) -> M03RV11FoldQualificationLineage:
...
def _qualification_batch_and_trace() -> tuple[
    ...
    dates, assets = 3, 4
    prediction = torch.zeros((dates, assets, 4), dtype=torch.float64)
    ...
    operator_receipts = tuple(f"{index + 1:x}" * 64 for index in range(12))
    batch = M03RV11AlphaPretrainingBatch(
        corrected_batch=base,
        setting=resolve_m03r_v11_setting(1),
        residual_operator_receipt_sha256=operator_receipts,
        available_risky_asset_count=(3,) * 12,
        factor_qualified_risky_asset_count=(3,) * 12,
        effective_design_rank=(2,) * 12,
        weighted_residual_degrees_of_freedom=(1,) * 12,
    ...
    provisional = M03RV11SimpleSleeveTrace(
        ...
        signal_operator_receipt_sha256=tuple(
            operator_receipts[
                date * len(M03R_V11_HORIZONS) + M03R_V11_HORIZONS.index(30)
            ]
            for date in range(dates)
        ),
...
def test_round_trip_candidate_requires_six_exact_checkpoint_lineages() -> None:
    ...
    result = qualify_m03r_v11_round_trip_candidate(lineages, bootstrap)
...
def test_round_trip_candidate_rejects_rehashed_evidence_for_other_checkpoint() -> None:
    ...
    with pytest.raises(M03RV11QualificationRuntimeError, match="reloaded checkpoint"):
        drifted.validate()


def test_fold_builder_derives_metrics_from_exact_batch_and_v11_trace() -> None:
    loaded, batch, trace = _qualification_batch_and_trace()
    result = build_m03r_v11_fold_qualification_lineage(loaded, batch, trace)
...
def test_fold_builder_rejects_trace_using_other_operator_inventory() -> None:
    ...
    with pytest.raises(M03RV11QualificationRuntimeError, match="drifted"):
        build_m03r_v11_fold_qualification_lineage(loaded, batch, drifted)
