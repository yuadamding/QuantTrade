...
def _artifacts() -> M03RV14PackageArtifacts:
    image_digest = "f" * 64
    return M03RV14PackageArtifacts(
...
def test_v14_package_plan_binds_two_workers_and_round_trips(tmp_path: Path) -> None:
    plan = build_m03r_v14_package_plan(_artifacts(), _schedule())
    ...
    file_sha = write_m03r_v14_package_plan(path, plan)
    assert load_m03r_v14_package_plan(path, expected_file_sha256=file_sha) == plan


def test_v14_package_plan_rejects_worker_artifact_drift() -> None:
    plan = build_m03r_v14_package_plan(_artifacts(), _schedule())
    workers = list(plan.panel.workers)
    workers[1] = replace(workers[1], projector_manifest_sha256="f" * 64)
    with pytest.raises(M03RV14PredictiveWorkerError, match="panel"):
        replace(plan, panel=replace(plan.panel, workers=tuple(workers))).validate()


def test_v14_execution_authorization_is_exact_and_predictive_only(
    tmp_path: Path,
) -> None:
    plan = build_m03r_v14_package_plan(_artifacts(), _schedule())
    plan_path = tmp_path / "package-plan.json"
    plan_file_sha = write_m03r_v14_package_plan(plan_path, plan)
    ...
    file_sha = write_m03r_v14_execution_authorization(path, authorization, plan)
    loaded = load_m03r_v14_execution_authorization(
        path,
        expected_file_sha256=file_sha,
        package=plan,
    )
    assert loaded == authorization
    assert loaded.runtime_entrypoint == M03R_V14_RUNTIME_ENTRYPOINT
...
def test_v14_execution_authorization_rejects_economic_or_2026_access(
    tmp_path: Path,
) -> None:
    plan = build_m03r_v14_package_plan(_artifacts(), _schedule())
    plan_path = tmp_path / "package-plan.json"
