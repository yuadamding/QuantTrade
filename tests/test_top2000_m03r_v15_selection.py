...
def test_v15_joint_gate_passes_only_complete_positive_evidence() -> None:
    traces = _traces(gross=0.001)
    plan = build_m03r_v15_bootstrap_plan(
        tuple(row.origin_indices for row in traces)
    )
    result = qualify_m03r_v15_predictive_candidate(traces, plan)  # type: ignore[arg-type]
    assert result.setting_id == M03R_V15_SETTING_IDS[0]
...
def test_v15_joint_gate_stops_negative_gross_evidence() -> None:
    traces = _traces(gross=-0.001)
    plan = build_m03r_v15_bootstrap_plan(
        tuple(row.origin_indices for row in traces)
    )
