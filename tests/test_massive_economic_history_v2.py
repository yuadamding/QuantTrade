...
from rl_quant.features.massive_economic_history_v2 import (
...
def _record(source_kind: str, **values: object) -> dict[str, object]:
    row = dict(values)
    row["source_row_receipt_sha256"] = economic_event_source_row_receipt_v2(
...
def _publish_source(
    ...
    *,
    ...
    publish_massive_source_object(
        ...
        root=root,
        relative_payload_path=relative,
        dataset_id=MASSIVE_ECONOMIC_EVENT_SOURCE_V2_DATASETS[source_kind],
        source_object_key=relative,
        requested_at_ms=1,
        downloaded_at_ms=1,
        schema_sha256=MASSIVE_ECONOMIC_EVENT_SOURCE_V2_SOURCE_SCHEMA_SHA256,
        entitlement_receipt_sha256=DIGEST,
        committed_at_ms=1,
    )
...
def _authority(
    ...
    *,
    ...
    cash = _record(
        ...
        one_step_return=0.0,
        event_sequence=0,
        upstream_source_receipt_sha256="6" * 64,
    )
    loaded = (
        _publish_source(
            tmp_path,
    ...
    return build_massive_economic_event_authority_v2(
        root=tmp_path,
...
def _run(
    ...
    authority = _authority(tmp_path, identity=identity, corporate=corporate)
    origin = build_massive_economic_history_origin_v2(
...
def test_committed_source_row_cannot_be_changed_under_retained_receipt(
    ...
    with pytest.raises(
        ...
        parse_massive_economic_event_source_v2(root=tmp_path, loaded_source=loaded)
...
def test_materialized_history_round_trips_and_detects_byte_tampering(
    ...
    artifact = materialize_massive_economic_history_v2(
    ...
    validate_massive_economic_history_v2(root=output_root, artifact=artifact)
