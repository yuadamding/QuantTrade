...
def _artifacts() -> M03RV11PackageArtifacts:
    return M03RV11PackageArtifacts(
...
def test_v11_package_round_trip_binds_common_schedule_and_stays_unlaunched(
    tmp_path: Path,
) -> None:
    plan = build_m03r_v11_package_plan(_artifacts(), _schedule())
    path = tmp_path / "v11-package.json"
    file_sha = write_m03r_v11_package_plan(path, plan)
    loaded = load_m03r_v11_package_plan(path, expected_file_sha256=file_sha)
...
def test_v11_package_rejects_cache_or_worker_artifact_drift(tmp_path: Path) -> None:
    with pytest.raises(M03RV11PackageError, match="cache"):
        build_m03r_v11_package_plan(
    ...
    path = tmp_path / "v11-package.json"
    file_sha = write_m03r_v11_package_plan(path, plan)
    with pytest.raises(M03RV11PackageError, match="file hash"):
        load_m03r_v11_package_plan(path, expected_file_sha256="f" * 64)
    assert len(file_sha) == 64


def test_v11_separate_execution_authorization_binds_exact_unlaunched_package(
    tmp_path: Path,
) -> None:
    package = build_m03r_v11_package_plan(_artifacts(), _schedule())
    ...
    loaded = load_m03r_v11_execution_authorization(
        authorization_path,
        expected_file_sha256=authorization_file_sha,
        package=package,
    )
    assert loaded.runtime_entrypoint == M03R_V11_RUNTIME_ENTRYPOINT
    ...
    with pytest.raises(M03RV11PackageError, match="authorization"):
