...
def _inputs() -> tuple[Any, ...]:
    geometry = render_m03r_v15_fold_geometries(1001)[5]
    ...
    policy = SimpleNamespace(
        v15_setting=SimpleNamespace(setting_index=0),
...
def test_v15_qualification_replays_exact_full_context_tail(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rl_quant.training.top2000_m03r_v15_qualification_runtime as runtime
    ...
    result = run_m03r_v15_fold_qualification(
        cache,
        geometry,
        risk_source,
        risk_state,
...
def test_v15_qualification_rejects_model_drift_before_cache_build(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rl_quant.training.top2000_m03r_v15_qualification_runtime as runtime
    ...
    with pytest.raises(M03RV15QualificationRuntimeError, match="model"):
        run_m03r_v15_fold_qualification(
            cache,
            geometry,
            risk_source,
...
def test_v15_risk_state_uses_exact_fold_origins_and_past_returns(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import rl_quant.training.top2000_m03r_v15_qualification_runtime as runtime
    ...
    result = build_m03r_v15_qualification_risk_state(
        cache,
        geometry,
        risk,
