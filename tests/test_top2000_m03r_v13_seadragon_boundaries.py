...
def _package_authorization(tmp_path: Path):
    ...
    package_file_sha = write_m03r_v13_package_plan(package_path, package)
...
def _qualification(
    *,
    ...
    return M03RV13PredictiveQualification(
        setting_index=setting_index,
        setting_id=M03R_V13_SETTING_IDS[setting_index],
...
def test_v13_attach_config_binds_authorization_and_two_workers(
    ...
    config = lifecycle.M03RV13AttachSupervisorConfig(
        job_name="qt-m03r-v13-predictive-a04",
        run_id="qt-m03r-v13-predictive-s17-20260814-a04",
        job_uid="job-uid",
...
def test_v13_lifecycle_reconstructs_exact_typed_qualification() -> None:
    ...
    with pytest.raises(lifecycle.M03RV13SeadragonLifecycleError):
        lifecycle._validate_qualification(
            value,
            setting_index=0,
            expected_fold_traces=list(qualification.fold_trace_sha256),
...
def test_v13_workflow_file_hash_convention_is_newline_bound() -> None:
...
def test_v13_operator_and_lifecycle_have_disjoint_mutation_surfaces() -> None:
    ...
    assert "top2000_m03r_v13_static_validate" not in inspect.getsource(static_gate)


def test_v13_create_config_is_two_worker_predictive_only(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    assert operator.M03RV13CreateOperatorConfig(**fields).completions == 2
    with pytest.raises(operator.M03RV13SeadragonOperatorError, match="identity"):
        operator.M03RV13CreateOperatorConfig(
...
def test_v13_worker_validator_matches_exact_v13_artifacts(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(lifecycle, "SEADRAGON_QUANTTRADE_ROOT", str(tmp_path))
    ...
    startup = {
        "schema": lifecycle.M03R_V13_STARTUP_SCHEMA,
        "package_plan_sha256": package.package_plan_sha256,
        "package_plan_file_sha256": package_sha,
    ...
    for fold_index, completed_updates in enumerate(worker.fold_optimizer_updates):
        ...
        unsigned = {
            "schema": lifecycle.M03R_V13_FOLD_TERMINAL_SCHEMA,
            "package_plan_sha256": package.package_plan_sha256,
            "authorization_receipt_sha256": auth.receipt_sha256,
            "worker_plan_sha256": worker.receipt_sha256,
            "setting_index": 0,
    ...
    bootstrap = {
        ...
        "fold_lengths": [63] * 6,
        "bootstrap_seed": lifecycle.M03R_V13_PREDICTIVE_SPEC.bootstrap_seed,
        "draw_sha256_by_block": ["a" * 64, "b" * 64, "c" * 64],
        "block_sessions": [10, 21, 30],
        "replicates": lifecycle.M03R_V13_PREDICTIVE_SPEC.bootstrap_replicates,
        "protocol_sha256": lifecycle.M03R_V13_PROTOCOL_SHA256,
        "schema": lifecycle.M03R_V13_BOOTSTRAP_PLAN_SCHEMA,
    }
    ...
    unsigned_terminal = {
        "schema": lifecycle.M03R_V13_WORKER_TERMINAL_SCHEMA,
        "package_plan_sha256": package.package_plan_sha256,
        "package_plan_file_sha256": package_sha,
        "authorization_receipt_sha256": auth.receipt_sha256,
        "worker_plan_sha256": worker.receipt_sha256,
